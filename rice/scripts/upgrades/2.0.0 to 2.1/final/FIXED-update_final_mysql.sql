...
((select KIM_TYP_ATTR_ID from (select (max(cast(KIM_TYP_ATTR_ID as decimal)) + 1)
  ...
  where KIM_TYP_ATTR_ID is not NULL and KIM_TYP_ATTR_ID rlike '^[1-9][0-9]{0,3}$' ) as tmptable),
...
values ((select perm_id from (select (max(cast(perm_id as decimal)) + 1) as perm_id from krim_perm_t where perm_id is not NULL and perm_id rlike '^[1-9][0-9]{0,3}$' )
         as tmptable),
...
values ((select attr_data_id from
          (select (max(cast(attr_data_id as decimal)) + 1) as attr_data_id from krim_perm_attr_data_t where attr_data_id is not NULL and attr_data_id rlike '^[1-9][0-9]{0,3}$' )
         as tmptable),
...
values ((select role_perm_id from
          (select (max(cast(role_perm_id as decimal)) + 1) as role_perm_id from krim_role_perm_t where role_perm_id is not NULL and role_perm_id rlike '^[1-9][0-9]{0,3}$' )
         as tmptable),
