...
-- this is redundant to the create table statement above
...
values ((select perm_tmpl_id from
        (select (max(cast(perm_tmpl_id as decimal)) + 1) as perm_tmpl_id from krim_perm_tmpl_t where perm_tmpl_id is not NULL and perm_tmpl_id rlike '^[1-9][0-9]{0,3}$' )
        as tmptable),
...
values ((select perm_id from
        (select (max(cast(perm_id as decimal)) + 1) as perm_id from krim_perm_t where perm_id is not NULL and perm_id rlike '^[1-9][0-9]{0,3}$' )
        as tmptable),
...
values ((select role_id from
        (select (max(cast(role_id as decimal)) + 1) as role_id from krim_role_t where role_id is not NULL and role_id rlike '^[1-9][0-9]{0,3}$' )
        as tmptable),
...
values ((select role_mbr_id from
        (select (max(cast(role_mbr_id as decimal)) + 1) as role_mbr_id from krim_role_mbr_t where role_mbr_id is not NULL and role_mbr_id rlike '^[1-9][0-9]{0,3}$' )
        as tmptable),
