...
public class SendNotificationMessageController extends BaseSendNotificationController {
    ...
    private boolean hasPotentialRecipients(NotificationBo notification) {
        ...
        return notification.getChannel().getRecipientLists().size() > 0
                ...
                ||
                (notification.getChannel().isSubscribable() && timeIsInTheFuture(notification.getSendDateTimeValue()
                        .getTime()));
    }
    ...
    public ModelAndView submitSimpleNotificationMessage(
        ...
        try {
            ...
            NotificationBo notification = populateNotificationInstance(request,
                    model);
