...
<%@ attribute name="displayInvoiceInfoFields" required="false"
...
<kul:tab tabTitle="Invoice Info" defaultOpen="true" tabErrorKey="${PurapConstants.PAYMENT_REQUEST_INVOICE_TAB_ERRORS}">
    <div class="tab-container" align=center>
        ...
        <table cellpadding="0" cellspacing="0" class="datatable" summary="Invoice Info Section">
            ...
            <tr>
                <th align=right valign=middle class="bord-l-b">
                   <div align="right"><kul:htmlAttributeLabel attributeEntry="${documentAttributes.invoicePayDate}" /></div>
                </th>
                <td align=left valign=middle class="datacell">
                   <kul:htmlControlAttribute 
                   		attributeEntry="${documentAttributes.invoicePayDate}" property="document.invoicePayDate" datePicker="true"
            ...
            <tr>
                ...
                <th align=right valign=middle class="bord-l-b">
                   <div align="right"><kul:htmlAttributeLabel  attributeEntry="${documentAttributes.invoiceCostSourceCode}" readOnly="${not displayInitTab}" /></div>
                </th>
                <td align=left valign=middle class="datacell">
                   <kul:htmlControlAttribute attributeEntry="${documentAttributes.invoiceCostSourceCode}"
                   property="document.invoiceCostSourceCode"
                   extraReadOnlyProperty="document.invoiceCostSource.purchaseOrderCostSourceDescription"
                   readOnly="true" />
                </td>
            </tr>
            ...
            <tr>
                ...
                <td align=left valign=middle class="datacell">
                	<c:choose>
                	<c:when test="${not fullDocumentEntryCompleted}">                
                	<kul:htmlControlAttribute attributeEntry="${documentAttributes.vendorInvoiceAmount}" property="document.vendorInvoiceAmount" readOnly="${not KualiForm.document.vendorDetail.vendorHeader.vendorForeignIndicator}" />
					</c:when>
					<c:otherwise>
					&nbsp;
					</c:otherwise>
			...
			<tr>
			   ...
			   <tr>
                    ...
                    <td align=left valign=middle class="datacell">
                        ...
                        <kul:htmlControlAttribute attributeEntry="${DataDictionary.OleInvoiceDocument.attributes.invoiceType}" property="document.oleInvoiceType.invoiceType" extraReadOnlyProperty = "document.oleInvoiceType.invoiceType"  readOnly="${not (fullEntryMode or editPreExtract)}" />
                        <c:if test="${ (fullEntryMode or editPreExtract) }">
                        <kul:lookup boClassName="org.kuali.ole.select.businessobject.OleInvoiceType"  
									fieldConversions="invoiceTypeId:document.invoiceTypeId,invoiceType:document.invoiceType,invoiceType:document.oleInvoiceType.invoiceType"/>
                    </c:if>
                    </td>
                    <th align=right valign=middle class="bord-l-b">
                    ...
                    <td align=left valign=middle class="datacell">
                        <kul:htmlControlAttribute attributeEntry="${DataDictionary.OleInvoiceDocument.attributes.invoiceSubType}" property="document.oleInvoiceSubType.invoiceSubType" extraReadOnlyProperty = "document.oleInvoiceSubType.invoiceSubType" readOnly="${not (fullEntryMode or editPreExtract)}" />
                        <c:if test="${ (fullEntryMode or editPreExtract) }">
                        <kul:lookup boClassName="org.kuali.ole.select.businessobject.OleInvoiceSubType"  
									fieldConversions="invoiceSubTypeId:document.invoiceSubTypeId,invoiceSubType:document.invoiceSubType,invoiceSubType:document.oleInvoiceSubType.invoiceSubType"/>
                    </c:if>
                    </td>
			...
			<tr>
                    ...
                    <td align=left valign=middle class="datacell">
                    	<kul:htmlControlAttribute attributeEntry="${DataDictionary.OleInvoiceDocument.attributes.paymentMethodId}" property="document.paymentMethod.paymentMethodId"  extraReadOnlyProperty = "document.paymentMethod.paymentMethod" readOnly="${not (fullEntryMode or editPreExtract)}" />
                      	<c:if test="${ (fullEntryMode or editPreExtract) }">
                      		<kul:lookup boClassName="org.kuali.ole.select.businessobject.OlePaymentMethod"  
                                                                       fieldConversions="paymentMethodId:document.paymentMethod.paymentMethodId,paymentMethod:document.paymentMethod.paymentMethod"/>
                        </c:if> 
                    </td>
            </tr>   
