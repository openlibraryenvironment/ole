	    ...
	    <tr style="display: none;"  id="tab-${tabKey}-div">
          ...
          <c:if test="${fullEntryMode}">
          		<tr>
		        	...
		        	<th>
                        <kul:htmlAttributeLabel attributeEntry="${itemAttributes.receiptNotes}" />
                    </th>
		        ...
		        <tr>
			        ...
			        <td class="infoline">
					    <kul:htmlControlAttribute attributeEntry="${itemAttributes.receiptNotes}" property="${accountPrefix}receiptNotes" tabindexOverride="${tabindexOverrideBase + 0}"/>
				    </td>
				    <td class="infoline" colspan="${colSpanAction}">
					    <div align="center">
					        <html:image property="methodToCall.addReceiptNote.line${count}" src="${ConfigProperties.kr.externalizable.images.url}tinybutton-add1.gif" alt="Add" title="Add a Receipt Note" styleClass="tinybutton" tabindex="${tabindexOverrideBase + 0}"/>
					    </div>
					</td>	
		        </tr>	     
			</c:if>
			 <logic:notEmpty name="KualiForm" property="${accountPrefix}correctionReceiptNoteList">
			 	<tr>
			         <th>
			            <kul:htmlAttributeLabel attributeEntry="${itemAttributes.noteTypeId}" /></div>
			         </th>
			         <th>
	                    <kul:htmlAttributeLabel attributeEntry="${itemAttributes.receiptNotes}" /></div>
	                 </th>
	                 <c:choose>		    	
		     ...
		     <logic:iterate indexId="ctr" name="KualiForm" property="${accountPrefix}correctionReceiptNoteList" id="itemNotes">
				<c:if test="true">				
				<c:choose>	
				...
				<tr>
						...
						<c:when test="${fullEntryMode}">
							<td class="infoline">
							 <kul:htmlControlAttribute
				                property="${accountPrefix}correctionReceiptNoteList[${ctr}].noteTypeId"
							...
							<td class="infoline">
							    <kul:htmlControlAttribute 
		                        	attributeEntry="${itemAttributes.receiptNotes}" property="${accountPrefix}correctionReceiptNoteList[${ctr}].notes" 
					...
					<td class="infoline">
					    ...
					    <c:choose>		    	
					    	<c:when test="${itemNotes.objectId == null}">				    
					        	<html:image
						        	property="methodToCall.deleteReceiptNote.line${count}:${ctr}"
