            ...
            <td>
			  <c:choose>
				<c:when test="${isTransactionalDocument}">
				  <html:link target="_blank" styleClass="portal_link" title="Payment Information" href="kr/lookup.do?methodToCall=search&businessObjectClassName=org.kuali.ole.module.cam.businessobject.AssetPayment&docFormKey=88888888&returnLocation=portal.do&hideReturnLink=true&capitalAssetNumber=${capitalAssetNumber}">Click here</html:link>
				</c:when>
				<c:otherwise>
				  <html:link target="_blank" styleClass="portal_link" title="Payment Information" href="lookup.do?methodToCall=search&businessObjectClassName=org.kuali.ole.module.cam.businessobject.AssetPayment&docFormKey=88888888&returnLocation=portal.do&hideReturnLink=true&capitalAssetNumber=${capitalAssetNumber}">Click here</html:link>				
				</c:otherwise>
			  </c:choose>
