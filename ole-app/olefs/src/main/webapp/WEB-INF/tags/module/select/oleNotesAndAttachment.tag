...
<c:if test="${empty intialOpen}">
    <c:set var="intialOpen" value="${!empty documentNotes or (not empty defaultOpen and defaultOpen)}" />
</c:if>
<c:if test="${!empty intialOpen}">
    <c:set var="intialOpen" value="${!empty documentNotes or (not empty defaultOpen and defaultOpen) and intialOpen}" />
</c:if>
...
<kul:tab tabTitle="${tabTitle}" defaultOpen="${intialOpen}"  tabErrorKey="${Constants.DOCUMENT_NOTES_ERRORS},attachmentFile" tabItemCount="${fn:length(documentNotes)}" transparentBackground="${transparentBackground}" >
