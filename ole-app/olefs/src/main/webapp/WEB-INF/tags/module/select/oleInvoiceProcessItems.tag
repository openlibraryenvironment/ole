...
<c:set var="colspanBlank" value="6"/>
...
<kul:tab tabTitle="Process Items" defaultOpen="true" tabErrorKey="${PurapConstants.ITEM_TAB_ERRORS}">
	...
	<table cellpadding="0" cellspacing="0" class="datatable" summary="Items Section">
	    ...
	    <c:if test="${empty isCreditMemo or !isCreditMemo}" >
    		...
    		<purap:purPOLineItemTotals documentAttributes="${documentAttributes}" mainColumnCount="${mainColumnCount+10}" />
    		
	    	<select:oleInvoiceItems
		...
		<c:if test="${KualiForm.document.vendorDetail.vendorHeader.vendorForeignIndicator}">
			...
			<c:set var="colSpanItemType" value="${colSpanItemType-2 }"/>
			<c:set var="colSpanTotalLabel" value="${colSpanTotalLabel+2}"/>
		</c:if>
		<c:if test="${not KualiForm.document.vendorDetail.vendorHeader.vendorForeignIndicator}">
			...
			<c:set var="colSpanTotalLabel" value="${colSpanTotalLabel}"/>
		</c:if>
		<tr>
			...
			<td valign=middle class="datacell" colspan="${colSpanTotalAmount+1}">
			    <div align="right"><b>
			...
			<td class="datacell" colspan="${colSpanTotalBlank+colspanBlank}">&nbsp;</td>
		</tr>
		...
		<select:olemiscitems 
			...
			mainColumnCount="${mainColumnCount+4}"
			colSpanItemType="${colSpanItemType+1}" 
			colSpanDescription="${colSpanDescription+1}" 
			colSpanExtendedPrice="${colSpanExtendedPrice+1}"
			colSpanAmountPaid="0">
			<jsp:attribute name="specialItemTotalOverride">
				<tr>
		    		    ...
		    		    <div align="right"><b>
		    		    	<c:choose>
		    		    		<c:when test="${isCreditMemo && !(KualiForm.document.creditMemoType eq 'Vendor')}" >
		                            <kul:htmlControlAttribute
		                                attributeEntry="${documentAttributes.grandTotalExcludingDiscount}"
					...
					<td class="datacell" colspan="${colSpanTotalBlank+colspanBlank}">
						&nbsp;
					</td>
				</tr>
			</jsp:attribute>
		...
		<tr>
			...
			<td valign=middle class="datacell" colspan="${colSpanTotalAmount+1}">
			    <div align="right"><b>
                    <kul:htmlControlAttribute
                        attributeEntry="${DataDictionary.InvoiceDocument.grandPreTaxTotal}"
                        property="document.grandPreTaxTotal"
                        readOnly="true" />&nbsp; </b>
                </div>
			</td>
			<td valign="middle" class="datacell" colspan="${colSpanTotalBlank+colspanBlank}">
				&nbsp;
			</td>
		</tr>
		...
		<tr>
			...
			<td valign=middle class="datacell" colspan="${colSpanTotalAmount+1}">
			    <div align="right"><b>
                    <kul:htmlControlAttribute
                        attributeEntry="${DataDictionary.InvoiceDocument.grandTaxAmount}"
		...
		<tr>
			...
			<td class="datacell" colspan="${colSpanTotalBlank+colspanBlank}">
              ...
              <c:if test="${isCreditMemo}" >
	              <c:if test="${empty KualiForm.document.invoiceDocument.recurringPaymentTypeCode and !empty KualiForm.editingMode['allowReopenPurchaseOrder']}">
	                <kul:htmlControlAttribute
	              ...
	              <c:if test="${not empty KualiForm.document.invoiceDocument.recurringPaymentTypeCode and not fullDocumentEntryCompleted}">
	              	Recurring PO
	              </c:if>			
              </c:if>		
			</td>
