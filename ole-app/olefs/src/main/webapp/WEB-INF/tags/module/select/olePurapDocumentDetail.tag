...
function checkImmediatePaymentIndicator(proformaIndicator){
	var div = document.getElementsByTagName('div');
	if(proformaIndicator.checked){
		document.getElementById('document.immediatePaymentIndicator').checked=true;
	}
	else{ 
		document.getElementById('document.immediatePaymentIndicator').checked=false;
    }		
...
<table cellpadding="0" cellspacing="0" class="datatable" summary="Detail Section">
	    ...
	    <tr>
	        ...
	        <td align=left valign=middle class="datacell">
	            ...
	            <c:if test="${(fullEntryMode or amendmentEntry) and not (contentReadOnly or internalPurchasingReadOnly)}" >
	                <kul:lookup boClassName="org.kuali.ole.coa.businessobject.Organization" fieldConversions="organizationCode:document.organizationCode,chartOfAccountsCode:document.chartOfAccountsCode" />
	            </c:if>
	        </td>
	        <th align=right valign=middle class="bord-l-b">
	        ...
	        <td align=left valign=middle class="datacell">
	            <kul:htmlControlAttribute
	                ...
	                readOnly="${(fullEntryMode and editableFundingSource)}"
	                tabindexOverride="${tabindexOverrideBase + 5}"/> 
	                <!-- ${not (fullEntryMode and editableFundingSource)} -->
		...
		<tr>
            ...
            <td align=left valign=middle class="datacell">
                ...
                <c:if test="${preRouteChangeMode}" >
                    <kul:lookup
                        boClassName="org.kuali.ole.vnd.businessobject.ContractManager"
            ...
            <!-- OLE-4140 - Contract manager field is commented above, for alignment purpose empty tag is given below-->
            <th align=right valign=middle class="bord-l-b">
            </th>
            <td align=left valign=middle class="datacell">
            </td>
    ...
    <c:if test="${purapTaxEnabled or purchaseOrder}">
      ...
      <%-- OLE-3405 : *never* display this row 
	  <tr>
	      <th align=right valign=middle class="bord-l-b">
...
<tr>
             <c:if test="${purchaseOrder}">
		        ...
		        <%--<th align=right valign=middle class="bord-l-b">
		            <div align="right"><kul:htmlAttributeLabel attributeEntry="${documentAttributes.licensingRequirementIndicator}" /></div>
		        </th>
		        <td align=left valign=middle class="datacell">
		            <kul:htmlControlAttribute
		                property="document.licensingRequirementIndicator"
		                attributeEntry="${documentAttributes.licensingRequirementIndicator}"
		                readOnly="true"
		                tabindexOverride="${tabindexOverrideBase + 5}"/>
		      ...
		      <c:if test="${not purchaseOrder}">
		       ...
		       <%-- <c:if test="${!(purchaseOrder or paymentRequest)}">
		        <th align=right valign=middle class="bord-l-b">
		            <div align="right"><kul:htmlAttributeLabel attributeEntry="${documentAttributes.licensingRequirementIndicator}" /></div>
		        </th>
		        <td align=left valign=middle class="datacell">
		            <kul:htmlControlAttribute
		                property="document.licensingRequirementIndicator"
		                attributeEntry="${documentAttributes.licensingRequirementIndicator}"
		                readOnly="${enableLicenseIndicator}"
		                tabindexOverride="${tabindexOverrideBase + 5}"/>
...
<tr>
               <th align=right valign=middle class="bord-l-b">
               		<c:if test="${paymentRequest}">
		            	<div align="right"><kul:htmlAttributeLabel attributeEntry="${documentAttributes.proformaIndicator}" /></div>
		        	</c:if>
		        </th>
		        <td align=left valign=middle class="datacell">
		        	<c:if test="${not paymentRequest}">
		        	...
		        	<c:if test="${paymentRequest}">
		            	<kul:htmlControlAttribute
		                property="document.proformaIndicator"
		                attributeEntry="${documentAttributes.proformaIndicator}"
		                readOnly="${not (fullEntryMode)}" 
		                tabindexOverride="${tabindexOverrideBase + 5}"
		                onclick = "checkImmediatePaymentIndicator(this)"/>
		        	</c:if>		        
		       </td>
		       <%--<c:if test="${!(paymentRequest)}">
		      <th align=right valign=middle class="bord-l-b">
		            <div align="right"><kul:htmlAttributeLabel attributeEntry="${documentAttributes.licensingRequirementCode}" /></div>
		        </th>
		       
		        <td align=left valign=middle class="datacell">
		            <kul:htmlControlAttribute
		                property="document.licensingRequirementCode"
		                attributeEntry="${documentAttributes.licensingRequirementCode}"
		                readOnly="true"
		                tabindexOverride="${tabindexOverrideBase + 15}"
		                />
		        </td>
