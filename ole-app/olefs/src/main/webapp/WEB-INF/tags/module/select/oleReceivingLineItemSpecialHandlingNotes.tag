	...
	<c:if test="${isOpen != 'true' && isOpen != 'TRUE'}">
	    <tr style="display: none;"  id="tab-${tabKey}-div">
          ...
          <table>    
			 ...
			 <logic:present name="KualiForm" property="${accountPrefix}specialHandlingNoteList">
		     <logic:iterate indexId="ctr" name="KualiForm" property="${accountPrefix}specialHandlingNoteList" id="itemNotes">
				<c:if test="true">				
				<c:choose>	
				...
				<tr>
					...
					<td class="infoline">
					    <kul:htmlControlAttribute 
                        	attributeEntry="${itemAttributes.notes}" property="${accountPrefix}specialHandlingNoteList[${ctr}].notes" 
                        	readOnly="true" tabindexOverride="${tabindexOverrideBase + 0}"/>
					</td>
				<c:if test="${fullEntryMode}"> 
				<td align=left valign=middle class="datacell">
          				  <kul:htmlControlAttribute
               			 property="${accountPrefix}specialHandlingNoteList[${ctr}].notesAck"
