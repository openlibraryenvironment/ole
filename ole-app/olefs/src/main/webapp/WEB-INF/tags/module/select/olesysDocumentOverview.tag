...
<c:set var="includeTotalAmount" value="${not empty editingMode[OLEConstants.AMOUNT_TOTALING_EDITING_MODE]}" />
<select:olekrDocumentOverview editingMode="${editingMode}" intialOpen="${KualiForm.document.overviewFlag}">
	  ...
	  <table cellpadding="0" cellspacing="0" class="datatable" summary="KFS Detail Section">
	    ...
	    <c:if test="${includePostingYear and includeBankCode}">
	      <tr>
	   	...
	   	<c:if test="${!empty KualiForm.documentActions[OLEConstants.YEAR_END_ACCOUNTING_PERIOD_VIEW_DOCUMENT_ACTION]}">	   
			  	...
			  	<kul:htmlAttributeHeaderCell
		       	...
		       	<td class="datacell-nowrap">
		          	<kul:htmlControlAttribute 
		                ...
		                readOnly="${readOnly || empty KualiForm.documentActions[OLEConstants.YEAR_END_ACCOUNTING_PERIOD_EDIT_DOCUMENT_ACTION]}"
		                readOnlyBody="true" >
