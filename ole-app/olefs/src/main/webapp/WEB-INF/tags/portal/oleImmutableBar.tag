...
<div class="header2">
  <div class="header2-left-focus">
    <div class="breadcrumb-focus"><a href="asdf.html"> 
    	<portal:olePortalLink displayTitle="false" title='Action List' url='${ConfigProperties.workflow.url}/ActionList.do'>
   		<img src="images-portal/icon-port-actionlist.gif" alt="action list" width="91" height="19" border="0"></portal:olePortalLink>
    	<portal:olePortalLink displayTitle="false" title='Document Search' url='${ConfigProperties.workflow.documentsearch.base.url}'>
    	<img src="images-portal/icon-port-docsearch.gif" alt="doc search" width="96" height="19" border="0"></portal:olePortalLink>
            <portal:olePortalLink displayTitle="false" title='Search Workbench' url="${ConfigProperties.application.url}/ole-kr-krad/olesearchcontroller?viewId=OLESearchView&methodToCall=start">
            <img src="images-portal/icon-port-searchWorkbench.png" alt="Search Workbench" width="130" height="19" border="0"></portal:olePortalLink>
     </div>
  </div>
</div>
...
<div id="search">
  <c:choose> 
    ...
    <c:when test="${fn:trim(ConfigProperties.environment) == fn:trim(ConfigProperties.production.environment.code)}" >
	      <html:form action="/olelogout.do" method="post" style="margin:0; display:inline">
    	    <input name="imageField" type="submit" value="Logout" class="go" title="Click to logout.">
      	  </html:form>
    </c:when>
    <c:otherwise> 
       <c:if test="${backdoorEnabled}">
        <html:form action="/olebackdoorlogin.do" method="post" style="margin:0; display:inline">
