...
<c:choose>
  ...
  <c:otherwise>
	...
	<c:if test="${empty channelTitle && empty channelUrl}">
		<c:set var="motd" value="<%= (new org.kuali.ole.sys.businessobject.defaultvalue.MessageOfTheDayFinder()).getValue() %>" scope="page"/>
 	...
 	<tr valign="top" bgcolor="#FFFFFF">
       <td width="15" class="leftback-focus">&nbsp;</td>
        <c:choose>
          ...
          <c:when test='${selectedTab == "oleDeliver"}'>
              <portal:oleDeliverTab environment="${environment}" riceUrl="${riceUrl }"/>
          </c:when>
          <c:when test='${selectedTab == "oleDescribe"}'>
              <portal:oleDescribeTab />
          </c:when>
          <c:when test='${selectedTab == "oleSelectAcquire"}'>
              <portal:oleSelectAcquireTab riceUrl="${riceUrl }" />
          </c:when>
            <c:when test='${selectedTab == "oleMaintenance"}'>
                <portal:oleMaintenance riceUrl="${riceUrl }" />
            </c:when>
