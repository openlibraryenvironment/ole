...
<c:if test="${KualiForm.editingMode[OLEConstants.BANK_ENTRY_VIEWABLE_EDITING_MODE]}">

  <c:if test="${empty KualiForm.documentActions[OLEConstants.KFS_ACTION_CAN_EDIT_BANK]}">
  ...
  <td class="${style}">
    ...
    <c:if test="${not readOnly}">
      &nbsp;
      <kul:lookup boClassName="org.kuali.ole.sys.businessobject.Bank" fieldConversions="bankCode:${property},bankName:${objectProperty}.bankName" lookupParameters="${lookupParameters}" readOnlyFields="${readOnlyFields}" autoSearch="true"/>
    </c:if>
