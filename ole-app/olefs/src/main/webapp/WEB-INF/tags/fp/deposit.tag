...
<%@ attribute name="deposit" required="true" type="org.kuali.ole.fp.businessobject.Deposit"%>
...
<c:if test="${depositType != OLEConstants.DepositConstants.DEPOSIT_TYPE_FINAL}">
    <c:set var="depositTitle" value="${depositTitle} ${depositIndex + 1}" />
</c:if>

<c:choose>
<c:when test="${depositType == OLEConstants.DepositConstants.DEPOSIT_TYPE_FINAL}">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="datatable">
    ...
    <c:if test="${depositType == OLEConstants.DepositConstants.DEPOSIT_TYPE_FINAL && !empty deposit.depositedCurrency && !empty deposit.depositedCoin}">
      <tr>
    ...
    <%-- deposit footer --%>
    <c:if test="${(depositType == OLEConstants.DepositConstants.DEPOSIT_TYPE_FINAL && allowCancelDeposits) || allowAdditionalDeposits}">
        <tr>
