...
<%@ attribute name="boPackageName" required="false"
              ...
              If this attribute is missing, it defaults to 'org.kuali.ole.coa.businessobject'." %>
...
<c:choose>
    ...
    <c:when test="${empty boPackageName}">
        <c:set var="boClassName" value="org.kuali.ole.coa.businessobject.${boClassSimpleName}"/>
    </c:when>
    <c:otherwise>
