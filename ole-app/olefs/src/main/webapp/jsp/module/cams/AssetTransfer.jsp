...
--%>
<%@ page import="org.kuali.ole.sys.context.SpringContext" %>
<%@ page import="org.kuali.ole.coa.service.AccountService" %>
...
<kul:documentPage showDocumentInfo="true" htmlFormAction="camsAssetTransfer" documentTypeName="AssetTransferDocument" renderMultipart="true" showTabButtons="true">
	...
	<sys:documentOverview editingMode="${KualiForm.editingMode}" viewYearEndAccountPeriod="${!empty KualiForm.documentActions[OLEConstants.YEAR_END_ACCOUNTING_PERIOD_VIEW_DOCUMENT_ACTION]}" />
			...
			<tr>
				...
				<td class="grid" width="25%">
					...
					<c:if test="${not readOnlyChartCode}">				
						...
						&nbsp;
		                <kul:lookup boClassName="org.kuali.ole.coa.businessobject.Chart" fieldConversions="chartOfAccountsCode:document.organizationOwnerChartOfAccountsCode" lookupParameters="document.organizationOwnerChartOfAccountsCode:chartOfAccountsCode" />
	                </c:if>
				</td>						
			</tr>
			<tr>
		...
		<table width="100%" cellpadding="0" cellspacing="0" class="datatable">								
			...
			<tr>
				...
				<td class="grid" width="25%"><kul:htmlControlAttribute property="document.buildingCode" attributeEntry="${assetTransferAttributes.buildingCode}" readOnly="${readOnly}"/>
					<c:if test="${not readOnly}">
						&nbsp;
		                <kul:lookup boClassName="org.kuali.ole.sys.businessobject.Building" fieldConversions="buildingCode:document.buildingCode,campusCode:document.campusCode" lookupParameters="document.buildingCode:buildingCode,document.campusCode:campusCode" />
	                </c:if>
				</td>
			...
			<tr>
				...
				<td class="grid" width="25%"><kul:htmlControlAttribute property="document.buildingRoomNumber" attributeEntry="${assetTransferAttributes.buildingRoomNumber}" readOnly="${readOnly}"/>
					<c:if test="${not readOnly}">
						&nbsp;
		                <kul:lookup boClassName="org.kuali.ole.sys.businessobject.Room" fieldConversions="buildingRoomNumber:document.buildingRoomNumber,buildingCode:document.buildingCode,campusCode:document.campusCode" lookupParameters="document.buildingRoomNumber:buildingRoomNumber,document.buildingCode:buildingCode,document.campusCode:campusCode" />
	                </c:if>
				</td>
