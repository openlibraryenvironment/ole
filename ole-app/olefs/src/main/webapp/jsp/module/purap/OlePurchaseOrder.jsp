...
<kul:documentPage showDocumentInfo="true"
    documentTypeName="OLE_PO"
    htmlFormAction="purapOlePurchaseOrder" renderMultipart="true"
    showTabButtons="true">
    ...
    <c:if test="${!empty KualiForm.editingMode['editableFundingSource']}">
        <c:set var="editableFundingSourceVal" value="true" scope="request" />
    </c:if>
    ...
    <c:otherwise>
		<c:if test="${empty KualiForm.editingMode['amendmentEntry']}">
			<sys:documentOverview editingMode="${KualiForm.editingMode}"
		        ...
		        <select:olePurapDocumentDetail
		        	documentAttributes="${DataDictionary.PurchaseOrderDocument.attributes}"
		            purchaseOrder="true"
		            detailSectionLabel="Purchase Order Detail"
		            editableFundingSource="${editableFundingSourceVal}" 
		...
		<c:if test="${not retransmitMode}" >
		    ...
		    <c:if test="${!lockB2BEntry}">
		      ...
		      <select:oleitems itemAttributes="${DataDictionary.PurchaseOrderItem.attributes}"
