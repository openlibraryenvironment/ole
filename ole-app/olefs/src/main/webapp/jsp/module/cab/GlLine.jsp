...
<kul:page showDocumentInfo="false" htmlFormAction="cabGlLine" renderMultipart="true"
	...
	<kul:tabTop tabTitle="Financial Document Capital Asset Info" defaultOpen="true">
		...
		<c:set var="CapitalAssetInformationDetailAttributes"	value="${DataDictionary.CapitalAssetInformationDetail.attributes}" />
	    	...
	    	<c:if test="${KualiForm.capitalAssetInformation.capitalAssetActionIndicator == OLEConstants.CapitalAssets.CAPITAL_ASSET_CREATE_ACTION_INDICATOR}" >
	    		<h3><c:out value="${OLEConstants.CapitalAssets.CREATE_CAPITAL_ASSETS_TAB_TITLE}"/></h3>
	    	</c:if>
	    	<c:if test="${KualiForm.capitalAssetInformation.capitalAssetActionIndicator == OLEConstants.CapitalAssets.CAPITAL_ASSET_MODIFY_ACTION_INDICATOR}" >
	    		<h3><c:out value="${OLEConstants.CapitalAssets.MODIFY_CAPITAL_ASSETS_TAB_TITLE}"/></h3>
	    	</c:if>
			<table width="100%" cellpadding="0" cellspacing="0" class="datatable">			
				...
				<tr>
					...
					<td class="grid" width="25%">
						<c:if test="${!empty KualiForm.capitalAssetInformation.capitalAssetNumber}">
							<kul:inquiry boClassName="org.kuali.ole.integration.cam.CapitalAssetManagementAsset" keyValues="capitalAssetNumber=${KualiForm.capitalAssetInformation.capitalAssetNumber}" render="true">
				...
				<tr>
					...
					<c:if test="${distributionAmountCode eq OLEConstants.CapitalAssets.DISTRIBUTE_COST_EQUALLY_CODE}">
						<c:set var="distributionAmountDescription" value="${OLEConstants.CapitalAssets.DISTRIBUTE_COST_EQUALLY_DESCRIPTION}" />
					</c:if>
					<c:if test="${distributionAmountCode eq OLEConstants.CapitalAssets.DISTRIBUTE_COST_BY_INDIVIDUAL_ASSET_AMOUNT_CODE}">
						<c:set var="distributionAmountDescription" value="${OLEConstants.CapitalAssets.DISTRIBUTE_COST_BY_INDIVIDUAL_ASSET_AMOUNT_DESCRIPTION}" />
					</c:if>
