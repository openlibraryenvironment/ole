...
import static org.kuali.ole.sys.OLEKeyConstants.AccountingLineParser.ERROR_INVALID_FILE_FORMAT;
...
import static org.kuali.ole.sys.OLEPropertyConstants.OVERRIDE_CODE;
import static org.kuali.ole.sys.OLEPropertyConstants.POSTING_YEAR;
import static org.kuali.ole.sys.OLEPropertyConstants.PROJECT_CODE;
import static org.kuali.ole.sys.OLEPropertyConstants.SEQUENCE_NUMBER;
...
public class AccountingLineParserBase implements AccountingLineParser {
            ...
            for (Entry<String, String> entry : attributeValueMap.entrySet()) {
                try {
                    try {
                        ...
                        throw new IllegalArgumentException("unable to complete accounting line population.", e);
                    }
                }
                    ...
                    // KULLAB-408
                    GlobalVariables.getMessageMap().putError(OLEConstants.ACCOUNTING_LINE_ERRORS, ERROR_INVALID_PROPERTY_VALUE, entry.getValue().toString(), entry.getKey(), accountingLineAsString + "  : Line Number " + lineNo.toString());
    ...
    protected void putCommonAttributesInMap(Map<String, String> attributeValueMap, AccountingDocument document, Integer sequenceNumber) {
        attributeValueMap.put(OLEPropertyConstants.DOCUMENT_NUMBER, document.getDocumentNumber());
    ...
    protected List<AccountingLine> importAccountingLines(String fileName, InputStream stream, AccountingDocument transactionalDocument, boolean isSource) {
        ...
        try {
            ...
            while ((accountingLineAsString = br.readLine()) != null) {
                ...
                if (StringUtils.isBlank(StringUtils.remove(StringUtils.deleteWhitespace(accountingLineAsString),OLEConstants.COMMA))) {
                    continue;
                }
                ...
                try {
                    ...
                    GlobalVariables.getMessageMap().putError((isSource ? "sourceAccountingLines" : "targetAccountingLines"), OLEKeyConstants.ERROR_ACCOUNTING_DOCUMENT_ACCOUNTING_LINE_IMPORT_GENERAL, new String[] { e.getMessage() });
                }
            }
        }
        catch (IOException e) {
            throw new IllegalArgumentException("unable to readLine from bufferReader in accountingLineParserBase", e);
        }
        finally {
            try {
                br.close();
