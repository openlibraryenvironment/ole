...
public class DisbursementVoucherPayeeDetail extends PersistableBusinessObjectBase {
    ...
    public String getDisbVchrPaymentReasonName() {
        this.refreshReferenceObject(OLEPropertyConstants.DISB_VCHR_PAYMENT_REASON);
    ...
    public StateEbo getDisbVchrPayeeState() {
        if ( StringUtils.isBlank(disbVchrPayeeStateCode) || StringUtils.isBlank(OLEConstants.COUNTRY_CODE_UNITED_STATES ) ) {
            disbVchrPayeeState = null;
        } else {
            if ( disbVchrPayeeState == null || !StringUtils.equals( disbVchrPayeeState.getCode(),disbVchrPayeeStateCode) || !StringUtils.equals(disbVchrPayeeState.getCountryCode(), OLEConstants.COUNTRY_CODE_UNITED_STATES ) ) {
                ModuleService moduleService = SpringContext.getBean(KualiModuleService.class).getResponsibleModuleService(StateEbo.class);
                if ( moduleService != null ) {
                    Map<String,Object> keys = new HashMap<String, Object>(2);
                    keys.put(LocationConstants.PrimaryKeyConstants.COUNTRY_CODE, OLEConstants.COUNTRY_CODE_UNITED_STATES);/*RICE20_REFACTORME*/
    ...
    public PostalCodeEbo getDisbVchrPayeePostalZipCode() {
        if ( StringUtils.isBlank(disbVchrPayeeZipCode) || StringUtils.isBlank(OLEConstants.COUNTRY_CODE_UNITED_STATES ) ) {
            disbVchrPayeePostalZipCode = null;
        } else {
            if ( disbVchrPayeePostalZipCode == null || !StringUtils.equals( disbVchrPayeePostalZipCode.getCode(), disbVchrPayeeZipCode) || !StringUtils.equals(disbVchrPayeePostalZipCode.getCountryCode(), OLEConstants.COUNTRY_CODE_UNITED_STATES ) ) {
