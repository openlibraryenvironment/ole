...
import org.kuali.ole.sys.service.ElectronicFundTransferActionHelper;
...
import org.kuali.ole.sys.web.struts.ElectronicFundTransferForm;
import org.kuali.ole.sys.web.struts.ElectronicPaymentClaimClaimedHelper;
...
public class ElectronicFundTransferClaimActionHelper implements ElectronicFundTransferActionHelper {
    ...
    protected boolean checkChosenDocumentType(String chosenDoc) {
        ...
        if (StringUtils.isBlank(chosenDoc)) {
            GlobalVariables.getMessageMap().putError(ElectronicFundTransferClaimActionHelper.CHOSEN_DOCUMENT_PROPERTY, OLEKeyConstants.ElectronicPaymentClaim.ERROR_EFT_NO_CHOSEN_CLAIMING_DOCTYPE, new String[]{});
            result = false;
        }
        return result;
    }
        ...
        for (ElectronicPaymentClaim claim: claims) {
            if (StringUtils.isBlank(claim.getReferenceFinancialDocumentNumber()) && !preClaimedByCheckbox.contains(claim.getElectronicPaymentClaimRepresentation())) {
                ...
                if (StringUtils.isNotBlank(claim.getReferenceFinancialDocumentNumber())) {
                    ...
                    if (!isValidDocRef) {
                        GlobalVariables.getMessageMap().putError(ElectronicFundTransferClaimActionHelper.CLAIM_PROPERTY+"["+count+"]", OLEKeyConstants.ElectronicPaymentClaim.ERROR_PRE_CLAIMING_DOCUMENT_DOES_NOT_EXIST, new String[] { claim.getReferenceFinancialDocumentNumber() });
    ...
    protected boolean handleDocumentationForClaim(String hasDocumentation) {
        ...
        if (StringUtils.isBlank(hasDocumentation) || !hasDocumentation.equalsIgnoreCase("yep")) {
            GlobalVariables.getMessageMap().putError(ElectronicFundTransferClaimActionHelper.HAS_DOCUMENTATION_PROPERTY, OLEKeyConstants.ElectronicPaymentClaim.ERROR_NO_DOCUMENTATION, new String[] {});
            success = false;
        }
