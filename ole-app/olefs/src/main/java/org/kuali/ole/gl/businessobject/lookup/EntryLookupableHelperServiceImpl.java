...
import org.kuali.ole.gl.service.EntryService;
...
public class EntryLookupableHelperServiceImpl extends AbstractGeneralLedgerLookupableHelperServiceImpl {
    ...
    public void validateSearchParameters(Map fieldValues) {
        ...
        if (!StringUtils.isEmpty(valueFiscalYear)) {
            try {
                ...
                GlobalVariables.getMessageMap().putError("universityFiscalYear", OLEKeyConstants.ERROR_CUSTOM, new String[] { "Fiscal Year must be a four-digit number" });
                throw new ValidationException("errors in search criteria");
            }
        }

        if (!allRequiredsForAccountSearch(fieldValues) && !allRequiredsForDocumentSearch(fieldValues)) {
            GlobalVariables.getMessageMap().putError("universityFiscalYear", OLEKeyConstants.ERROR_GL_LOOKUP_ENTRY_NON_MATCHING_REQUIRED_FIELDS, new String[] {});
            throw new ValidationException("errors in search criteria");
        }
    }
    ...
    public HtmlData getInquiryUrl(BusinessObject businessObject, String propertyName) {
        if (OLEPropertyConstants.DOCUMENT_NUMBER.equals(propertyName)) {
