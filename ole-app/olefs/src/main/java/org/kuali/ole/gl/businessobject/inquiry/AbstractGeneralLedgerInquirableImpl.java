...
public abstract class AbstractGeneralLedgerInquirableImpl extends KfsInquirableImpl {
    ...
    public HtmlData getInquiryUrl(BusinessObject businessObject, String attributeName) {
        ...
        String baseUrl = OLEConstants.INQUIRY_ACTION;
        ...
        parameters.put(OLEConstants.BUSINESS_OBJECT_CLASS_ATTRIBUTE, inquiryBusinessObjectClass.getName());
        ...
        if (isUserDefinedAttribute) {
            ...
            parameters.put(OLEConstants.RETURN_LOCATION_PARAMETER, Constant.RETURN_LOCATION_VALUE);
            parameters.put(OLEConstants.GL_BALANCE_INQUIRY_FLAG, "true");
        ...
        if (businessObject instanceof AccountBalance) {
            ...
            if ("financialObject.financialObjectLevel.financialConsolidationObject.finConsolidationObjectCode".equals(attributeName)) {
                parameters.put(OLEPropertyConstants.CHART_OF_ACCOUNTS_CODE, ab.getChartOfAccountsCode());
            }
    ...
    protected boolean isExclusiveField(Object keyName, Object keyValue) {
        ...
        if (keyName != null && keyValue != null) {
            ...
            if (convertedKeyName.equals(OLEPropertyConstants.SUB_ACCOUNT_NUMBER) && keyValue.equals(Constant.CONSOLIDATED_SUB_ACCOUNT_NUMBER)) {
                return true;
            }
            else if (convertedKeyName.equals(OLEPropertyConstants.SUB_OBJECT_CODE) && keyValue.equals(Constant.CONSOLIDATED_SUB_OBJECT_CODE)) {
                return true;
            }
            else if (convertedKeyName.equals(OLEPropertyConstants.OBJECT_TYPE_CODE) && keyValue.equals(Constant.CONSOLIDATED_OBJECT_TYPE_CODE)) {
                return true;
            }
            if (convertedKeyName.equals(OLEPropertyConstants.SUB_ACCOUNT_NUMBER) && keyValue.equals(OLEConstants.getDashSubAccountNumber())) {
                return true;
            }
            else if (convertedKeyName.equals(OLEPropertyConstants.SUB_OBJECT_CODE) && keyValue.equals(OLEConstants.getDashFinancialSubObjectCode())) {
                return true;
            }
            else if (convertedKeyName.equals(OLEPropertyConstants.PROJECT_CODE) && keyValue.equals(OLEConstants.getDashProjectCode())) {
                return true;
            }
        }
        return false;
...
//        }
    ...
    protected HtmlData getDocTypeInquiryUrl(String docTypeCode) {
        ...
        Properties parameters = new Properties();
        parameters.put(OLEConstants.DISPATCH_REQUEST_PARAMETER, OLEConstants.START_METHOD);
        parameters.put(OLEConstants.BUSINESS_OBJECT_CLASS_ATTRIBUTE, DocumentTypeEBO.class.getName());
        parameters.put(OLEConstants.DOC_FORM_KEY, "88888888");
        parameters.put(OLEPropertyConstants.DOCUMENT_TYPE_ID, docTypeDTO.getId().toString());

        Map<String, String> inquiryFields = new HashMap<String, String>();
        inquiryFields.put(OLEPropertyConstants.DOCUMENT_TYPE_ID, docTypeDTO.getId().toString());
