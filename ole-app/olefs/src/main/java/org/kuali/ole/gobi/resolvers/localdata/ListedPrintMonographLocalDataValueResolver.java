 ...
 */
public class  ListedPrintMonographLocalDataValueResolver extends LocalDataMappingValueResolver {
    @Override
    ...
    private String getAttributeValue() {
        ...
        PurchaseOrder.Order.ListedPrintMonograph listedPrintMonograph = order.getListedPrintMonograph();
        if (null != listedPrintMonograph) {
            PurchaseOrder.Order.ListedPrintMonograph.OrderDetail orderDetail = listedPrintMonograph.getOrderDetail();
            ...
            if (getSourceFieldName().equalsIgnoreCase(GobiConstants.LOCAL_DATA1) ||
                ...
                orderDetail = listedPrintMonograph.getOrderDetail();
                List<PurchaseOrder.Order.ListedPrintMonograph.OrderDetail.LocalData> localData =
                        orderDetail.getLocalData();

                for (Iterator<PurchaseOrder.Order.ListedPrintMonograph.OrderDetail.LocalData> iterator = localData.iterator(); iterator.hasNext(); ) {
                    PurchaseOrder.Order.ListedPrintMonograph.OrderDetail.LocalData local = iterator.next();
