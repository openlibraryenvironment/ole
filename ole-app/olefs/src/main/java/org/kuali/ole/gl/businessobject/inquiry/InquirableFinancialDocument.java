...
public class InquirableFinancialDocument {
    ...
    private String getUrl(String originCode, String docNumber) {
        if (OLEConstants.ORIGIN_CODE_KUALI.equals(originCode) && !StringUtils.isBlank(docNumber)) {
            return kualiConfigurationService.getPropertyValueAsString(OLEConstants.WORKFLOW_URL_KEY) + "/DocHandler.do?docId=" + docNumber + "&command=displayDocSearchView";
        }
        return OLEConstants.EMPTY_STRING;
    }
