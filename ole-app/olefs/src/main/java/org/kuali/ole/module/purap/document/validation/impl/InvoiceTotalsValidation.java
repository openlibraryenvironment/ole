...
public class InvoiceTotalsValidation extends GenericValidation {
    ...
    public boolean validate(AttributedDocumentEvent event) {
        ...
        // total amount NOT INCLUDING tax
            ...
            if (document.getTotalPreTaxDollarAmountAllItems(excludeArray).compareTo(document.getVendorInvoiceAmount()) != 0
                    && !document.isUnmatchedOverride()) {
                if (!KNSGlobalVariables.getMessageList().contains(
                        PurapKeyConstants.WARNING_PAYMENT_REQUEST_VENDOR_INVOICE_AMOUNT_INVALID)) {
        ...
        // total amount INCLUDING sales tax (since if the vendor invoices with sales tax, then we pay it)
            ...
            if (document.getTotalDollarAmountAllItems(excludeArray).compareTo(document.getVendorInvoiceAmount()) != 0
                    && !document.isUnmatchedOverride()) {
                if (!KNSGlobalVariables.getMessageList().contains(
                        PurapKeyConstants.WARNING_PAYMENT_REQUEST_VENDOR_INVOICE_AMOUNT_INVALID)) {
                    KNSGlobalVariables.getMessageList().add(
    ...
    protected void flagLineItemTotals(List<PurApItem> itemList) {
        for (PurApItem purApItem : itemList) {
            InvoiceItem item = (InvoiceItem) purApItem;
