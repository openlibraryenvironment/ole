...
public class VendorLookupableHelperServiceImpl extends AbstractLookupableHelperServiceImpl {
    ...
    public List<HtmlData> getCustomActionUrls(BusinessObject businessObject, List pkNames) {
        ...
        boolean hasPermission = KimApiServiceLocator.getPermissionService().isAuthorized(
                GlobalVariables.getUserSession().getPerson().getPrincipalId(), nameSpaceCode,
                OLEConstants.Vendor.CREATE_VENDOR_DIVISION, Collections.<String, String> emptyMap());
        if (vendor.isVendorParentIndicator() && vendor.isActiveIndicator() && hasPermission ) {
            // only allow active parent vendors to create new divisions
            anchorHtmlDataList.add(super.getUrlData(businessObject, OLEConstants.MAINTENANCE_NEWWITHEXISTING_ACTION, VendorConstants.CREATE_DIVISION, pkNames));
        }
