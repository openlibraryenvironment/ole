...
public class SubObjCdGlobalRule extends GlobalDocumentRuleBase {
    ...
    public boolean processCustomAddCollectionLineBusinessRules(MaintenanceDocument document, String collectionName, PersistableBusinessObject bo) {
        ...
        if (bo instanceof AccountGlobalDetail) {
            ...
            if (!checkEmptyValue(detail.getAccountNumber())) {
                ...
                GlobalVariables.getMessageMap().putError("accountNumber", OLEKeyConstants.ERROR_REQUIRED, "Account Number");
                success &= false;
            }
            ...
            if (!checkEmptyValue(detail.getFinancialObjectCode())) {
                ...
                GlobalVariables.getMessageMap().putError("financialObjectCode", OLEKeyConstants.ERROR_REQUIRED, "Financial Object Code");
                success &= false;
            }
            if (!checkEmptyValue(detail.getUniversityFiscalYear())) {
                // put an error about financial object code
                GlobalVariables.getMessageMap().putError("universityFiscalYear", OLEKeyConstants.ERROR_REQUIRED, "University Fiscal Year");
                success &= false;
            }
    ...
    public boolean checkSubObjectCodeDetails(SubObjectCodeGlobalDetail dtl) {
        ...
        if (StringUtils.isNotBlank(dtl.getFinancialObjectCode()) && StringUtils.isNotBlank(dtl.getChartOfAccountsCode()) && dtl.getUniversityFiscalYear() > 0) {
            ...
            if (ObjectUtils.isNull(dtl.getChartOfAccounts()) || ObjectUtils.isNull(dtl.getUniversityFiscal()) || ObjectUtils.isNull(dtl.getFinancialObject())) {
                GlobalVariables.getMessageMap().putError("financialObjectCode", OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_INVALID_OBJECT_CODE, new String[] { dtl.getFinancialObjectCode(), dtl.getChartOfAccountsCode(), dtl.getUniversityFiscalYear().toString() });
            }
        }
        success &= GlobalVariables.getMessageMap().getErrorCount() == originalErrorCount;
    ...
    protected boolean checkForSubObjCdGlobalDetails(List<SubObjectCodeGlobalDetail> subObjCdGlobalDetails) {
        if (subObjCdGlobalDetails == null || subObjCdGlobalDetails.size() == 0) {
            putFieldError(OLEConstants.MAINTENANCE_ADD_PREFIX + OLEPropertyConstants.SUB_OBJ_CODE_CHANGE_DETAILS + "." + OLEPropertyConstants.FINANCIAL_DOCUMENT_TYPE_CODE, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_NO_OBJECT_CODE);
            return false;
        }
        return true;
    }
    ...
    protected boolean checkForAccountGlobalDetails(List<AccountGlobalDetail> acctChangeDetails) {
        if (acctChangeDetails == null || acctChangeDetails.size() == 0) {
            putFieldError(OLEConstants.MAINTENANCE_ADD_PREFIX + OLEPropertyConstants.ACCOUNT_CHANGE_DETAILS + "." + OLEPropertyConstants.FINANCIAL_DOCUMENT_TYPE_CODE, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_NO_ACCOUNT);
            return false;
        }
        return true;
    }
    ...
    protected boolean checkFiscalYear(SubObjectCodeGlobal socChangeDocument, SubObjectCodeGlobalDetail socChangeDetail, int lineNum, boolean add) {
        ...
        if (ObjectUtils.isNull(socChangeDetail.getUniversityFiscal())) {
            if (add) {
                errorPath = OLEConstants.MAINTENANCE_ADD_PREFIX + OLEPropertyConstants.SUB_OBJ_CODE_CHANGE_DETAILS + "." + "universityFiscalYear";
                putFieldError(errorPath, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_FISCAL_YEAR_MUST_EXIST);
            }
            else {
                errorPath = OLEPropertyConstants.SUB_OBJ_CODE_CHANGE_DETAILS + "[" + lineNum + "]." + "universityFiscalYear";
                putFieldError(errorPath, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_FISCAL_YEAR_MUST_EXIST);
            }
            success &= false;
            return success;
        ...
        if (!socChangeDocument.getUniversityFiscal().equals(socChangeDetail.getUniversityFiscal())) {
            if (add) {
                errorPath = OLEConstants.MAINTENANCE_ADD_PREFIX + OLEPropertyConstants.SUB_OBJ_CODE_CHANGE_DETAILS + "." + "universityFiscalYear";
                putFieldError(errorPath, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_FISCAL_YEAR_MUST_BE_SAME);
            }
            else {
                errorPath = OLEPropertyConstants.SUB_OBJ_CODE_CHANGE_DETAILS + "[" + lineNum + "]." + "universityFiscalYear";
                putFieldError(errorPath, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_FISCAL_YEAR_MUST_BE_SAME);
    ...
    protected boolean checkChartOnSubObjCodeDetails(SubObjectCodeGlobal socChangeDocument, SubObjectCodeGlobalDetail socChangeDetail, int lineNum, boolean add) {
        ...
        if (socChangeDetail.getChartOfAccounts() == null) {
            if (add) {
                errorPath = OLEConstants.MAINTENANCE_ADD_PREFIX + OLEPropertyConstants.SUB_OBJ_CODE_CHANGE_DETAILS + "." + "chartOfAccountsCode";
                putFieldError(errorPath, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_CHART_MUST_EXIST);
            }
            else {
                errorPath = OLEPropertyConstants.SUB_OBJ_CODE_CHANGE_DETAILS + "[" + lineNum + "]." + "chartOfAccountsCode";
                putFieldError(errorPath, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_CHART_MUST_EXIST);
            }
            success &= false;
            return success;
        ...
        if (!socChangeDocument.getChartOfAccounts().equals(socChangeDetail.getChartOfAccounts())) {
            if (add) {
                errorPath = OLEConstants.MAINTENANCE_ADD_PREFIX + OLEPropertyConstants.SUB_OBJ_CODE_CHANGE_DETAILS + "." + "chartOfAccountsCode";
                putFieldError(errorPath, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_CHART_MUST_BE_SAME);
            }
            else {
                errorPath = OLEPropertyConstants.SUB_OBJ_CODE_CHANGE_DETAILS + "[" + lineNum + "]." + "chartOfAccountsCode";
                putFieldError(errorPath, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_CHART_MUST_BE_SAME);
    ...
    protected boolean checkChartOnAccountDetails(SubObjectCodeGlobal socChangeDocument, AccountGlobalDetail acctDetail, int lineNum, boolean add) {
        ...
        if (!socChangeDocument.getChartOfAccounts().equals(acctDetail.getChartOfAccounts())) {
            if (add) {
                errorPath = OLEConstants.MAINTENANCE_ADD_PREFIX + OLEPropertyConstants.ACCOUNT_CHANGE_DETAILS + "." + "chartOfAccountsCode";
                putFieldError(errorPath, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_CHART_MUST_BE_SAME);
            }
            else {
                errorPath = OLEPropertyConstants.ACCOUNT_CHANGE_DETAILS + "[" + lineNum + "]." + "chartOfAccountsCode";
                putFieldError(errorPath, OLEKeyConstants.ERROR_DOCUMENT_GLOBAL_SUBOBJECTMAINT_CHART_MUST_BE_SAME);
            }
            success &= false;
            return success;
