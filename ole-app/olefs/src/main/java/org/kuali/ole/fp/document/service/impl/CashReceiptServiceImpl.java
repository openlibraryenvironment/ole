...
public class CashReceiptServiceImpl implements CashReceiptService {
     ...
     * 
     * @see org.kuali.ole.fp.document.service.CashReceiptService#getCashReceiptVerificationUnit(org.kuali.rice.krad.bo.user.KualiUser)
     */
    @Override
     ...
     * 
     * @see org.kuali.ole.fp.document.service.CashReceiptService#getCashReceipts(java.lang.String, java.lang.String)
     */
    @Override
    ...
    public List<CashReceiptDocument> getPopulatedCashReceipts(String verificationUnit, String[] statii) {
        ...
        List<CashReceiptDocument> documents = new ArrayList<CashReceiptDocument>(businessObjectService.findMatchingOrderBy(CashReceiptDocument.class, queryCriteria, OLEPropertyConstants.DOCUMENT_NUMBER, true));
    ...
    protected Map buildCashReceiptCriteriaMap(String campusCode, String[] statii) {
        ...
        if (statii.length == 1) {
            queryCriteria.put(OLEConstants.CashReceiptConstants.CASH_RECEIPT_DOC_HEADER_STATUS_CODE_PROPERTY_NAME, statii[0]);
        }
        else if (statii.length > 0) {
            List<String> statusList = Arrays.asList(statii);
            queryCriteria.put(OLEConstants.CashReceiptConstants.CASH_RECEIPT_DOC_HEADER_STATUS_CODE_PROPERTY_NAME, statusList);
        }

        queryCriteria.put(OLEConstants.CashReceiptConstants.CASH_RECEIPT_CAMPUS_LOCATION_CODE_PROPERTY_NAME, campusCode);
     ...
     * 
     * @see org.kuali.ole.fp.document.service.CashReceiptService#addCashDetailsToCashDrawer(org.kuali.ole.fp.document.CashReceiptDocument)
     */
    @Override
    public void addCashDetailsToCashDrawer(CashReceiptDocument crDoc) {
        ...
        if (crDoc.getCurrencyDetail() != null && !crDoc.getCurrencyDetail().isEmpty()) {
            CurrencyDetail cumulativeCurrencyDetail = cashManagementDao.findCurrencyDetailByCashieringStatus(drawer.getReferenceFinancialDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_RECEIPTS);
        ...
        if (crDoc.getCoinDetail() != null && !crDoc.getCoinDetail().isEmpty()) {
            CoinDetail cumulativeCoinDetail = cashManagementDao.findCoinDetailByCashieringStatus(drawer.getReferenceFinancialDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_RECEIPTS);
    ...
    /**
     * @see org.kuali.module.financial.service.CashReceiptTotalsVerificationService#areCashTotalsInvalid(org.kuali.ole.fp.document.CashReceiptDocument)
     */
    @Override
    public boolean areCashTotalsInvalid(CashReceiptDocument cashReceiptDocument) {
        ...
        boolean isInvalid = isTotalInvalid(cashReceiptDocument, cashReceiptDocument.getTotalCheckAmount(), documentEntryName, OLEPropertyConstants.TOTAL_CHECK_AMOUNT);
        isInvalid |= isTotalInvalid(cashReceiptDocument, cashReceiptDocument.getTotalCashAmount(), documentEntryName, OLEPropertyConstants.TOTAL_CASH_AMOUNT);
        isInvalid |= isTotalInvalid(cashReceiptDocument, cashReceiptDocument.getTotalCoinAmount(), documentEntryName, OLEPropertyConstants.TOTAL_COIN_AMOUNT);

        isInvalid |= isTotalInvalid(cashReceiptDocument, cashReceiptDocument.getTotalDollarAmount(), documentEntryName, OLEPropertyConstants.SUM_TOTAL_AMOUNT);
