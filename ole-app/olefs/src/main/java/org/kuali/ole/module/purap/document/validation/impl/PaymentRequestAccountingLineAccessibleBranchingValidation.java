...
public class PaymentRequestAccountingLineAccessibleBranchingValidation extends BranchingValidation {
    ...
    protected String determineBranch(AttributedDocumentEvent event) {
        ...
        if (StringUtils.equals(PaymentRequestStatuses.APPDOC_AWAITING_ACCOUNTS_PAYABLE_REVIEW, status)) {
            return null;
        } else if (StringUtils.equals(PaymentRequestStatuses.APPDOC_AWAITING_PAYMENT_REVIEW, status)) {
            return null;
        } else if (StringUtils.equals(PaymentRequestStatuses.APPDOC_AWAITING_FISCAL_REVIEW, status)) {
            return null;
        } else {
