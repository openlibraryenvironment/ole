...
import org.kuali.ole.sys.dataaccess.BusinessObjectMetaDataDao;
...
@NonTransactional
public class OleBusinessObjectMetaDataServiceImpl implements OleBusinessObjectMetaDataService {
    private Logger LOG = Logger.getLogger(OleBusinessObjectMetaDataServiceImpl.class);
    ...
    public DataMappingFieldDefinition getDataMappingFieldDefinition(String componentClass, String propertyName) {
        ...
        primaryKeys.put(OLEPropertyConstants.COMPONENT_CLASS, componentClass);
        primaryKeys.put(OLEPropertyConstants.PROPERTY_NAME, propertyName);
        FunctionalFieldDescription functionalFieldDescription = businessObjectService.findByPrimaryKey(FunctionalFieldDescription.class, primaryKeys);
    ...
    public DataMappingFieldDefinition getDataMappingFieldDefinition(FunctionalFieldDescription functionalFieldDescription) {
        BusinessObjectEntry businessObjectEntry = (BusinessObjectEntry) dataDictionaryService.getDataDictionary().getBusinessObjectEntry(functionalFieldDescription.getComponentClass());
        String propertyType = "";
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("OleBusinessObjectMetaDataServiceImpl unable to get type of property: " + functionalFieldDescription.getPropertyName(), e);
            }
        }
    ...
    public List<BusinessObjectComponent> findBusinessObjectComponents(String namespaceCode, String componentLabel) {
        ...
        if (StringUtils.isNotBlank(componentLabel)) {
            ...
            try {
                ...
                LOG.error("OleBusinessObjectMetaDataServiceImpl unable to parse componentLabel pattern, ignoring.", ex);
            }
        }
        for (org.kuali.rice.krad.datadictionary.BusinessObjectEntry businessObjectEntry : dataDictionaryService.getDataDictionary().getBusinessObjectEntries().values()) {
            if ((StringUtils.isBlank(namespaceCode) || namespaceCode.equals(KRADServiceLocatorWeb.getKualiModuleService().getNamespaceCode(businessObjectEntry.getBusinessObjectClass())))
                ...
                matchingBusinessObjectComponents.put(businessObjectEntry.getBusinessObjectClass(), new BusinessObjectComponent(KRADServiceLocatorWeb.getKualiModuleService().getNamespaceCode(businessObjectEntry.getBusinessObjectClass()), (BusinessObjectEntry) businessObjectEntry));
            }
        }
    ...
    public List<BusinessObjectProperty> findBusinessObjectProperties(String namespaceCode, String componentLabel, String propertyLabel) {
        ...
        if (StringUtils.isNotBlank(propertyLabel)) {
            ...
            try {
                ...
                LOG.error("OleBusinessObjectMetaDataServiceImpl unable to parse propertyLabel pattern, ignoring.", ex);
            }
        }
        ...
        for (BusinessObjectComponent businessObjectComponent : businessObjectComponents) {
            for (AttributeDefinition attributeDefinition : dataDictionaryService.getDataDictionary().getBusinessObjectEntry(businessObjectComponent.getComponentClass().toString()).getAttributes()) {
                if (!attributeDefinition.getName().endsWith(OLEPropertyConstants.VERSION_NUMBER) && !attributeDefinition.getName().endsWith(OLEPropertyConstants.OBJECT_ID) && ((propertyLabelRegex == null) || propertyLabelRegex.matcher(attributeDefinition.getLabel().toUpperCase()).matches())) {
    ...
    public List<FunctionalFieldDescription> findFunctionalFieldDescriptions(String namespaceCode, String componentLabel, String propertyLabel, String description, String active) {
        ...
        fieldValues.put(OLEPropertyConstants.NAMESPACE_CODE, namespaceCode);
        fieldValues.put(OLEPropertyConstants.COMPONENT_CLASS, buildOrCriteria(componentClasses));
        fieldValues.put(OLEPropertyConstants.PROPERTY_NAME, buildOrCriteria(propertyNames));
        fieldValues.put(OLEPropertyConstants.DESCRIPTION, description);
        fieldValues.put(OLEPropertyConstants.ACTIVE, active);
