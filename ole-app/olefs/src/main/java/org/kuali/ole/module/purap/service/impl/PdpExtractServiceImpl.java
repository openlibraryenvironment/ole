...
import org.kuali.ole.module.purap.batch.service.PurapRunDateService;
...
import org.kuali.ole.vnd.businessobject.AliasType;
...
public class PdpExtractServiceImpl implements PdpExtractService {
    ...
    /**
     * @see org.kuali.ole.module.purap.service.PdpExtractService#extractPayments(Date)
     */
    @Override
    ...
    protected void updateCreditMemo(VendorCreditMemoDocument creditMemoDocument, Person puser, Date processRunDate) {
        try {
            ...
            doc.setApplicationDocumentStatus(PurapConstants.CreditMemoStatuses.APPDOC_EXTRACTED);
    ...
    protected boolean validatePaymentGroup(PaymentGroup paymentGroup) {
        ...
        if (noteLines > maxNoteLines) {
            ...
            for (PaymentDetail payDetail : pds) {
                if (OLEConstants.FinancialDocumentTypeCodes.VENDOR_CREDIT_MEMO.equals(payDetail.getFinancialDocumentTypeCode())) {
    ...
    protected PaymentGroup populatePaymentGroup(PaymentRequestDocument paymentRequestDocument, Batch batch) {
        ...
        paymentGroup.setPaymentStatusCode(OLEConstants.PdpConstants.PAYMENT_OPEN_STATUS_CODE);
        paymentGroup.setBankCode(paymentRequestDocument.getBankCode());

        String postalCode = paymentRequestDocument.getVendorPostalCode();
        if (OLEConstants.COUNTRY_CODE_UNITED_STATES.equals(paymentRequestDocument.getVendorCountry())) {
        ...
        Integer vendorHeaderGeneratedIdentifier = paymentRequestDocument.getVendorHeaderGeneratedIdentifier();
        String aliasName = getVendorAliasName(vendorHeaderGeneratedIdentifier);
        if(aliasName != null){
            paymentGroup.setVendorAliasName(aliasName);
        }
...
//        }
    ...
    protected PaymentGroup populatePaymentGroup(VendorCreditMemoDocument creditMemoDocument, Batch batch) {
        ...
        if (OLEConstants.COUNTRY_CODE_UNITED_STATES.equals(creditMemoDocument.getVendorCountry())) {
            // Add a dash in the zip code if necessary
            if (postalCode.length() > 5) {
                postalCode = postalCode.substring(0, 5) + "-" + postalCode.substring(5);
            }
        ...
        paymentGroup.setPayeeIdTypeCd(PdpConstants.PayeeIdTypeCodes.VENDOR_ID);
        Integer vendorHeaderGeneratedIdentifier = creditMemoDocument.getVendorHeaderGeneratedIdentifier();
        String aliasName = getVendorAliasName(vendorHeaderGeneratedIdentifier);
        if(aliasName != null){
            paymentGroup.setVendorAliasName(aliasName);
        }
...
//        }
    ...
    protected class AccountingInfo {
        ...
        public void setProjectCode(String projectCode) {
            if (projectCode == null) {
                this.projectCode = OLEConstants.getDashProjectCode();
            } else {
        ...
        public void setSubAccount(String subAccount) {
            if (subAccount == null) {
                this.subAccount = OLEConstants.getDashSubAccountNumber();
            } else {
        ...
        public void setSubObjectCode(String subObjectCode) {
            if (subObjectCode == null) {
                this.subObjectCode = OLEConstants.getDashFinancialSubObjectCode();
            } else {
    ...
    public String getVendorAliasName(Integer vendorHeaderGeneratedIdentifier) {
        ...
        searchMap.put(OLEConstants.VENDOR_HDR_GEN_ID, vendorHeaderGeneratedIdentifier);
        List<VendorAlias> vendorAliasList = (List<VendorAlias>) SpringContext.getBean(BusinessObjectService.class).findMatching(VendorAlias.class, searchMap);
        Integer vendorAliasTypeId;
        if (vendorAliasList.size() > 0) {
            for (VendorAlias vendorAlias : vendorAliasList) {
                vendorAliasTypeId = vendorAlias.getVendorAliasTypeId();
                if (vendorAliasTypeId != null) {
                    String vendorAliasTypeName = getVendorAliasTypeName(vendorAliasTypeId);
                    if (vendorAliasTypeName != null && vendorAliasTypeName.equalsIgnoreCase(OLEConstants.EXTERNAL_VNDR_CD)) {
                        //return vendorAliasTypeName;
                        return vendorAlias.getVendorAliasName();
                    }
                }
            }
        }
    ...
    public String getVendorAliasTypeName(Integer vendorAliasTypeId) {
        Map searchMap = new HashMap();
        searchMap.put(OLEConstants.ALIAS_TYP_ID, vendorAliasTypeId);
        AliasType aliasType = SpringContext.getBean(BusinessObjectService.class).findByPrimaryKey(AliasType.class, searchMap);
        if (aliasType != null) {
            return aliasType.getAliasType();
        }
        return null;
    }
