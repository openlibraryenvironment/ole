...
import org.kuali.ole.sys.businessobject.TaxRegionType;
...
public class TaxRegionTypeLookupableServiceImpl extends KualiLookupableHelperServiceImpl {
    ...
    public HtmlData getReturnUrl(BusinessObject businessObject, LookupForm lookupForm, List returnKeys, BusinessObjectRestrictions restrictions) {
        ...
        if (docTypeName.equals(lookupForm.getFormKey())) { 
            ...
            parameters.put(OLEConstants.DISPATCH_REQUEST_PARAMETER, OLEConstants.MAINTENANCE_NEWWITHEXISTING_ACTION);
            parameters.put(OLEConstants.BUSINESS_OBJECT_CLASS_ATTRIBUTE, TaxRegion.class.getName());
            parameters.put(OLEConstants.OVERRIDE_KEYS, OLEConstants.TaxRegionConstants.TAX_REGION_TYPE_CODE);
            parameters.put(OLEConstants.REFRESH_CALLER, OLEConstants.TaxRegionConstants.TAX_REGION_TYPE_CODE + "::" + ((TaxRegionType) businessObject).getTaxRegionTypeCode());
            
            final String href = UrlFactory.parameterizeUrl(OLEConstants.MAINTENANCE_ACTION, parameters);
    ...
    public List<HtmlData> getCustomActionUrls(BusinessObject businessObject, List pkNames) {
        ...
        parameters.put(OLEConstants.OVERRIDE_KEYS, OLEConstants.TaxRegionConstants.TAX_REGION_TYPE_CODE);
        parameters.put(OLEConstants.REFRESH_CALLER, OLEConstants.TaxRegionConstants.TAX_REGION_TYPE_CODE + "::" + ((TaxRegionType) businessObject).getTaxRegionTypeCode());
        parameters.put(OLEConstants.TaxRegionConstants.TAX_REGION_TYPE_CODE, ((TaxRegionType) businessObject).getTaxRegionTypeCode());
        String href = UrlFactory.parameterizeUrl(OLEConstants.MAINTENANCE_ACTION, parameters);
