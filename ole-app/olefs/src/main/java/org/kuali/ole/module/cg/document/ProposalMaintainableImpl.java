...
import org.kuali.ole.module.cg.businessobject.ProposalResearchRisk;
...
import org.kuali.ole.module.cg.businessobject.defaultvalue.NextProposalNumberFinder;
import org.kuali.ole.module.cg.document.service.RoutingFormResearchRiskService;
...
public class ProposalMaintainableImpl extends FinancialSystemMaintainable {
    ...
    public void refresh(String refreshCaller, Map fieldValues, MaintenanceDocument document) {
        refreshProposal(OLEConstants.KUALI_LOOKUPABLE_IMPL.equals(fieldValues.get(OLEConstants.REFRESH_CALLER)));
    ...
    private void refreshProposal(boolean refreshFromLookup) {
        ...
        getNewCollectionLine(OLEPropertyConstants.PROPOSAL_SUBCONTRACTORS).refreshNonUpdateableReferences();
    ...
    private void refreshProposalProjectDirectors(boolean refreshFromLookup) {
        if (refreshFromLookup) {
            getNewCollectionLine(OLEPropertyConstants.PROPOSAL_PROJECT_DIRECTORS).refreshNonUpdateableReferences();
            refreshNonUpdateableReferences(getProposal().getProposalProjectDirectors());
        }
        else {
            refreshWithSecondaryKey((ProposalProjectDirector) getNewCollectionLine(OLEPropertyConstants.PROPOSAL_PROJECT_DIRECTORS));
