...
public class OLEDocumentSearchCustomizer implements SearchableAttribute, DocumentSearchCustomizer {
    private static final org.apache.log4j.Logger LOG = org.apache.log4j.Logger.getLogger(OLEDocumentSearchCustomizer.class);

    protected SearchableAttribute searchableAttribute;

    public OLEDocumentSearchCustomizer() {
        this(new OLESearchableAttribute());
    }

    public OLEDocumentSearchCustomizer(SearchableAttribute searchableAttribute) {
    ...
    public DocumentSearchCriteria customizeCriteria(DocumentSearchCriteria documentSearchCriteria) {
        ...
        if ( documentSearchCriteria.getDocumentAttributeValues().containsKey( OLESearchableAttribute.DISPLAY_TYPE_SEARCH_ATTRIBUTE_NAME ) ) {
            ...
            searchOptions.put(OLESearchableAttribute.DISPLAY_TYPE_SEARCH_ATTRIBUTE_NAME,
                            newCriteria.getDocumentAttributeValues().get(OLESearchableAttribute.DISPLAY_TYPE_SEARCH_ATTRIBUTE_NAME));
            newCriteria.setSearchOptions(searchOptions);
            newCriteria.getDocumentAttributeValues().remove( OLESearchableAttribute.DISPLAY_TYPE_SEARCH_ATTRIBUTE_NAME );
    ...
    public DocumentSearchResultSetConfiguration customizeResultSetConfiguration(DocumentSearchCriteria documentSearchCriteria) {
        ...
        List<String> displayTypeList = documentSearchCriteria.getSearchOptions().get(OLESearchableAttribute.DISPLAY_TYPE_SEARCH_ATTRIBUTE_NAME);
        if ( displayTypeList != null && !displayTypeList.isEmpty() ) {

            String displayType =  displayTypeList.get(0);
            if ( StringUtils.equals(displayType, OLESearchableAttribute.WORKFLOW_DISPLAY_TYPE_VALUE)) {
