...
public class PaymentRequestAccountingLineRuleHelperServiceImpl extends PurapAccountingLineRuleHelperServiceImpl {
    ...
    /**
     * @see org.kuali.ole.module.purap.service.impl.AccountsPayableAccountingLineRuleHelperServiceImpl#hasRequiredOverrides(org.kuali.ole.sys.businessobject.AccountingLine, java.lang.String)
    ...
    public boolean hasRequiredOverrides(AccountingLine line, String overrideCode) {
        ...
        if (ObjectUtils.isNotNull(account)) {
            ...
            if (PaymentRequestStatuses.APPDOC_AWAITING_SUB_ACCT_MGR_REVIEW.equals(docStatus) ||
                ...
                String expirationExtensionDays = SpringContext.getBean(ParameterService.class).getParameterValueAsString(ScrubberStep.class, OLEConstants.SystemGroupParameterNames.GL_SCRUBBER_VALIDATION_DAYS_OFFSET);
                ...
                if (account.isExpired() && account.isForContractsAndGrants() && (SpringContext.getBean(DateTimeService.class).dateDiff(account.getAccountExpirationDate(), SpringContext.getBean(DateTimeService.class).getCurrentDate(), true) > expirationExtensionDaysInt)) {
                    GlobalVariables.getMessageMap().putError(OLEConstants.ACCOUNT_NUMBER_PROPERTY_NAME, PurapKeyConstants.ERROR_ITEM_ACCOUNT_EXPIRED_REPLACE, account.getAccountNumber());
                    hasOverrides = false;
                }
            }
        } else {
    ...
    public boolean isValidSubObjectCode(SubObjectCode subObjectCode, DataDictionary dataDictionary, String errorPropertyName) {
        ...
        // MSU Contribution OLEMI-8345 DTT-4169 OLECNTRB-960
        // check active flag
        if (!subObjectCodeForValidation.isActive()) {
            if (((PaymentRequestDocument) getDocument()).getApplicationDocumentStatus().equals(
                    PaymentRequestStatuses.APPDOC_AWAITING_FISCAL_REVIEW)) {
                GlobalVariables.getMessageMap().putError(errorPropertyName, OLEKeyConstants.ERROR_INACTIVE, label);
                return false;
            }
        }
    ...
    /**
     * @see org.kuali.ole.sys.document.service.impl.AccountingLineRuleHelperServiceImpl#isValidSubAccount(org.kuali.ole.coa.businessobject.SubAccount, org.kuali.rice.krad.datadictionary.DataDictionary, java.lang.String)
     */
    @Override
    public boolean isValidSubAccount(SubAccount subAccount, DataDictionary dataDictionary, String errorPropertyName,
        ...
        // MSU Contribution OLEMI-8345 DTT-4023 OLECNTRB-960
