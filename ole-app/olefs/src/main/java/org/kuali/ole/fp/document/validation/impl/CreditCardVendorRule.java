...
import org.kuali.ole.coa.businessobject.SubObjectCodeCurrent;
import org.kuali.ole.fp.businessobject.CreditCardVendor;
...
public class CreditCardVendorRule extends MaintenanceDocumentRuleBase {
    ...
    protected boolean processCustomRouteDocumentBusinessRules(MaintenanceDocument document) {
        ...
        if ( StringUtils.isNotBlank( newCreditCardVendor.getIncomeSubAccountNumber() ) ) {
            ...
            if (checkRequiredSubAccount("Income")) {
                ...
                if (existenceSubAccount == null) {
                    putFieldError("incomeSubAccountNumber", OLEKeyConstants.ERROR_CCV_INVALIDSUBACCOUNT, "Income Sub-Account Number, " + newCreditCardVendor.getIncomeSubAccountNumber());
                }
                else

                // check the Sub-Account is active
                if (!existenceSubAccount.isActive()) {
                    putFieldError("incomeSubAccountNumber", OLEKeyConstants.ERROR_INACTIVE, "Income Sub-Account");
                }
            }
        }
        ...
        if ( StringUtils.isNotBlank( newCreditCardVendor.getExpenseSubAccountNumber() ) ) {
            if (checkRequiredSubAccount("Expense")) {
                ...
                if (existenceSubAccount == null) {
                    putFieldError("expenseSubAccountNumber", OLEKeyConstants.ERROR_CCV_INVALIDSUBACCOUNT, "Expense Sub-Account Number, " + newCreditCardVendor.getExpenseSubAccountNumber());
                ...
                // check the Sub-Account is active
                if (!existenceSubAccount.isActive()) {
                    putFieldError("expenseSubAccountNumber", OLEKeyConstants.ERROR_INACTIVE, "Expense Sub-Account");
                }
            }
        }
        ...
        if ( StringUtils.isNotBlank( newCreditCardVendor.getIncomeFinancialSubObjectCode() ) ) {
            if (checkRequiredSubObjectCode("Income")) {
                ...
                if (existenceSubObj == null) {
                    putFieldError("incomeFinancialSubObjectCode", OLEKeyConstants.ERROR_CCV_INVALIDSUBOBJECT, "Income Sub-Object Code, " + newCreditCardVendor.getIncomeFinancialSubObjectCode());
                }
                else
                // check the Sub-Object is active
                if (!existenceSubObj.isActive()) {
                    putFieldError("incomeFinancialSubObjectCode", OLEKeyConstants.ERROR_INACTIVE, "Income Sub-Object");
                }
        ...
        if ( StringUtils.isNotBlank( newCreditCardVendor.getExpenseFinancialSubObjectCode() ) ) {
            if (checkRequiredSubObjectCode("Expense")) {
                ...
                if (existenceSubObj == null) {
                    putFieldError("expenseFinancialSubObjectCode", OLEKeyConstants.ERROR_CCV_INVALIDSUBOBJECT, "Expense Sub-Object Code, " + newCreditCardVendor.getExpenseFinancialSubObjectCode());
                }
                else
                // check the Sub-Object is active
                if (!existenceSubObj.isActive()) {
                    putFieldError("expenseFinancialSubObjectCode", OLEKeyConstants.ERROR_INACTIVE, "Expense Sub-Object");
                }
            }
        }
    ...
    protected boolean checkCreditCardVendorNumber() {
        ...
        if (ccvNumber == null) {
            ...
            putFieldError("financialDocumentCreditCardVendorNumber", OLEKeyConstants.ERROR_NUMERIC, "Vendor Credit Card Number");
            return false;
        }
        else if (ccvNumber.length() < 5) {
            String errorMessage[] = null;
            errorMessage = new String[] { "Vendor Credit Card Number", "5" };
            putFieldError("financialDocumentCreditCardVendorNumber", OLEKeyConstants.ERROR_MIN_LENGTH, errorMessage);
            return false;
        }
    ...
    protected boolean checkExistingActiveAccount( String chartOfAccountsCode, String accountNumber, String fieldName, String errorMessage) {
        ...
        if (account == null) {
            putFieldError(fieldName, OLEKeyConstants.ERROR_EXISTENCE, errorMessage);
            return result;
        }

        // check whether expired or not
        if (account.isExpired()) {
            putFieldError(fieldName, OLEKeyConstants.ERROR_EXPIRED, errorMessage);
            return result;
        }

        // check whether closed or not
        if (!account.isActive()) {
            putFieldError(fieldName, OLEKeyConstants.ERROR_CLOSED, errorMessage);
            return result;
        }
    ...
    protected boolean checkRequiredSubAccount(String string) {
        ...
        if (string.equals("Income")) {
            if (newCreditCardVendor.getIncomeFinancialChartOfAccountsCode() == null) {
                putFieldError("incomeFinancialChartOfAccountsCode", OLEKeyConstants.ERROR_CCV_INCOME_SUBACCOUNT_REQUIRED, "Income Chart");
                returnVal = false;
            }

            if (newCreditCardVendor.getIncomeAccountNumber() == null) {
                putFieldError("incomeAccountNumber", OLEKeyConstants.ERROR_CCV_INCOME_SUBACCOUNT_REQUIRED, "Income Account Number");
                returnVal = false;
            }
        }
        ...
        if (string.equals("Expense")) {
            if (newCreditCardVendor.getExpenseFinancialChartOfAccountsCode() == null) {
                putFieldError("expenseFinancialChartOfAccountsCode", OLEKeyConstants.ERROR_CCV_EXPENSE_SUBACCOUNT_REQUIRED, "Expense Chart");
                returnVal = false;
            }

            if (newCreditCardVendor.getExpenseAccountNumber() == null) {
                putFieldError("expenseAccountNumber", OLEKeyConstants.ERROR_CCV_EXPENSE_SUBACCOUNT_REQUIRED, "Expense Account Number");
                returnVal = false;
            }
        }
    ...
    protected boolean checkRequiredSubObjectCode(String string) {
        ...
        if (string.equals("Income")) {
            if ( StringUtils.isBlank( newCreditCardVendor.getIncomeFinancialChartOfAccountsCode() ) ) {
                putFieldError("incomeFinancialChartOfAccountsCode", OLEKeyConstants.ERROR_CCV_INCOME_SUBOBJ_REQUIRED, "Income Chart");
                returnVal = false;
            }

            if ( StringUtils.isBlank( newCreditCardVendor.getIncomeAccountNumber() ) ) {
                putFieldError("incomeAccountNumber", OLEKeyConstants.ERROR_CCV_INCOME_SUBOBJ_REQUIRED, "Income Account Number");
                returnVal = false;
            }

            if ( StringUtils.isBlank( newCreditCardVendor.getIncomeFinancialObjectCode() ) ) {
                putFieldError("incomeFinancialObjectCode", OLEKeyConstants.ERROR_CCV_INCOME_SUBOBJ_REQUIRED, "Income Object Code");
                returnVal = false;
            }
        ...
        if (string.equals("Expense")) {
            if ( StringUtils.isBlank( newCreditCardVendor.getExpenseFinancialChartOfAccountsCode() ) ) {
                putFieldError("expenseFinancialChartOfAccountsCode", OLEKeyConstants.ERROR_CCV_EXPENSE_SUBOBJ_REQUIRED, "Expense Chart");
                returnVal = false;
            }

            if ( StringUtils.isBlank( newCreditCardVendor.getExpenseAccountNumber() ) ) {
                putFieldError("expenseAccountNumber", OLEKeyConstants.ERROR_CCV_EXPENSE_SUBOBJ_REQUIRED, "Expense Account Number");
                returnVal = false;
            }

            if ( StringUtils.isBlank( newCreditCardVendor.getExpenseFinancialObjectCode() ) ) {
                putFieldError("expenseFinancialObjectCode", OLEKeyConstants.ERROR_CCV_EXPENSE_SUBOBJ_REQUIRED, "Expense Object Code");
                returnVal = false;
            }
