...
public class KualiHeartbeatServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp){
		...
		try {
	        ...
	        DocumentType docDTO = SpringContext.getBean(DocumentTypeService.class).getDocumentTypeByName(OLEConstants.ROOT_DOCUMENT_TYPE);
	        if ( docDTO == null  ) {
	            sb.append( "POTENTIAL PROBLEM: " + OLEConstants.ROOT_DOCUMENT_TYPE + " document type not found.  Missing KFS Document hierarchy." );
	            errors = true;
	        }
	        if ( !errors ) {
