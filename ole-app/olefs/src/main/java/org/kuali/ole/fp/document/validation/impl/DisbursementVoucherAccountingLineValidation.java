...
public class DisbursementVoucherAccountingLineValidation extends GenericValidation {
    ...
    //protected static final String DV_PAYMENT_REASON_PROPERTY_PATH = OLEPropertyConstants.DV_PAYEE_DETAIL + "." + OLEPropertyConstants.DISB_VCHR_PAYMENT_REASON_CODE;
    protected static final String DV_PAYEE_ID_NUMBER_PROPERTY_PATH = OLEPropertyConstants.DV_PAYEE_DETAIL + "." + OLEPropertyConstants.DISB_VCHR_PAYEE_ID_NUMBER;
...
/*        if (StringUtils.isBlank(dvDocument.getDvPayeeDetail().getDisbVchrPaymentReasonCode())) {
            if (!errors.containsMessageKey(OLEKeyConstants.ERROR_DV_ADD_LINE_MISSING_PAYMENT_REASON)) {
                errors.putErrorWithoutFullErrorPath(OLEPropertyConstants.DOCUMENT + "." + DV_PAYMENT_REASON_PROPERTY_PATH, OLEKeyConstants.ERROR_DV_ADD_LINE_MISSING_PAYMENT_REASON);
            }
            valid = false;
        }*/
        ...
        if (StringUtils.isBlank(dvDocument.getDvPayeeDetail().getDisbVchrPayeeIdNumber())) {
            if (!errors.containsMessageKey(OLEKeyConstants.ERROR_DV_ADD_LINE_MISSING_PAYEE)) {
                errors.putErrorWithoutFullErrorPath(OLEPropertyConstants.DOCUMENT + "." + DV_PAYEE_ID_NUMBER_PROPERTY_PATH, OLEKeyConstants.ERROR_DV_ADD_LINE_MISSING_PAYEE);
            }
            valid = false;
        }
    ...
    public boolean validateObjectCode(AccountingDocument financialDocument, AccountingLine accountingLine) {
        ...
        if (!accountingLine.getObjectCode().isFinancialObjectActiveCode()) {
            errors.putError(OLEPropertyConstants.FINANCIAL_OBJECT_CODE, OLEKeyConstants.ERROR_INACTIVE, "Object Code");
    ...
    public boolean validateAccountNumber(AccountingDocument financialDocument, AccountingLine accountingLine) {
        ...
        String errorKey = OLEPropertyConstants.ACCOUNT_NUMBER;
      ...
      //Commented for the jira issue OLE-3415
        ...
        /*if (accountNumberAllowed) {
            ParameterEvaluator evaluator = SpringContext.getBean(ParameterEvaluatorService.class).getParameterEvaluator(DisbursementVoucherDocument.class, DisbursementVoucherConstants.FUNCTION_CODE_GLOBAL_RESTRICTION_PARM_NM, accountingLine.getAccount().getFinancialHigherEdFunctionCd());
            // accountNumberAllowed is true now
            accountNumberAllowed = evaluator.evaluateAndAddError(SourceAccountingLine.class, "account.financialHigherEdFunctionCd", OLEPropertyConstants.ACCOUNT_NUMBER);
        }*/
