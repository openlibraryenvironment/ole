...
public class SubAccountReviewRoleTypeServiceImpl extends RoleTypeServiceBase {
    ...
    protected boolean performMatch(Map<String,String> qualification, Map<String,String> roleQualifier) {
        if (StringUtils.isBlank(roleQualifier.get(OleKimAttributes.ACCOUNT_NUMBER))) {
            if (KimCommonUtils.storedValueNotSpecifiedOrInputValueMatches(roleQualifier, qualification, OleKimAttributes.CHART_OF_ACCOUNTS_CODE) 
                    && KimCommonUtils.storedValueNotSpecifiedOrInputValueMatches(roleQualifier, qualification, OleKimAttributes.ORGANIZATION_CODE) 
                    && KimCommonUtils.storedValueNotSpecifiedOrInputValueMatches(roleQualifier, qualification, OleKimAttributes.SUB_ACCOUNT_NUMBER)) {
            ...
            if (KimCommonUtils.storedValueNotSpecifiedOrInputValueMatches(roleQualifier, qualification, OleKimAttributes.CHART_OF_ACCOUNTS_CODE) 
                    && KimCommonUtils.storedValueNotSpecifiedOrInputValueMatches(roleQualifier, qualification, OleKimAttributes.ACCOUNT_NUMBER) 
                    && KimCommonUtils.storedValueNotSpecifiedOrInputValueMatches(roleQualifier, qualification, OleKimAttributes.SUB_ACCOUNT_NUMBER)) {
                Set<String> potentialParentDocumentTypeNames = new HashSet<String>(1);
                if (roleQualifier.containsKey(KimConstants.AttributeConstants.DOCUMENT_TYPE_NAME)) {
                    potentialParentDocumentTypeNames.add(roleQualifier.get(KimConstants.AttributeConstants.DOCUMENT_TYPE_NAME));
    ...
    public List<RemotableAttributeError> validateAttributes(String kimTypeId, Map<String,String> attributes) {
        ...
        String chartCode = attributes.get(OleKimAttributes.CHART_OF_ACCOUNTS_CODE);
        String organizationCode = attributes.get(OleKimAttributes.ORGANIZATION_CODE);
        String accountNumber = attributes.get(OleKimAttributes.ACCOUNT_NUMBER);
        String subAccountNumber = attributes.get(OleKimAttributes.SUB_ACCOUNT_NUMBER);
        ...
        if (StringUtils.isEmpty(accountNumber) && StringUtils.isEmpty(organizationCode)) {
            ...
            for ( RemotableAttributeError err : originalErrorMap ) {
                if ( err.getAttributeName().equals( OleKimAttributes.ACCOUNT_NUMBER ) ) {
                    // remove (by not adding) to the new error map
                } else if ( err.getAttributeName().equals( OleKimAttributes.ORGANIZATION_CODE ) ) {
                    // remove (by not adding) to the new error map                    
                } else if ( err.getAttributeName().equals(OleKimAttributes.CHART_OF_ACCOUNTS_CODE) && StringUtils.isEmpty(chartCode) ) {
                    // remove (by not adding) to the new error map                                        
                } else if ( err.getAttributeName().equals(OleKimAttributes.SUB_ACCOUNT_NUMBER) && StringUtils.isEmpty(subAccountNumber) ) {
                    // remove (by not adding) to the new error map                    
                } else {
            ...
            if (StringUtils.isEmpty(chartCode)) {
                Builder caBuilder = RemotableAttributeError.Builder.create(OleKimAttributes.CHART_OF_ACCOUNTS_CODE, OLEKeyConstants.ERROR_CHART_OR_ORG_NOTEMPTY_ALL_REQUIRED);
                errorMap.add(caBuilder.build());
            }
                
            if (StringUtils.isEmpty(subAccountNumber)) {
                Builder caBuilder = RemotableAttributeError.Builder.create(OleKimAttributes.SUB_ACCOUNT_NUMBER, OLEKeyConstants.ERROR_CHART_OR_ORG_NOTEMPTY_ALL_REQUIRED);
                errorMap.add(caBuilder.build());                
            }
        }
        return errorMap;
    ...
    public List<KimAttributeField> getAttributeDefinitions(String kimTypeId) {
        ...
        for (KimAttributeField definition : super.getAttributeDefinitions(kimTypeId)) {
            ...
            List<String> attributeList = new ArrayList<String>(){{
                add(OleKimAttributes.SUB_ACCOUNT_NUMBER);
                add(OleKimAttributes.CHART_OF_ACCOUNTS_CODE);
                add(OleKimAttributes.ACCOUNT_NUMBER);
                add(OleKimAttributes.ORGANIZATION_CODE);
            }};
