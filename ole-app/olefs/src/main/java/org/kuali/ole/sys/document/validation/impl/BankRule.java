...
public class BankRule extends MaintenanceDocumentRuleBase {
    ...
    protected boolean checkPartiallyFilledOutReferences() {
        ...
        valid &= checkForPartiallyFilledOutReferenceForeignKeys(OLEPropertyConstants.CASH_OFFSET_ACCOUNT);
        valid &= checkForPartiallyFilledOutReferenceForeignKeys(OLEPropertyConstants.CASH_OFFSET_OBJECT);
    ...
    protected boolean validateFieldsForBankOffsetEntries() {
        ...
        if (SpringContext.getBean(BankService.class).isBankSpecificationEnabled()) {
            ...
            if (StringUtils.isBlank(newBank.getCashOffsetAccountNumber())) {
                putFieldError(OLEPropertyConstants.CASH_OFFSET_ACCOUNT_NUMBER, OLEKeyConstants.Bank.ERROR_MISSING_CASH_ACCOUNT_NUMBER);
                valid = false;
            }
    
            if (StringUtils.isBlank(newBank.getCashOffsetObjectCode())) {
                putFieldError(OLEPropertyConstants.CASH_OFFSET_OBJECT_CODE, OLEKeyConstants.Bank.ERROR_MISSING_CASH_OBJECT_CODE);
                valid = false;
            }
        }
    ...
    protected boolean validateBankAccountNumber() {
        ...
        if ( StringUtils.isNotBlank(newBank.getBankAccountNumber() )
            ...
            Collection existingBanks = getBoService().findMatching(Bank.class, Collections.singletonMap(OLEPropertyConstants.BANK_ACCOUNT_NUMBER, newBank.getBankAccountNumber()));
            if ( existingBanks != null && !existingBanks.isEmpty() ) {
                putFieldError(OLEPropertyConstants.BANK_ACCOUNT_NUMBER, OLEKeyConstants.Bank.ERROR_ACCOUNT_NUMBER_NOT_UNIQUE);
                return false;
            }
        }
        return true;        
