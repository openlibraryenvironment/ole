...
public class OleDiscountPaymentRequestEvent extends KualiDocumentEventBase {

    private OlePaymentRequestItem payItem;

    protected OleDiscountPaymentRequestEvent(String errorPathPrefix, Document document) {
        super("Creating Note Tag" + getDocumentId(document), errorPathPrefix, document);
    }

    public OleDiscountPaymentRequestEvent(Document document, OlePaymentRequestItem payItem) {
        this("", document);
    ...
    public boolean invokeRuleMethod(BusinessRule rule) {
        return ((OleValidationRule) rule).processCustomAddDiscountPaymentRequestBusinessRules(document, payItem);
    }
    ...
    public void setPayItem(OlePaymentRequestItem payItem) {
        this.payItem = payItem;
    }
