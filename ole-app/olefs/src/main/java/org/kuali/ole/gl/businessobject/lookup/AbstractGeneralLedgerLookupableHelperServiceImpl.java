...
public abstract class AbstractGeneralLedgerLookupableHelperServiceImpl extends AbstractLookupableHelperServiceImpl {
    ...
    protected boolean isConsolidationSelected(Map fieldValues) {
        ...
        String subObjectCode = (String) fieldValues.get(OLEPropertyConstants.SUB_OBJECT_CODE);
        ...
        String objectTypeCode = (String) fieldValues.get(OLEPropertyConstants.OBJECT_TYPE_CODE);
        ...
        if (Constant.DEBIT_CREDIT_EXCLUDE.equals(debitCreditOption)){
            for(Object table : resultTable) {
                ...
                for(Column col: columns) {
                    ...
                    if (propertyName.equals(OLEPropertyConstants.TRANSACTION_DEBIT_CREDIT_CODE)) {
                        debitCreditCode = col.getPropertyValue();
                    }
                    else if (!propertyName.equals(OLEPropertyConstants.TRANSACTION_LEDGER_ENTRY_AMOUNT)){
                        newColumnList.add(col);
                    }

                    if(propertyName.equals(OLEPropertyConstants.FINANCIAL_OBJECT_TYPE_CODE)) {
                        objectType = col.getPropertyValue();
                    }

                    if (propertyName.equals(OLEPropertyConstants.TRANSACTION_LEDGER_ENTRY_AMOUNT)) {
                        amountCol = col;
                    }
