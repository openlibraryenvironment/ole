...
public class CollectorFlatFileInputType extends BatchInputFileTypeBase {
    ...
    /**
     * @see org.kuali.ole.sys.batch.BatchInputType#getAuthorPrincipalName(java.io.File)
     */
    ...
    public String getFileTypeIdentifer() {
        return OLEConstants.COLLECTOR_FLAT_FILE_TYPE_INDENTIFIER;
    }
    ...
    public String getTitleKey() {
        return OLEKeyConstants.MESSAGE_BATCH_UPLOAD_TITLE_COLLECTOR_FLAT_FILE;
    }

    /**
     * @see org.kuali.ole.sys.batch.BatchInputFileType#parse(byte[])
     */
                ...
                if (fileLine.length() >= 27) {  //if no rec_type, probably a blank or almost blank line at end of file
                    ...
                    if ("HD".equals(recordType)) {  // this is a header line
                        ...
                        try {
                            ...
                            currentBatch.getMessageMap().putError(OLEConstants.GLOBAL_ERRORS, OLEKeyConstants.ERROR_CUSTOM, e.getMessage());
                        }
    ...
    protected void ensurePreviousBatchTerminated(CollectorBatch currentBatch, int lineNumber) {
        if (currentBatch != null) {
            ...
            currentBatch.getMessageMap().putError(OLEConstants.GLOBAL_ERRORS, OLEKeyConstants.Collector.MISSING_TRAILER_RECORD, Integer.toString(lineNumber));
    ...
    protected CollectorBatch createHeaderlessBatchIfNecessary(CollectorBatch currentBatch, List<CollectorBatch> batchList, int lineNumber) {
        ...
        headerlessBatch.getMessageMap().putError(OLEConstants.GLOBAL_ERRORS, OLEKeyConstants.Collector.MISSING_HEADER_RECORD, Integer.toString(lineNumber));
    ...
    protected OriginEntryFull createOriginEntry(String fileLine, Date curDate, UniversityDate universityDate, int lineNumber, MessageMap messageMap) {
        ...
        try{        
            ...
            for (Message orginEntryError : originEntryErrorMessages) {
               messageMap.putError(OLEConstants.GLOBAL_ERRORS, OLEKeyConstants.ERROR_CUSTOM, orginEntryError.getMessage()); 
            }
    ...
    protected String extractRecordType(String line) {
        ...
        recordType = (collectorBatch.getValue(line, pMap.get(OLEPropertyConstants.COLLECTOR_BATCH_RECORD_TYPE), pMap.get(OLEPropertyConstants.BATCH_SEQUENCE_NUMBER)));
