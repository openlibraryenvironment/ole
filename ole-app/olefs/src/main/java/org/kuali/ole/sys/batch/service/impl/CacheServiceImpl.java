...
/**
 * @see org.kuali.ole.sys.batch.service.CacheService
 */
@NonTransactional
public class CacheServiceImpl implements CacheService {
    ...
    /**
     * @see org.kuali.ole.sys.batch.service.CacheService#clearSystemCache()
     */
    @Override
    ...
    public void clearNamedCache(String cacheName) {
        CacheManager cm = CoreImplServiceLocator.getCacheManagerRegistry().getCacheManager(
                OLEConstants.KFS_CORE_DISTRIBUTED_CACHE_MANAGER);
        if (cm != null) {
            Cache cache = cm.getCache(cacheName);
            if (cache != null) {
                cache.clear();
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Cleared " + cacheName + " cache in cache manager "
                            + OLEConstants.KFS_CORE_DISTRIBUTED_CACHE_MANAGER + ".");
                }
            }
            else {
                // this is at debug level intentionally, since not all BOs have caches
                LOG.debug("Unable to find cache for " + cacheName + " in cache manager "
                        + OLEConstants.KFS_CORE_DISTRIBUTED_CACHE_MANAGER);
            }
        }
        else {
            LOG.info("Unable to find cache manager " + OLEConstants.KFS_CORE_DISTRIBUTED_CACHE_MANAGER
                    + " when attempting to clear " + cacheName);
        }
    }

    @Override
    public void clearKfsBusinessObjectCache(Class boClass) {
        String cacheName = OLEConstants.APPLICATION_NAMESPACE_CODE + "/" + boClass.getSimpleName();
