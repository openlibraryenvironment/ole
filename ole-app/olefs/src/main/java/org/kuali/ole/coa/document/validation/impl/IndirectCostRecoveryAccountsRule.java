...
abstract public class IndirectCostRecoveryAccountsRule extends KfsMaintenanceDocumentRuleBase {
    ...
    protected boolean checkIndirectCostRecoveryAccount(String chartOfAccountsCode, String accountNumber, BigDecimal icraAccountLinePercentage) {
        ...
        if (StringUtils.isBlank(chartOfAccountsCode)) {
            GlobalVariables.getMessageMap().putError(OLEPropertyConstants.ICR_CHART_OF_ACCOUNTS_CODE, OLEKeyConstants.ERROR_REQUIRED, 
                    getDDAttributeLabel(OLEPropertyConstants.ICR_CHART_OF_ACCOUNTS_CODE));
            success &= false;
        }
        
        if (StringUtils.isBlank(accountNumber)) {
            GlobalVariables.getMessageMap().putError(OLEPropertyConstants.ICR_ACCOUNT_NUMBER, OLEKeyConstants.ERROR_REQUIRED,
                    getDDAttributeLabel(OLEPropertyConstants.ICR_ACCOUNT_NUMBER));
            success &= false;
        }
        ...
        if (StringUtils.isNotBlank(chartOfAccountsCode) && StringUtils.isNotBlank(accountNumber)) {
            ...
            chartAccountMap.put(OLEPropertyConstants.CHART_OF_ACCOUNTS_CODE, chartOfAccountsCode);
            if (SpringContext.getBean(BusinessObjectService.class).countMatching(Chart.class, chartAccountMap) < 1) {
                GlobalVariables.getMessageMap().putError(OLEPropertyConstants.ICR_CHART_OF_ACCOUNTS_CODE, OLEKeyConstants.ERROR_EXISTENCE, chartOfAccountsCode);
                success &= false;
            }
            chartAccountMap.put(OLEPropertyConstants.ACCOUNT_NUMBER, accountNumber);
            if (SpringContext.getBean(BusinessObjectService.class).countMatching(Account.class, chartAccountMap) < 1) {
                GlobalVariables.getMessageMap().putError(OLEPropertyConstants.ICR_ACCOUNT_NUMBER, OLEKeyConstants.ERROR_EXISTENCE, chartOfAccountsCode + "-" + accountNumber);
                success &= false;
            }
        }
        ...
        if (icraAccountLinePercentage.compareTo(BigDecimal.ZERO) <= 0 || icraAccountLinePercentage.compareTo(BD100) == 1){
            GlobalVariables.getMessageMap().putError(OLEPropertyConstants.ICR_ACCOUNT_LINE_PERCENT, 
                    OLEKeyConstants.ERROR_DOCUMENT_ACCMAINT_ICR_ACCOUNT_INVALID_LINE_PERCENT);
            success &= false;
        }
    ...
    protected boolean checkIndirectCostRecoveryAccountDistributions() {
        ...
        if (totalDistribution.compareTo(BD100) != 0){
            putFieldError(boFieldPath, OLEKeyConstants.ERROR_DOCUMENT_ACCMAINT_ICR_ACCOUNT_TOTAL_NOT_100_PERCENT);
            result &= false;
        }
