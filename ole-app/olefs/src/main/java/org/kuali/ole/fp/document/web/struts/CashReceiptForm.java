...
public class CashReceiptForm extends CapitalAccountingLinesFormBase implements CapitalAssetEditable{
    ...
    public String getFinancialDocumentStatusMessage() {
        ...
        if (financialDocumentStatusCode.equals(CashReceipt.VERIFIED)) {
            financialDocumentStatusMessage = SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(OLEKeyConstants.CashReceipt.MSG_VERIFIED_BUT_NOT_AWAITING_DEPOSIT);
        }
            ...
            if (cmd != null) {
                ...
                String loadCMDocUrl = SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(OLEKeyConstants.CashManagement.URL_LOAD_DOCUMENT_CASH_MGMT);
                loadCMDocUrl = StringUtils.replace(loadCMDocUrl, "{0}", cmdFinancialDocNbr);

                financialDocumentStatusMessage = SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(OLEKeyConstants.CashReceipt.MSG_VERIFIED_AND_AWAITING_DEPOSIT);
                financialDocumentStatusMessage = StringUtils.replace(financialDocumentStatusMessage, "{0}", loadCMDocUrl);
            }
        }
        else if (financialDocumentStatusCode.equals(OLEConstants.DocumentStatusCodes.APPROVED)) {
            CashManagementDocument cmd = SpringContext.getBean(CashManagementService.class).getCashManagementDocumentForCashReceiptId(crd.getDocumentNumber());
            if (cmd != null) {
                String cmdFinancialDocNbr = cmd.getDocumentNumber();
                ...
                financialDocumentStatusMessage = SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(OLEKeyConstants.CashReceipt.MSG_VERIFIED_AND_DEPOSITED);
                financialDocumentStatusMessage = StringUtils.replace(financialDocumentStatusMessage, "{0}", loadCMDocUrl);
            }
        }
            ...
            if (crd.getDocumentHeader().getWorkflowDocument().isEnroute()) {
                ...
                if ( cd != null ) {
                    if (crd.getDocumentHeader().getWorkflowDocument().isApprovalRequested()
                        ...
                        cashDrawerStatusMessage = SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(OLEKeyConstants.CashReceipt.MSG_CASH_DRAWER_CLOSED_VERIFICATION_NOT_ALLOWED);
    ...
    /**
     * @see org.kuali.ole.sys.web.struts.KualiAccountingDocumentFormBase#getExcludedmethodToCall()
     */
    @Override
