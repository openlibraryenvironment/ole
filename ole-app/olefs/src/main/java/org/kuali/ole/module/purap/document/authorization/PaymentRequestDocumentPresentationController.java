...
import org.kuali.ole.module.purap.PurapAuthorizationConstants.PaymentRequestEditMode;
...
public class PaymentRequestDocumentPresentationController extends PurchasingAccountsPayableDocumentPresentationController {
    ...
    public Set<String> getEditModes(Document document) {
        ...
        if (SpringContext.getBean(PurapService.class).isFullDocumentEntryCompleted(paymentRequestDocument)) {
          ...
          /*
            ...
            boolean canClosePO = KIMServiceLocator.getIdentityManagementService().hasPermission(GlobalVariables.getUserSession().getPerson().getPrincipalId(), nameSpaceCode,
                    OLEConstants.OlePaymentRequest.CAN_CLOSE_PO, permissionDetails);
            if(canClosePO) {
        ...
        if (PurchaseOrderStatuses.APPDOC_AWAIT_TAX_REVIEW
                .equals(paymentRequestDocument.getApplicationDocumentStatus())) {
            editModes.add(PaymentRequestEditMode.TAX_AREA_EDITABLE);
        }
    ...
    protected boolean canHold(PaymentRequestDocument paymentRequestDocument) {
        if (canHold == null) {
            ...
            boolean can = !paymentRequestDocument.isHoldIndicator()
            ...
            if (can) {
                can = SpringContext.getBean(FinancialSystemWorkflowHelperService.class)
                        .isAdhocApprovalRequestedForPrincipal(
                                paymentRequestDocument.getFinancialSystemDocumentHeader().getWorkflowDocument(),
                                GlobalVariables.getUserSession().getPrincipalId());
                can = can
                        || !PaymentRequestStatuses.STATUSES_DISALLOWING_HOLD.contains(paymentRequestDocument
    ...
    protected boolean canRequestCancel(PaymentRequestDocument paymentRequestDocument) {
        if (canRequestCancel == null) {
            boolean can = !paymentRequestDocument.isPaymentRequestedCancelIndicator()
                    && !paymentRequestDocument.isHoldIndicator() && !paymentRequestDocument.isExtracted();
            if (can) {
                can = SpringContext.getBean(FinancialSystemWorkflowHelperService.class)
                        .isAdhocApprovalRequestedForPrincipal(
                                paymentRequestDocument.getFinancialSystemDocumentHeader().getWorkflowDocument(),
                                GlobalVariables.getUserSession().getPrincipalId());
                can = can
                        || !PaymentRequestStatuses.STATUSES_DISALLOWING_REQUEST_CANCEL.contains(paymentRequestDocument
                        .getApplicationDocumentStatus());
            }
    ...
    protected boolean canEditPreExtraction(PaymentRequestDocument paymentRequestDocument) {
        if (canEditPreExtraction == null) {
            boolean can = (!paymentRequestDocument.isExtracted()
                    ...
                    .isAdhocApprovalRequestedForPrincipal(
                            paymentRequestDocument.getFinancialSystemDocumentHeader().getWorkflowDocument(),
                            GlobalVariables.getUserSession().getPrincipalId()) && !PurapConstants.PaymentRequestStatuses.CANCELLED_STATUSES
