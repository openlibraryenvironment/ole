...
import org.kuali.ole.coa.businessobject.AccountIntf;
import org.kuali.ole.coa.businessobject.CarryForwardReversionProcessOrganizationInfo;
import org.kuali.ole.coa.businessobject.ClosedAccountOrganizationReversion;
import org.kuali.ole.coa.businessobject.ObjectCode;
import org.kuali.ole.coa.businessobject.OrganizationReversion;
import org.kuali.ole.coa.businessobject.OrganizationReversionCategory;
import org.kuali.ole.coa.businessobject.OrganizationReversionCategoryInfo;
import org.kuali.ole.coa.service.OrganizationReversionService;
import org.kuali.ole.coa.service.PriorYearAccountService;
import org.kuali.ole.gl.GeneralLedgerConstants;
import org.kuali.ole.gl.batch.service.OrganizationReversionCategoryLogic;
import org.kuali.ole.gl.batch.service.OrganizationReversionProcess;
import org.kuali.ole.gl.batch.service.OrganizationReversionUnitOfWorkService;
...
import org.kuali.ole.gl.businessobject.OrgReversionUnitOfWorkCategoryAmount;
...
public class OrganizationReversionProcessImpl implements OrganizationReversionProcess, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        this.DEFAULT_FINANCIAL_DOCUMENT_TYPE_CODE = getParameterService().getParameterValueAsString(OleParameterConstants.GENERAL_LEDGER_BATCH.class, GeneralLedgerConstants.ANNUAL_CLOSING_DOCUMENT_TYPE);
        ...
        this.CASH_REVERTED_TO_MESSAGE = getConfigurationService().getPropertyValueAsString(OLEKeyConstants.OrganizationReversionProcess.CASH_REVERTED_TO);
        this.FUND_BALANCE_REVERTED_TO_MESSAGE = getConfigurationService().getPropertyValueAsString(OLEKeyConstants.OrganizationReversionProcess.FUND_BALANCE_REVERTED_TO);
        this.CASH_REVERTED_FROM_MESSAGE = getConfigurationService().getPropertyValueAsString(OLEKeyConstants.OrganizationReversionProcess.CASH_REVERTED_FROM);
        this.FUND_BALANCE_REVERTED_FROM_MESSAGE = getConfigurationService().getPropertyValueAsString(OLEKeyConstants.OrganizationReversionProcess.FUND_BALANCE_REVERTED_FROM);
        this.FUND_CARRIED_MESSAGE = getConfigurationService().getPropertyValueAsString(OLEKeyConstants.OrganizationReversionProcess.FUND_CARRIED);
        this.FUND_REVERTED_TO_MESSAGE = getConfigurationService().getPropertyValueAsString(OLEKeyConstants.OrganizationReversionProcess.FUND_REVERTED_TO);
        this.FUND_REVERTED_FROM_MESSAGE = getConfigurationService().getPropertyValueAsString(OLEKeyConstants.OrganizationReversionProcess.FUND_REVERTED_FROM);
    ...
    public void organizationReversionProcess(Map jobParameters, Map<String, Integer> organizationReversionCounts) {
        ...
        try {
            ...
            Iterator<Balance> balances = getBalanceService().findOrganizationReversionBalancesForFiscalYear((Integer) jobParameters.get(OLEConstants.UNIV_FISCAL_YR), usePriorYearInformation);
    ...
    protected void retrieveCurrentReversionAndAccount(Balance bal) throws FatalErrorException {
        ...
        if ((organizationReversion == null) || (!organizationReversion.getChartOfAccountsCode().equals(bal.getChartOfAccountsCode())) || (!organizationReversion.getOrganizationCode().equals(account.getOrganizationCode()))) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Organization Reversion Service: " + getOrganizationReversionService() + "; fiscal year: " + (Integer) jobParameters.get(OLEConstants.UNIV_FISCAL_YR) + "; account: " + account + "; account organization code: " + account.getOrganizationCode() + "; balance: " + bal + "; balance chart: " + bal.getChartOfAccountsCode());
            }
            organizationReversion = getOrganizationReversionService().getByPrimaryId((Integer) jobParameters.get(OLEConstants.UNIV_FISCAL_YR), bal.getChartOfAccountsCode(), account.getOrganizationCode());
        }

        if (organizationReversion == null) {
            // we can't find an organization reversion for this balance? Throw exception
            throw new FatalErrorException("No Organization Reversion found for: " + (Integer) jobParameters.get(OLEConstants.UNIV_FISCAL_YR) + "-" + bal.getChartOfAccountsCode() + "-" + account.getOrganizationCode());
        }
    ...
    public void initializeProcess() {
        ...
        this.paramFiscalYear = (Integer) jobParameters.get(OLEConstants.UNIV_FISCAL_YR);
        ...
        if (getCashOrganizationReversionCategoryLogic().containsObjectCode(bal.getFinancialObject()) && bal.getBalanceTypeCode().equals(systemOptions.getActualFinancialBalanceTypeCd())) {
            ...
            for (OrganizationReversionCategory cat : categoryList) {
                ...
                if (logic.containsObjectCode(bal.getFinancialObject())) {
                    ...
                    else if (OLEConstants.BALANCE_TYPE_CURRENT_BUDGET.equals(bal.getBalanceTypeCode())) {
                        // Budget
    ...
    protected OriginEntryFull getEntry() {
        ...
        entry.setUniversityFiscalYear((Integer) jobParameters.get(OLEConstants.UNIV_FISCAL_YR));
        entry.setUniversityFiscalPeriodCode(OLEConstants.MONTH13);
        ...
        entry.setTransactionDebitCreditCode(OLEConstants.GL_BUDGET_CODE);
        entry.setTransactionDate((Date) jobParameters.get(OLEConstants.TRANSACTION_DT));
    ...
    public void generateCashReversions(List<OriginEntryFull> originEntriesToWrite) throws FatalErrorException {
        ...
        getPersistenceService().retrieveReferenceObject(entry, OLEPropertyConstants.FINANCIAL_OBJECT);
        ...
        if (unitOfWork.getTotalCash().compareTo(KualiDecimal.ZERO) > 0) {
            entry.setTransactionDebitCreditCode(OLEConstants.GL_CREDIT_CODE);
        }
        else {
            entry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
        ...
        entry.setAccountNumber(unitOfWork.accountNumber);
        entry.setSubAccountNumber(unitOfWork.subAccountNumber);
        entry.setFinancialObjectCode((String) jobParameters.get(OLEConstants.FUND_BAL_OBJECT_CD));
        entry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
    ...
    public void generateMany(List<OriginEntryFull> originEntriesToWrite) throws FatalErrorException {
        ...
        for (Iterator<OrganizationReversionCategory> iter = categoryList.iterator(); iter.hasNext();) {
            ...
            if (!amount.getCarryForward().isZero()) {
                ...
                OriginEntryFull entry = getEntry();
                entry.setUniversityFiscalYear((Integer) jobParameters.get(OLEConstants.UNIV_FISCAL_YR) + 1);
                entry.setChartOfAccountsCode(unitOfWork.chartOfAccountsCode);
                entry.setAccountNumber(unitOfWork.accountNumber);
                entry.setSubAccountNumber(unitOfWork.subAccountNumber);
                entry.setFinancialObjectCode((String) jobParameters.get(OLEConstants.BEG_BUD_CASH_OBJECT_CD));
                entry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
                entry.setFinancialBalanceTypeCode(OLEConstants.BALANCE_TYPE_CURRENT_BUDGET);
                ...
                entry.setUniversityFiscalPeriodCode(OLEConstants.MONTH1);
                entry.setDocumentNumber(DEFAULT_DOCUMENT_NUMBER_PREFIX + unitOfWork.accountNumber);
                entry.setTransactionLedgerEntryDescription(FUND_CARRIED_MESSAGE + (Integer) jobParameters.get(OLEConstants.UNIV_FISCAL_YR));
    ...
    public void generateCarryForwards(List<OriginEntryFull> originEntriesToWrite) throws FatalErrorException {
        ...
        entry.setAccountNumber(unitOfWork.accountNumber);
        entry.setSubAccountNumber(unitOfWork.subAccountNumber);
        entry.setFinancialObjectCode((String) jobParameters.get(OLEConstants.UNALLOC_OBJECT_CD));
    ...
    public void generateReversions(List<OriginEntryFull> originEntriesToWrite) throws FatalErrorException {
        ...
        entry.setUniversityFiscalPeriodCode(OLEConstants.MONTH13);
        entry.setDocumentNumber(DEFAULT_DOCUMENT_NUMBER_PREFIX + unitOfWork.accountNumber);
        if (unitOfWork.accountNumber.equals(OLEConstants.getDashSubAccountNumber())) {
    ...
    public void calculateTotals() throws FatalErrorException {
        ...
        for (OrganizationReversionCategory category : categoryList) {
            ...
            if (OLEConstants.RULE_CODE_R1.equals(ruleCode) || OLEConstants.RULE_CODE_N1.equals(ruleCode) || OLEConstants.RULE_CODE_C1.equals(ruleCode)) {
            ...
            if (OLEConstants.RULE_CODE_A.equals(ruleCode)) {
                unitOfWork.addTotalCarryForward(amount.getAvailable());
                amount.addCarryForward(amount.getAvailable());
                unitOfWork.addTotalReversion(amount.getAvailable().negated());
                amount.setAvailable(KualiDecimal.ZERO);
            ...
            if (OLEConstants.RULE_CODE_C1.equals(ruleCode) || OLEConstants.RULE_CODE_C2.equals(ruleCode)) {
            ...
            if (OLEConstants.RULE_CODE_N1.equals(ruleCode) || OLEConstants.RULE_CODE_N2.equals(ruleCode)) {
                if (amount.getAvailable().compareTo(KualiDecimal.ZERO) < 0) {
                    unitOfWork.addTotalCarryForward(amount.getAvailable());
                    amount.addCarryForward(amount.getAvailable());
                    unitOfWork.addTotalReversion(amount.getAvailable().negated());
