...
public class PayeeAchAccountRule extends MaintenanceDocumentRuleBase {
    ...
    protected boolean checkForDuplicateRecord() {
        ...
        if (matches > 0) {
            putFieldError(PdpPropertyConstants.PAYEE_ID_NUMBER, OLEKeyConstants.ERROR_DOCUMENT_PAYEEACHACCOUNTMAINT_DUPLICATE_RECORD);
            valid = false;
        }
    ...
    protected boolean checkTransactionTypeAllowed() {
        ...
        if (!allowed) {
            putFieldError(PdpPropertyConstants.ACH_TRANSACTION_TYPE, OLEKeyConstants.ERROR_DOCUMENT_PAYEEACHACCOUNTMAINT_TRANSACTION_TYPE_NOT_ALLOWED, transType);
        }
        return allowed;
    }
