...
import static org.kuali.ole.sys.OLEPropertyConstants.AWARD_ACCOUNTS;
import static org.kuali.ole.sys.OLEPropertyConstants.AWARD_PROJECT_DIRECTORS;
import static org.kuali.ole.sys.OLEPropertyConstants.AWARD_SUBCONTRACTORS;
import static org.kuali.ole.sys.OLEPropertyConstants.DOCUMENT;
import static org.kuali.ole.sys.OLEPropertyConstants.NEW_MAINTAINABLE_OBJECT;
...
//        }
    ...
    public void refresh(String refreshCaller, Map fieldValues, MaintenanceDocument document) {
        if (StringUtils.equals("proposalLookupable", (String) fieldValues.get(OLEConstants.REFRESH_CALLER))) {
            ...
            if (awarded) {
                ...
                GlobalVariables.getMessageMap().putError(OLEPropertyConstants.PROPOSAL_NUMBER, OLEKeyConstants.ERROR_AWARD_PROPOSAL_AWARDED, new String[] { getAward().getProposalNumber().toString() });
            ...
            // GlobalVariables.getMessageMap().putError(OLEPropertyConstants.PROPOSAL_NUMBER,
            // OLEKeyConstants.ERROR_AWARD_PROPOSAL_INACTIVE, new String[] { getAward().getProposalNumber().toString() });
            // }

            // copy over proposal values after refresh
            if (!awarded) {
                refreshAward(true);
                fieldValues.put(OLEConstants.REFERENCES_TO_REFRESH, "proposal");
                super.refresh(refreshCaller, fieldValues, document);
                getAward().populateFromProposal(getAward().getProposal());
                refreshAward(true);
            }
        } else {
            refreshAward(OLEConstants.KUALI_LOOKUPABLE_IMPL.equals(fieldValues.get(OLEConstants.REFRESH_CALLER)));
            super.refresh(refreshCaller, fieldValues, document);
        }
