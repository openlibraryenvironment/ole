...
public abstract class OrganizationHierarchyAwareRoleTypeServiceBase extends RoleTypeServiceBase implements DelegationTypeService {
    ...
    protected static final String DOCUMENT_TYPE_NAME = "OLE_ORR";

    private List<String> workflowRoutingAttributes = new ArrayList<String>(2);
    {
        workflowRoutingAttributes.add( OleKimAttributes.CHART_OF_ACCOUNTS_CODE );
        workflowRoutingAttributes.add( OleKimAttributes.ORGANIZATION_CODE );
    ...
    public Map<String,String> convertQualificationForMemberRoles(String namespaceCode, String roleName, String memberRoleNamespaceCode, String memberRoleName, Map<String,String> qualification) {
        ...
        if ( StringUtils.isBlank( qualification.get(KimConstants.AttributeConstants.CAMPUS_CODE ) )
                && StringUtils.isNotBlank( OleKimAttributes.CHART_OF_ACCOUNTS_CODE)
                && StringUtils.isNotBlank( OleKimAttributes.ORGANIZATION_CODE ) ) {
            Map<String,String> newQualification = new HashMap<String,String>(qualification);
            try {
                Organization org = organizationService.getByPrimaryId(qualification.get(OleKimAttributes.CHART_OF_ACCOUNTS_CODE), qualification.get(OleKimAttributes.ORGANIZATION_CODE));
                if ( org != null ) {
    ...
    protected class SortableRoleMembershipHolder implements Comparable<SortableRoleMembershipHolder> {
        ...
        public SortableRoleMembershipHolder( RoleMembership rmi ) {
            this.rmi = rmi;
            chart = rmi.getQualifier().get(OleKimAttributes.CHART_OF_ACCOUNTS_CODE);
            org = rmi.getQualifier().get(OleKimAttributes.ORGANIZATION_CODE);
            RoleMembership.Builder builder = RoleMembership.Builder.create(rmi);
