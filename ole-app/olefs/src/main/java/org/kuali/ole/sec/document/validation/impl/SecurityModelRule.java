 ...
 */
package org.kuali.ole.sec.document.validation.impl;
...
public class SecurityModelRule extends MaintenanceDocumentRuleBase {
    ...
    protected boolean verifyModelNameIsUnique(SecurityModel securityModel, String errorKeyPrefix) {
        ...
        searchValues.put(OLEPropertyConstants.NAME, securityModel.getName());

        int matchCount = getBusinessObjectService().countMatching(SecurityModel.class, searchValues);
        if (matchCount > 0) {
            GlobalVariables.getMessageMap().putError(errorKeyPrefix + OLEPropertyConstants.NAME, SecKeyConstants.ERROR_MODEL_NAME_NON_UNIQUE, securityModel.getName());
            isValid = false;
        }
