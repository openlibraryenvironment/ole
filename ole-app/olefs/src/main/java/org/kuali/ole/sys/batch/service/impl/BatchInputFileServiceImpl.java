...
public class BatchInputFileServiceImpl implements BatchInputFileService {
     ...
     *
     * @see org.kuali.ole.sys.batch.service.BatchInputFileService#parse(org.kuali.ole.sys.batch.BatchInputFileType, byte[])
     */
    @Override
     ...
     *
     * @see org.kuali.ole.sys.batch.service.BatchInputFileService#validate(org.kuali.ole.sys.batch.BatchInputFileType, java.lang.Object)
     */
    @Override
    ...
    /**
     * @see org.kuali.ole.sys.batch.service.BatchInputFileService#save(org.kuali.rice.kim.api.identity.Person,
     *      org.kuali.ole.sys.batch.BatchInputFileType, java.lang.String, java.io.InputStream)
     */
    @Override
     ...
     */
    @Override
    public String save(Person user, BatchInputFileType batchInputFileType, String fileUserIdentifier, InputStream fileContents, Object parsedObject,String destinationFilePath,String extension) throws AuthorizationException, FileStorageException {
        if (user == null || batchInputFileType == null || fileContents == null) {
            LOG.error("an invalid(null) argument was given");
            throw new IllegalArgumentException("an invalid(null) argument was given");
        }
        ...
        if (LOG.isDebugEnabled()){
            LOG.debug("batchInputFileType.getDirectoryPath()"+batchInputFileType.getDirectoryPath());
            LOG.debug("destinationFilePath"+destinationFilePath);
       ...
       String directory = batchInputFileType.getDirectoryPath();
       String[] directoryPath = directory.toLowerCase().split("staging");
       String orderType = directoryPath[0]+destinationFilePath;
       if (LOG.isDebugEnabled()){
            LOG.debug("DirectoryPath" + directoryPath[0]);
            LOG.debug("DirectoryPath" + batchInputFileType.getDirectoryPath());
            LOG.debug("orderType" + orderType);
       }
         String  saveFileName = orderType + "/" + batchInputFileType.getFileName(user.getPrincipalName(), parsedObject, fileUserIdentifier,destinationFilePath);
                 saveFileName += "." + extension;//batchInputFileType.getFileExtension();
    ...
    /**
     * @see org.kuali.ole.sys.batch.service.BatchInputFileService#isBatchInputTypeActive(org.kuali.ole.sys.batch.BatchInputFileType)
     */
    @Override
    public boolean isBatchInputTypeActive(BatchInputFileType batchInputFileType) {
        ...
        List<String> activeInputTypes = new ArrayList<String>( SpringContext.getBean(ParameterService.class).getParameterValuesAsString(OleParameterConstants.FINANCIAL_SYSTEM_BATCH.class, SystemGroupParameterNames.ACTIVE_INPUT_TYPES_PARAMETER_NAME) );
    ...
    /**
     * @see org.kuali.ole.sys.batch.service.BatchInputFileService#listInputFileNamesWithDoneFile(org.kuali.ole.sys.batch.BatchInputFileType)
     */
    @Override
    public List<String> listInputFileNamesWithDoneFile(BatchInputFileType batchInputFileType) {
        ...
        if(LOG.isDebugEnabled()){
            LOG.debug("batchTypeDirectory ------------------------------------------->" + batchTypeDirectory);
            LOG.debug("batchInputFileType ------------------------------------------->" + batchInputFileType);
            LOG.debug("----------------Done File ----------------------->" + doneFiles);
        }
      //  LOG.info( "doneFiles.length --------------------------------------------->"+doneFiles.length);
        List<String> batchInputFiles = new ArrayList<String>();
        if(doneFiles==null) {
            return batchInputFiles;
        }
        if(doneFiles != null){
            LOG.debug("----------Done File Not Null-----------");
            for (int i = 0; i < doneFiles.length; i++) {
                ...
                if(LOG.isDebugEnabled()){
                    LOG.debug("doneFile ------------------------------------------->" + doneFile);
                    LOG.debug("DataFile being processed ------------------------------------------->" + dataFile);
                }
     ...
     *
     * @see org.kuali.ole.sys.batch.service.BatchInputFileService#isFileUserIdentifierProperlyFormatted(java.lang.String)
     */
    @Override
