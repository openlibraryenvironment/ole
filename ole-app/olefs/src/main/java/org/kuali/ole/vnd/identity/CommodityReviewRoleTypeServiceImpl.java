 ...
 */
package org.kuali.ole.vnd.identity;
...
public class CommodityReviewRoleTypeServiceImpl extends RoleTypeServiceBase {
    ...
    protected boolean performMatch(Map<String,String> qualification, Map<String,String> roleQualifier) {
        if (qualification.containsKey(OleKimAttributes.PURCHASING_COMMODITY_CODE) && qualification.get(OleKimAttributes.PURCHASING_COMMODITY_CODE).matches(roleQualifier.get(OleKimAttributes.PURCHASING_COMMODITY_CODE).replaceAll("\\*", ".*")) && KimCommonUtils.storedValueNotSpecifiedOrInputValueMatches(roleQualifier, qualification, KimConstants.AttributeConstants.CAMPUS_CODE)) {
