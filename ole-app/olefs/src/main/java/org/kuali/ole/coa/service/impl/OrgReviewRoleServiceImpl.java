...
import org.kuali.ole.coa.identity.OleKimDocDelegateMember;
import org.kuali.ole.coa.identity.OleKimDocRoleMember;
...
public class OrgReviewRoleServiceImpl implements OrgReviewRoleService {
    ...
    // note: this assumes that all use the OLE-SYS namespace
    ...
    {
        potentialParentDocumentTypeNames.add(OLEConstants.COAConstants.FINANCIAL_SYSTEM_TRANSACTIONAL_DOCUMENT);
        potentialParentDocumentTypeNames.add(OLEConstants.COAConstants.FINANCIAL_SYSTEM_COMPLEX_MAINTENANCE_DOCUMENT);
        potentialParentDocumentTypeNames.add(OLEConstants.COAConstants.FINANCIAL_SYSTEM_SIMPLE_MAINTENANCE_DOCUMENT);
    ...
    public boolean hasOrganizationHierarchy(String documentTypeName) {
        ...
        return getDocumentTypeService().hasRouteNodeForDocumentTypeName(OLEConstants.RouteLevelNames.ORGANIZATION_HIERARCHY, documentTypeName);
    }
    ...
    public boolean hasAccountingOrganizationHierarchy(String documentTypeName) {
        ...
        return getDocumentTypeService().hasRouteNodeForDocumentTypeName(OLEConstants.RouteLevelNames.ACCOUNTING_ORGANIZATION_HIERARCHY, documentTypeName);
    }
     ...
     *  2. Else, if OLE is the document type selected, set the Review Types radio group to both and leave it editable.
    ...
    public List<String> getRolesToConsider(String documentTypeName) throws ValidationException {
        ...
        if(StringUtils.isBlank(documentTypeName) || OLEConstants.ROOT_DOCUMENT_TYPE.equals(documentTypeName) ){
            rolesToConsider.add(OLEConstants.SysKimApiConstants.ORGANIZATION_REVIEWER_ROLE_NAME);
            rolesToConsider.add(OLEConstants.SysKimApiConstants.ACCOUNTING_REVIEWER_ROLE_NAME);
        } else {
            String closestParentDocumentTypeName = getClosestOrgReviewRoleParentDocumentTypeName(documentTypeName);
            if(documentTypeName.equals(OLEConstants.COAConstants.FINANCIAL_SYSTEM_TRANSACTIONAL_DOCUMENT)
                    || OLEConstants.COAConstants.FINANCIAL_SYSTEM_TRANSACTIONAL_DOCUMENT.equals(closestParentDocumentTypeName)) {
                rolesToConsider.add(OLEConstants.SysKimApiConstants.ACCOUNTING_REVIEWER_ROLE_NAME);
            } else {
                ...
                if(hasOrganizationHierarchy || documentTypeName.equals(OLEConstants.COAConstants.FINANCIAL_SYSTEM_COMPLEX_MAINTENANCE_DOCUMENT)
                        || OLEConstants.COAConstants.FINANCIAL_SYSTEM_COMPLEX_MAINTENANCE_DOCUMENT.equals(closestParentDocumentTypeName) ) {
                    rolesToConsider.add(OLEConstants.SysKimApiConstants.ORGANIZATION_REVIEWER_ROLE_NAME);
                }
    ...
    protected void saveDelegateMemberToKim( OrgReviewRole orr ) {
        ...
        RoleService roleService = KimApiServiceLocator.getRoleService();
        // Save delegation(s)
        List<OleKimDocDelegateMember> delegationMembers = getDelegationMembersToSave(orr);

        for( OleKimDocDelegateMember dm : delegationMembers ) {
            ...
            if ( StringUtils.isNotBlank(dm.getDelegationMemberId()) ) {
                ...
                if ( member != null ) {
                    ...
                    // OLEMI-9628 : fixing issue with the delegate switch from primary to secondary
    ...
    protected Role getRoleInfo( String roleName ) {
        ...
        if ( role == null ) {
            role = KimApiServiceLocator.getRoleService().getRoleByNamespaceCodeAndName( OLEConstants.SysKimApiConstants.ORGANIZATION_REVIEWER_ROLE_NAMESPACECODE, roleName);
    ...
    protected List<OleKimDocDelegateMember> getDelegationMembersToSave(OrgReviewRole orr){
        OleKimDocDelegateMember delegationMember = null;
        if(orr.isEdit() && StringUtils.isNotBlank(orr.getDelegationMemberId())){
            delegationMember = new OleKimDocDelegateMember( KimApiServiceLocator.getRoleService().getDelegationMemberById(orr.getDelegationMemberId()) );
        }

        if(delegationMember==null){
            delegationMember = new OleKimDocDelegateMember();
    ...
    protected OleKimDocRoleMember getRoleMemberToSave(Role role, OrgReviewRole orr){
        OleKimDocRoleMember roleMember = null;
        if ( orr.getPerson() != null ) {
            roleMember = new OleKimDocRoleMember(role.getId(), MemberType.PRINCIPAL, orr.getPerson().getPrincipalId());
        } else if ( orr.getGroup() != null ) {
            roleMember = new OleKimDocRoleMember(role.getId(), MemberType.GROUP, orr.getGroup().getId() );
        } else if( orr.getRole() != null ){
            roleMember = new OleKimDocRoleMember(role.getId(), MemberType.ROLE, orr.getRole().getId() );
        }
        if ( roleMember != null ) {
            if(orr.isEdit()){
    ...
    protected List<String> getRolesToSaveFor(List<String> roleNamesToConsider, String reviewRolesIndicator){
        if(roleNamesToConsider!=null){
            ...
            if(OLEConstants.COAConstants.ORG_REVIEW_ROLE_ORG_ACC_ONLY_CODE.equals(reviewRolesIndicator)){
                roleToSaveFor.add(OLEConstants.SysKimApiConstants.ACCOUNTING_REVIEWER_ROLE_NAME);
            } else if(OLEConstants.COAConstants.ORG_REVIEW_ROLE_ORG_ONLY_CODE.equals(reviewRolesIndicator)){
                roleToSaveFor.add(OLEConstants.SysKimApiConstants.ORGANIZATION_REVIEWER_ROLE_NAME);
            } else{
    ...
    protected List<OleKimDocRoleMember> getRoleMembers(OrgReviewRole orr){
        List<OleKimDocRoleMember> objectsToSave = new ArrayList<OleKimDocRoleMember>();
        ...
        for(String roleName: roleNamesToSaveFor){
            ...
            OleKimDocRoleMember roleMemberToSave = getRoleMemberToSave(roleInfo, orr);
    ...
    protected Map<String,String> getAttributes(OrgReviewRole orr, String kimTypeId){
        ...
        List<OleKimDocumentAttributeData> attributeDataList = new ArrayList<OleKimDocumentAttributeData>();
        OleKimDocumentAttributeData attributeData = getAttribute(kimTypeId, OleKimAttributes.CHART_OF_ACCOUNTS_CODE, orr.getChartOfAccountsCode());
        if ( attributeData != null ) {
            attributeDataList.add(attributeData);
        }

        attributeData = getAttribute(kimTypeId, OleKimAttributes.ORGANIZATION_CODE, orr.getOrganizationCode());
        if ( attributeData != null ) {
            attributeDataList.add(attributeData);
        }
        ...
        attributeData = getAttribute(kimTypeId, OleKimAttributes.ACCOUNTING_LINE_OVERRIDE_CODE, orr.getOverrideCode());
        if ( attributeData != null ) {
            attributeDataList.add(attributeData);
        }

        attributeData = getAttribute(kimTypeId, OleKimAttributes.FROM_AMOUNT, orr.getFromAmountStr());
        if ( attributeData != null ) {
            attributeDataList.add(attributeData);
        }

        attributeData = getAttribute(kimTypeId, OleKimAttributes.TO_AMOUNT, orr.getToAmountStr());
        if ( attributeData != null ) {
            attributeDataList.add(attributeData);
        }
...
//    }
    ...
    protected OleKimDocumentAttributeData getAttribute( String kimTypeId, String attributeName, String attributeValue ) {
        if ( StringUtils.isNotBlank(attributeValue) ) {
            ...
            if( attribute != null ){
                OleKimDocumentAttributeData attributeData = new OleKimDocumentAttributeData();
