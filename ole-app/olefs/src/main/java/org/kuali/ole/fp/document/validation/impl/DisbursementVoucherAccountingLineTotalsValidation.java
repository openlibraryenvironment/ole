...
import org.kuali.ole.sys.OleAuthorizationConstants.DisbursementVoucherEditMode;
...
public class DisbursementVoucherAccountingLineTotalsValidation extends AccountingLineGroupTotalsUnchangedValidation {
    ...
    /**
     * @see org.kuali.ole.sys.document.validation.impl.AccountingLineGroupTotalsUnchangedValidation#validate(org.kuali.ole.sys.document.validation.event.AttributedDocumentEvent)
     */
    @Override
        ...
        if (this.hasRequiredEditMode(currentEditModes, candidateEditModes)) {
            ...
            if (!this.hasRequiredEditMode(currentEditModes, foreignDraftAndWireTransferEditModes)) {
                ...
                if (persistedDocument.getDisbVchrCheckTotalAmount().isLessThan(dvDocument.getDisbVchrCheckTotalAmount())) {
                    GlobalVariables.getMessageMap().putError(OLEPropertyConstants.DOCUMENT + "." + OLEPropertyConstants.DISB_VCHR_CHECK_TOTAL_AMOUNT, OLEKeyConstants.ERROR_DV_CHECK_TOTAL_CHANGE);
                    return false;
                }
            }
