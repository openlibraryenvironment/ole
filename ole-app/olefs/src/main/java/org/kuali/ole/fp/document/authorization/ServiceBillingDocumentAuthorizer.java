...
import org.kuali.ole.sys.document.authorization.AccountingDocumentAuthorizerBase;
...
public class ServiceBillingDocumentAuthorizer extends AccountingDocumentAuthorizerBase {
     ...
     * @see org.kuali.ole.sys.document.authorization.FinancialSystemTransactionalDocumentAuthorizerBase#getDocumentActions(org.kuali.rice.kns.document.Document, org.kuali.rice.kim.api.identity.Person, java.util.Set)
     */
    @Override
    public Set<String> getDocumentActions(Document document, Person user, Set<String> documentActionsFromPresentationController) {
        ...
        boolean canCopyOrErrorCorrect = (documentActions.contains(KRADConstants.KUALI_ACTION_CAN_COPY) || documentActions.contains(OLEConstants.KFS_ACTION_CAN_ERROR_CORRECT)) ? canModifyAllSourceAccountingLines(document, user) : true;
        ...
        if (documentActions.contains(OLEConstants.KFS_ACTION_CAN_ERROR_CORRECT)) {
            if (!canCopyOrErrorCorrect) {
                documentActions.remove(OLEConstants.KFS_ACTION_CAN_ERROR_CORRECT);
            }
        }
    ...
    protected boolean canModifyAllSourceAccountingLines(Document document, Person user) {
        for (Object accountingLineAsObject : ((AccountingDocument)document).getSourceAccountingLines()) {
            if (!canModifyAccountingLine(document, ((AccountingLine)accountingLineAsObject), user)) {
                return false;
            }
        }
        return true;
    ...
    public boolean canModifyAccountingLine(Document document, AccountingLine accountingLine, Person user) {
        return isAuthorizedByTemplate(document, OLEConstants.CoreModuleNamespaces.OLE, OLEConstants.PermissionTemplate.MODIFY_ACCOUNTING_LINES.name, user.getPrincipalId(), buildPermissionDetails(document), buildRoleQualifiers(accountingLine));
    }
    ...
    protected Map<String, String> buildPermissionDetails(Document document) {
        ...
        permissionDetails.put(OleKimAttributes.ROUTE_NODE_NAME, DocumentAuthorizerBase.PRE_ROUTING_ROUTE_NAME); // route node = PreRoute
