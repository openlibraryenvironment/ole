...
import org.kuali.ole.module.purap.document.validation.AddReceivingItemRule;
import org.kuali.ole.module.purap.document.validation.ContinuePurapRule;
...
public class LineItemReceivingDocumentRule extends DocumentRuleBase implements ContinuePurapRule, AddReceivingItemRule {
    ...
    private static org.apache.log4j.Logger LOG = org.apache.log4j.Logger.getLogger(LineItemReceivingDocumentRule.class);

    @Override
    protected boolean processCustomRouteDocumentBusinessRules(Document document) {
        LOG.debug("Inside processCustomRouteDocumentBusinessRules of LineItemReceivingDocumentRule");
        boolean valid = true;
        LineItemReceivingDocument lineItemReceivingDocument = (LineItemReceivingDocument) document;
        if (LOG.isDebugEnabled())
            LOG.debug("LineItemReceivingDocument :" + lineItemReceivingDocument);
        ...
        if (LOG.isDebugEnabled())
            LOG.debug("isValid from LineItemReceivingDocumentRule :" + valid);
        ...
        for (ReceivingItem item : (List<ReceivingItem>) receivingDocument.getItems()) {
            ...
            if (item.getItemType().isQuantityBasedGeneralLedgerIndicator()) {
                ...
                if (StringUtils.isEmpty(uomCode)) {
                    valid = false;
                    String attributeLabel = SpringContext.getBean(DataDictionaryService.class).getDataDictionary().getBusinessObjectEntry(item.getClass().getName()).getAttributeDefinition(OLEPropertyConstants.ITEM_UNIT_OF_MEASURE_CODE).getLabel();
                    GlobalVariables.getMessageMap().putError(OLEPropertyConstants.ITEM_UNIT_OF_MEASURE_CODE, OLEKeyConstants.ERROR_REQUIRED, attributeLabel + item.getItemUnitOfMeasureCode());
                } else {
                    // Find out whether the unit of measure code has existed in the database
                    Map<String, String> fieldValues = new HashMap<String, String>();
                    fieldValues.put(OLEPropertyConstants.ITEM_UNIT_OF_MEASURE_CODE, item.getItemUnitOfMeasureCode());
                    if (SpringContext.getBean(BusinessObjectService.class).countMatching(UnitOfMeasure.class, fieldValues) != 1) {
                        ...
                        valid = false;
                        GlobalVariables.getMessageMap().putError(OLEPropertyConstants.ITEM_UNIT_OF_MEASURE_CODE, PurapKeyConstants.PUR_ITEM_UNIT_OF_MEASURE_CODE_INVALID, item.getItemUnitOfMeasureCode());
                    }
                }
            }
        }
    ...
    /**
     * @see org.kuali.ole.module.purap.document.validation.AddReceivingItemRule#processAddReceivingItemRules(org.kuali.ole.module.purap.document.ReceivingDocument, org.kuali.ole.module.purap.businessobject.ReceivingItem)
     */
