...
public class VendorCreditMemoItemQuantityValidation extends GenericValidation {
    ...
    public boolean validate(AttributedDocumentEvent event) {
        ...
        if (itemForValidation.getItemQuantity() != null) {
            if (itemForValidation.getItemQuantity().isNegative() && cmDocument.getInvoiceIdentifier() == null) {
            ...
            if (itemForValidation.getItemQuantity().isGreaterThan(invoicedQuantity) && cmDocument.getInvoiceIdentifier() == null) {
            ...
            KualiDecimal invoicedQuantity = getSourceTotalInvoiceQuantity(cmDocument, itemForValidation);
            if (cmDocument.getInvoiceIdentifier() == null && itemForValidation.getItemType().isQuantityBasedGeneralLedgerIndicator() && (invoicedQuantity != null && invoicedQuantity.isGreaterThan(KualiDecimal.ZERO)) && (itemForValidation.getExtendedPrice() != null && itemForValidation.getExtendedPrice().isGreaterThan(KualiDecimal.ZERO))) {
                String label = dataDictionaryService.getAttributeErrorLabel(CreditMemoItem.class, PurapPropertyConstants.QUANTITY);
                GlobalVariables.getMessageMap().putError(errorKey, OLEKeyConstants.ERROR_REQUIRED, label);
                valid = false;
            }
        }
