...
public class OrgReviewRole extends PersistableBusinessObjectBase implements MutableInactivatable {
    ...
    public static final String CACHE_NAME = OLEConstants.APPLICATION_NAMESPACE_CODE + "/" + "OrgReviewRole";
    ...
    public static final String CHART_CODE_FIELD_NAME = OLEPropertyConstants.CHART_OF_ACCOUNTS_CODE;
    public static final String ORG_CODE_FIELD_NAME = OLEPropertyConstants.ORGANIZATION_CODE;
    ...
    public static final String OVERRIDE_CODE_FIELD_NAME = OLEPropertyConstants.OVERRIDE_CODE;
    ...
    protected List<OleKimDocumentAttributeData> attributes = new ArrayList<OleKimDocumentAttributeData>();
    ...
    public Organization getOrganization() {
        if (StringUtils.isBlank(getChartOfAccountsCode()) || StringUtils.isBlank(getOrganizationCode())) {
            ...
            if (organization == null || !StringUtils.equals(getChartOfAccountsCode(), chart.getChartOfAccountsCode())
                ...
                organization = getOrganizationService().getByPrimaryIdWithCaching(getChartOfAccountsCode(),
                        getOrganizationCode());
            }
        }
        return organization;
    ...
    private void setRoleNamesAndReviewIndicator(boolean hasFinancialSystemDocumentTypeCodeChanged){
        if(hasFinancialSystemDocumentTypeCodeChanged){
            ...
            if(isBothReviewRolesIndicator()) {
                setReviewRolesIndicatorOnDocTypeChange(OLEConstants.COAConstants.ORG_REVIEW_ROLE_ORG_ACC_BOTH_CODE);
            } else if(isAccountingOrgReviewRoleIndicator()) {
                setReviewRolesIndicatorOnDocTypeChange(OLEConstants.COAConstants.ORG_REVIEW_ROLE_ORG_ACC_ONLY_CODE);
            } else if(isOrgReviewRoleIndicator()) {
                setReviewRolesIndicatorOnDocTypeChange(OLEConstants.COAConstants.ORG_REVIEW_ROLE_ORG_ONLY_CODE);
            }
        }
    }
     ...
     */
    public void setAttributes(List<OleKimDocumentAttributeData> attributes) {
        this.attributes = attributes;
    }

    public List<OleKimDocumentAttributeData> getAttributes() {
        return attributes;
    }

    public String getAttributeValue(String attributeName){
        OleKimDocumentAttributeData attributeData = getAttribute(attributeName);
    ...
    public boolean isAccountingOrgReviewRoleIndicator() {
        return getRoleNamesToConsider()!=null &&
            getRoleNamesToConsider().contains(OLEConstants.SysKimApiConstants.ACCOUNTING_REVIEWER_ROLE_NAME);
    }
    /**
    ...
    public boolean isBothReviewRolesIndicator() {
        return getRoleNamesToConsider()!=null &&
            getRoleNamesToConsider().contains(OLEConstants.SysKimApiConstants.ORGANIZATION_REVIEWER_ROLE_NAME) &&
            getRoleNamesToConsider().contains(OLEConstants.SysKimApiConstants.ACCOUNTING_REVIEWER_ROLE_NAME);
    }
    /**
    ...
    public void extractAttributesFromMap( Map<String,String> attributes ) {
        ...
        setChartOfAccountsCode(getAttributeValue(OleKimAttributes.CHART_OF_ACCOUNTS_CODE));
        setOrganizationCode(getAttributeValue(OleKimAttributes.ORGANIZATION_CODE));
        setOverrideCode(getAttributeValue(OleKimAttributes.ACCOUNTING_LINE_OVERRIDE_CODE));
        setFromAmount(getAttributeValue(OleKimAttributes.FROM_AMOUNT));
        setToAmount(getAttributeValue(OleKimAttributes.TO_AMOUNT));
    ...
    public Map<String,String> getQualifierAsAttributeSet(List<OleKimDocumentAttributeData> qualifiers) {
        ...
        for(OleKimDocumentAttributeData data: qualifiers){
    ...
    public List<OleKimDocumentAttributeData> getAttributeSetAsQualifierList( Map<String,String> qualifiers) {
        ...
        List<OleKimDocumentAttributeData> attributesList = new ArrayList<OleKimDocumentAttributeData>();
        OleKimDocumentAttributeData attribData;
        for(String key: qualifiers.keySet()){
            KimTypeAttribute attribInfo = kimTypeInfo.getAttributeDefinitionByName(key);
            attribData = new OleKimDocumentAttributeData();
