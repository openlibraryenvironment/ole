 ...
 */
public class HoldingsSearchFieldKeyValuefinder extends UifKeyValuesFinderBase implements DocstoreConstants {
        ...
        for (DocTypeConfig docTypeConfig : documentSearchConfig.getDocTypeConfigs()) {
            if (docTypeConfig.getName().equals(docType)) {
                for (DocFormatConfig docFormatConfig : docTypeConfig.getDocFormatConfigList()) {
                   if (docFormatConfig.getName().equals("oleml") && DocType.HOLDINGS.getCode().equals(docType)) {
