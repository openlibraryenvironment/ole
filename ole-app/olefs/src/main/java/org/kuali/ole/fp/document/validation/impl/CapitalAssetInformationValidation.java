...
public class CapitalAssetInformationValidation extends GenericValidation {
        ...
        for (SourceAccountingLine sourceAccount : sourceAccountLines)  {
            if (capitalAssetBuilderModuleService.hasCapitalAssetObjectSubType(sourceAccount)) {
                ...
                if (!checkSourceDistributedAccountingLineExists(sourceAccount, capitalAssets)) {
                    ...
                    GlobalVariables.getMessageMap().putErrorWithoutFullErrorPath(OLEPropertyConstants.DOCUMENT + "." + OLEConstants.SOURCE_ACCOUNTING_LINE_ERRORS + "[" + accountIndex + "]" + "." + OLEPropertyConstants.ACCOUNT_NUMBER, OLEKeyConstants.ERROR_DOCUMENT_SOURCE_ACCOUNTING_LINE_NOT_DISTRIBUTED, sourceAccount.getAccountNumber());
                    distributed = false;
                    accountIndex++;
        ...
        for (TargetAccountingLine targetAccount : targetAccountLines)  {
            ...
            if (capitalAssetBuilderModuleService.hasCapitalAssetObjectSubType(targetAccount)) {
                ...
                if (!checkTargetDistributedAccountingLineExists(targetAccount, capitalAssets)) {
                    //account does not exist so put out an error message and get out.
                    GlobalVariables.getMessageMap().putErrorWithoutFullErrorPath(OLEPropertyConstants.DOCUMENT + "." + OLEConstants.TARGET_ACCOUNTING_LINE_ERRORS + "[" + accountIndex + "]" + "." + OLEPropertyConstants.ACCOUNT_NUMBER, OLEKeyConstants.ERROR_DOCUMENT_TARGET_ACCOUNTING_LINE_NOT_DISTRIBUTED, targetAccount.getAccountNumber());
                    distributed = false;
                    accountIndex++;
    ...
    protected boolean hasValidCapitalAssetInformation(AccountingDocument accountingDocument) {
        ...
        for (CapitalAssetInformation capitalAssetInformation : capitalAssets) {
            if (ObjectUtils.isNotNull(capitalAssetInformation)) {
                ...
                String parentName = (capitalAssetInformation.getCapitalAssetActionIndicator().equalsIgnoreCase(OLEConstants.CapitalAssets.CAPITAL_ASSET_CREATE_ACTION_INDICATOR) ? OLEPropertyConstants.CAPITAL_ASSET_INFORMATION :  OLEPropertyConstants.CAPITAL_ASSET_MODIFY_INFORMATION);
                errors.addToErrorPath(parentName);
                String errorPathPrefix = OLEPropertyConstants.CAPITAL_ASSET_INFORMATION + "[" + index + "].";
    ...
    protected boolean capitalAssetsAccountLinesMatchToAccountingLines(AccountingDocument accountingDocument) {
        ...
        int index = 0;
        for (CapitalAssetInformation capitalAsset : capitalAssets) {
            String errorPathPrefix = OLEPropertyConstants.DOCUMENT + "." + OLEPropertyConstants.CAPITAL_ASSET_INFORMATION + "[" + index + "]." + OLEPropertyConstants.CAPITAL_ASSET_NUMBER;
            
            if (!checkAccountingLineExists(accountingDocument, capitalAsset, errorPathPrefix)) {
              //  MessageMap errors = GlobalVariables.getMessageMap();
              //  errors.addToErrorPath(OLEPropertyConstants.DOCUMENT);
              //  String parentName = (capitalAsset.getCapitalAssetActionIndicator().equalsIgnoreCase(OLEConstants.CapitalAssets.CAPITAL_ASSET_CREATE_ACTION_INDICATOR) ? OLEPropertyConstants.CAPITAL_ASSET_INFORMATION :  OLEPropertyConstants.CAPITAL_ASSET_MODIFY_INFORMATION);
             //   errors.addToErrorPath(parentName);
             //   String errorPathPrefix = OLEPropertyConstants.CAPITAL_ASSET_INFORMATION + "[" + index + "].";
             //   errors.addToErrorPath(errorPathPrefix);
                //account does not exist so put out an error message and get out.
             //   GlobalVariables.getMessageMap().putErrorWithoutFullErrorPath(errorPathPrefix, OLEKeyConstants.ERROR_ASSET_ACCOUNT_NUMBER_LINE_NOT_IN_SOURCE_OR_TARGET_ACCOUNTING_LINES, accountNumber);
    ...
    protected boolean checkAccountingLineExists(AccountingDocument accountingDocument, CapitalAssetInformation capitalAsset, String errorPathPrefix) {
        ...
        for (CapitalAssetAccountsGroupDetails groupAccount: groupAccounts) {
            if (!accountLineExists(accountingDocument, groupAccount)) {
                ...
                GlobalVariables.getMessageMap().putErrorWithoutFullErrorPath(errorPathPrefix, OLEKeyConstants.ERROR_ASSET_ACCOUNT_NUMBER_LINE_NOT_IN_SOURCE_OR_TARGET_ACCOUNTING_LINES, groupAccount.getAccountNumber());
                return false;
            }
        }
        ...
        for (SourceAccountingLine sourceAccount : sourceAccountLines)  {
            if (capitalAssetBuilderModuleService.hasCapitalAssetObjectSubType(sourceAccount)) {
                ...
                if (sourceAccount.getAmount().compareTo(distributedAmount) != 0) {
                    //account does not exist so put out an error message and get out.
                    GlobalVariables.getMessageMap().putErrorWithoutFullErrorPath(OLEPropertyConstants.DOCUMENT + "." + OLEConstants.SOURCE_ACCOUNTING_LINE_ERRORS + "[" + accountIndex + "]" + "." + OLEPropertyConstants.AMOUNT, OLEKeyConstants.ERROR_DOCUMENT_SOURCE_ACCOUNTING_LINE_AMOUNT_NOT_DISTRIBUTED, sourceAccount.getAccountNumber());
                    amountMatch = false;
                    accountIndex++;
        ...
        for (TargetAccountingLine targetAccount : targetAccountLines)  {
            ...
            if (capitalAssetBuilderModuleService.hasCapitalAssetObjectSubType(targetAccount)) {
                ...
                if (targetAccount.getAmount().compareTo(distributedAmount) != 0) {
                    //account does not exist so put out an error message and get out.
                    GlobalVariables.getMessageMap().putErrorWithoutFullErrorPath(OLEPropertyConstants.DOCUMENT + "." + OLEConstants.TARGET_ACCOUNTING_LINE_ERRORS + "[" + accountIndex + "]" + "." + OLEPropertyConstants.AMOUNT, OLEKeyConstants.ERROR_DOCUMENT_TARGET_ACCOUNTING_LINE_AMOUNT_NOT_DISTRIBUTED, targetAccount.getAccountNumber());
                    amountMatch = false;
                    accountIndex++;
    ...
    protected boolean checkAmount(CapitalAssetInformation capitalAsset, String errorPathPrefix) {
        ...
        if (capitalAsset.getCapitalAssetLineAmount().compareTo(distributedAccountLinesAmount) != 0) {
            ...
            GlobalVariables.getMessageMap().putErrorWithoutFullErrorPath(errorPathPrefix, OLEKeyConstants.ERROR_ASSET_LINE_AMOUNT_NOT_EQUAL_TO_DISTRIBUTED_ACCOUNTING_LINES);
            return false;
        }
