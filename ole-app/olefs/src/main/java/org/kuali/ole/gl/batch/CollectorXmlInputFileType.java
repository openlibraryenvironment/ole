...
public class CollectorXmlInputFileType extends XmlBatchInputFileTypeBase {
    ...
    public String getFileTypeIdentifer() {
        return OLEConstants.COLLECTOR_XML_FILE_TYPE_INDENTIFIER;
    }
                ...
                for (OriginEntryFull originEntry : batch.getOriginEntries()) {
                    ...
                    if (StringUtils.isEmpty(originEntry.getChartOfAccountsCode())) {
                        if (acctserv.accountsCanCrossCharts()) {
                            GlobalVariables.getMessageMap().putError(OLEConstants.GLOBAL_ERRORS, OLEKeyConstants.ERROR_BATCH_UPLOAD_FILE_EMPTY_CHART, originEntry.getAccountNumber());
                            isValid = false;
                        }
                        else {
                            ...
                            if (account != null) {
                                ...
                                GlobalVariables.getMessageMap().putError(OLEConstants.GLOBAL_ERRORS, OLEKeyConstants.ERROR_BATCH_UPLOAD_FILE_INVALID_ACCOUNT, originEntry.getAccountNumber());
                                isValid = false;
                            }
                        }
                    }
            ...
            if (batch.getCollectorDetails() != null) {
                for (CollectorDetail collectorDetail : batch.getCollectorDetails()) {
                    ...
                    if (StringUtils.isEmpty(collectorDetail.getChartOfAccountsCode())) {
                        if (acctserv.accountsCanCrossCharts()) {
                            // report error
                            GlobalVariables.getMessageMap().putError(OLEConstants.GLOBAL_ERRORS, OLEKeyConstants.ERROR_BATCH_UPLOAD_FILE_EMPTY_CHART, collectorDetail.getAccountNumber());
                            isValid = false;
                        }
                        else {
                            // accountNumber shall not be empty, otherwise won't pass schema validation
                            ...
                            if (account != null) {
                                ...
                                GlobalVariables.getMessageMap().putError(OLEConstants.GLOBAL_ERRORS, OLEKeyConstants.ERROR_BATCH_UPLOAD_FILE_INVALID_ACCOUNT, collectorDetail.getAccountNumber());
                                isValid = false;
                            }
                        }
                    }
    ...
    public String getTitleKey() {
        return OLEKeyConstants.MESSAGE_BATCH_UPLOAD_TITLE_COLLECTOR;
    }
