...
import org.kuali.ole.fp.businessobject.format.CashReceiptDepositTypeFormatter;
...
public class DepositWizardForm extends KualiForm {
    ...
    public void setDefaultBankCode() {
        Bank defaultBank = SpringContext.getBean(BankService.class).getDefaultBankByDocType(OLEConstants.FinancialDocumentTypeCodes.CASH_MANAGEMENT);
        if (defaultBank != null) {
    ...
    public boolean isDepositFinal() {
        return (depositTypeCode.equals(OLEConstants.DepositConstants.DEPOSIT_TYPE_FINAL));
    }
    ...
    public void addCashReceiptToTargetTotal(CashReceiptDocument crDoc) {
        ...
        if (crDoc.getFinancialSystemDocumentHeader().getFinancialDocumentStatusCode().equals(OLEConstants.DocumentStatusCodes.CashReceipt.VERIFIED) || crDoc.getFinancialSystemDocumentHeader().getFinancialDocumentStatusCode().equals(OLEConstants.DocumentStatusCodes.CashReceipt.FINAL)) {
    ...
    public String getNoVerifiedCashErrorMessage() {
        String errorMessage = WebUtils.getKualiConfigurationService().getPropertyValueAsString(OLEKeyConstants.CashManagement.ERROR_NO_VERIFIED_CASH);
