 ...
 */
public abstract class RequestNoticesCallable extends NoticeCallable {
    private static final Logger LOG = Logger.getLogger(RequestNoticesCallable.class);
    ...
    public RequestNoticesCallable(Map requestMap) {
        ...
        this.deliverNoticeHandler = (DeliverNoticeHandler) requestMap.get("DeliverNoticeHandler");
    }
    ...
    public Object call() throws Exception{
        ...
        try {
            ...
            if (StringUtils.isNotBlank(mailContent) && !mailContent.contains("FreeMarker template error")) {
                ...
                postProcess();
                prepareNoticeResponses(filteredDeliverNotices, true);
            } else {
                prepareNoticeResponses(filteredDeliverNotices, false);
            }
        }catch (Exception e){
            e.printStackTrace();
            prepareNoticeResponses(filteredDeliverNotices, false);
    ...
    public String getParameterValue(String key) {
        ParameterKey parameterKey = ParameterKey.create(OLEConstants.APPL_ID, OLEConstants.DLVR_NMSPC,"Deliver" , key);
        //org.kuali.ole.OLEConstants.DELIVER_COMPONENT
        Parameter parameter = CoreServiceApiServiceLocator.getParameterRepositoryService().getParameter(parameterKey);
        return parameter != null ? parameter.getValue() : null;
    }

    private void prepareNoticeResponses(List<OLEDeliverNotice> oleDeliverNotices , boolean isNoticeSent){
        oleNGBatchNoticeResponse = new OleNGBatchNoticeResponse();
        int noticeCount = oleDeliverNotices.size();
        if(isNoticeSent){
            oleNGBatchNoticeResponse.setNoOfSuccessNotice(noticeCount);
        }else{
            oleNGBatchNoticeResponse.setNoOfFailureNotice(noticeCount);
            oleNGBatchNoticeResponse.setFailureRequestAndNoticeResponses(deliverNoticeHandler.getFailureRequestAndNoticeResponse(oleDeliverNotices));
        }
    }
