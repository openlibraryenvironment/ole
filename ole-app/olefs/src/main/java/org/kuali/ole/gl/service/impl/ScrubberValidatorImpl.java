...
import org.kuali.ole.gl.batch.service.AccountingCycleCachingService;
import org.kuali.ole.gl.businessobject.OriginEntryInformation;
import org.kuali.ole.gl.service.ScrubberValidator;
import org.kuali.ole.sys.Message;
import org.kuali.ole.sys.MessageBuilder;
...
import org.kuali.ole.sys.service.OriginationCodeService;
...
public class ScrubberValidatorImpl implements ScrubberValidator {
    ...
    protected static final Collection<String> debitOrCredit = Arrays.asList( OLEConstants.GL_DEBIT_CODE, OLEConstants.GL_CREDIT_CODE );
     ...
     * @see org.kuali.module.gl.service.ScrubberValidator#validateForInquiry(org.kuali.ole.bo.GeneralLedgerPendingEntry)
     */
    @Override
    public void validateForInquiry(GeneralLedgerPendingEntry entry) {
        ...
        if ((entry.getSubAccountNumber() == null) || (!StringUtils.hasText(entry.getSubAccountNumber()))) {
            entry.setSubAccountNumber(OLEConstants.getDashSubAccountNumber());
        }
        if ((entry.getFinancialSubObjectCode() == null) || (!StringUtils.hasText(entry.getFinancialSubObjectCode()))) {
            entry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
        }
        if ((entry.getProjectCode() == null) || (!StringUtils.hasText(entry.getProjectCode()))) {
            entry.setProjectCode(OLEConstants.getDashProjectCode());
        }
    }
    ...
    protected Message validateAccount(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, UniversityDate universityRunDate, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (originEntryAccount == null) {
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_ACCOUNT_NOT_FOUND, originEntry.getChartOfAccountsCode() + "-" + originEntry.getAccountNumber(), Message.TYPE_FATAL);
        }

        if (parameterService.getParameterValueAsString(OleParameterConstants.GENERAL_LEDGER_BATCH.class, OLEConstants.SystemGroupParameterNames.GL_ANNUAL_CLOSING_DOC_TYPE).equals(originEntry.getFinancialDocumentTypeCode())) {
        ...
        if ((continuationAccountBypassOriginationCodes.contains( originEntry.getFinancialSystemOriginationCode()))
            ...
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_ORIGIN_CODE_CANNOT_HAVE_CLOSED_ACCOUNT, originEntryAccount.getChartOfAccountsCode() + "-" + originEntry.getAccountNumber(), Message.TYPE_FATAL);
        }
    ...
    protected Message continuationAccountLogic(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, UniversityDate universityRunDate, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        for (int i = 0; i < 10; ++i) {
            if (checkedAccountNumbers.contains(chartCode + accountNumber)) {
                ...
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_CIRCULAR_DEPENDENCY_IN_CONTINUATION_ACCOUNT_LOGIC, Message.TYPE_FATAL);
            }

            if ((chartCode == null) || (accountNumber == null)) {
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_CONTINUATION_ACCOUNT_NOT_FOUND, Message.TYPE_FATAL);
            }
            ...
            if (null == continuationAccount) {
                ...
                if (continuationAccount.getAccountExpirationDate() == null) {
                    ...
                    //workingEntry.setSubAccountNumber(OLEConstants.getDashSubAccountNumber());
                    //workingEntry.setTransactionLedgerEntryDescription(kualiConfigurationService.getPropertyValueAsString(OLEKeyConstants.MSG_AUTO_FORWARD) + " " + originEntry.getChartOfAccountsCode() + originEntry.getAccountNumber() + originEntry.getTransactionLedgerEntryDescription());
                    ...
                    if (!originEntryAccount.isActive()){
                        return MessageBuilder.buildMessage(OLEKeyConstants.MSG_ACCOUNT_CLOSED_TO, chartCode+accountNumber, Message.TYPE_WARNING);
                    } else {
                        return MessageBuilder.buildMessage(OLEKeyConstants.MSG_ACCOUNT_EXPIRED_TO, chartCode+accountNumber, Message.TYPE_WARNING);
                    }
        ...
        return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_CONTINUATION_ACCOUNT_LIMIT_REACHED, Message.TYPE_FATAL);
    }
    ...
    protected long getAdjustedAccountExpirationDate(Account account) {
        ...
        if (account.isForContractsAndGrants() && (account.isActive())) {
            ...
            String daysOffset = parameterService.getParameterValueAsString(ScrubberStep.class, OLEConstants.SystemGroupParameterNames.GL_SCRUBBER_VALIDATION_DAYS_OFFSET);
...
//            UniversityDate universityDate = universityDateDao.getByPrimaryKey(originEntry.getFinancialDocumentReversalDate());
            ...
            if (universityDate == null) {
                ...
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_REVERSAL_DATE_NOT_FOUND, format.format(reversalDate), Message.TYPE_FATAL);
            }
            else {
    ...
    protected Message validateSubAccount(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (StringUtils.hasText(subAccount)) {
            ...
            if (dataDictionaryService.getAttributeForceUppercase(SubAccount.class, OLEPropertyConstants.SUB_ACCOUNT_NUMBER)) {
                subAccount = originEntry.getSubAccountNumber().toUpperCase();
            }

            if (!OLEConstants.getDashSubAccountNumber().equals(subAccount)) {
              ...
              //SubAccount originEntrySubAccount = getSubAccount(originEntry);
                if (originEntrySubAccount == null) {
                    ...
                    return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_SUB_ACCOUNT_NOT_FOUND, originEntry.getChartOfAccountsCode() + "-" + originEntry.getAccountNumber() + "-" + subAccount, Message.TYPE_FATAL);
                }
                else {
                    ...
                    if (originEntrySubAccount.isActive()) {
                        ...
                        if (parameterService.getParameterValueAsString(OleParameterConstants.GENERAL_LEDGER_BATCH.class, OLEConstants.SystemGroupParameterNames.GL_ANNUAL_CLOSING_DOC_TYPE).equals(originEntry.getFinancialDocumentTypeCode())) {
                            ...
                            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_SUB_ACCOUNT_NOT_ACTIVE, originEntry.getChartOfAccountsCode() + "-" + originEntry.getAccountNumber() + "-" + subAccount, Message.TYPE_FATAL);
                        }
                    }
                }
            }
    ...
    protected Message validateProjectCode(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (StringUtils.hasText(originEntry.getProjectCode()) && !OLEConstants.getDashProjectCode().equals(originEntry.getProjectCode())) {
            ...
            if (originEntryProject == null) {
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_PROJECT_CODE_NOT_FOUND, originEntry.getProjectCode(), Message.TYPE_FATAL);
            }
            else {
                if (originEntryProject.isActive()) {
                    ...
                    return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_PROJECT_CODE_MUST_BE_ACTIVE, originEntry.getProjectCode(), Message.TYPE_FATAL);
                }
            }
        }
        else {
            workingEntry.setProjectCode(OLEConstants.getDashProjectCode());
        }
    ...
    protected Message validateFiscalYear(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, UniversityDate universityRunDate, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if ((originEntry.getUniversityFiscalYear() == null) || (originEntry.getUniversityFiscalYear().intValue() == 0)) {
            ...
            //if (!originEntry.getFinancialBalanceTypeCode().equals(OLEConstants.BALANCE_TYPE_A21)){
        ...
        if (originEntryOption == null) {
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_UNIV_FISCAL_YR_NOT_FOUND, originEntry.getUniversityFiscalYear() + "", Message.TYPE_FATAL);
        }
        return null;
    }
    ...
    protected Message validateTransactionDate(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, UniversityDate universityRunDate, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (accountingCycleCachingService.getUniversityDate(originEntry.getTransactionDate()) == null) {
            //FSKD-193, KFSMI-5441
            //return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_TRANSACTION_DATE_INVALID, originEntry.getTransactionDate().toString(), Message.TYPE_FATAL);
            originEntry.setTransactionDate(transactionDate);
            workingEntry.setTransactionDate(transactionDate);
        }
        return null;
    ...
    protected Message validateDocumentType(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if ((originEntry.getFinancialDocumentTypeCode() == null) || !accountingCycleCachingService.isCurrentActiveAccountingDocumentType(originEntry.getFinancialDocumentTypeCode())) {
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_DOCUMENT_TYPE_NOT_FOUND, originEntry.getFinancialDocumentTypeCode(), Message.TYPE_FATAL);
        }
    ...
    protected Message validateOrigination(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (StringUtils.hasText(originEntry.getFinancialSystemOriginationCode())) {
            ...
            if (originEntryOrigination == null) {
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_ORIGIN_CODE_NOT_FOUND, originEntry.getFinancialSystemOriginationCode(), Message.TYPE_FATAL);
            }
            if (!originEntryOrigination.isActive()) {
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_ORIGIN_CODE_NOT_ACTIVE, originEntry.getFinancialSystemOriginationCode(), Message.TYPE_FATAL);
            }

            workingEntry.setFinancialSystemOriginationCode(originEntry.getFinancialSystemOriginationCode());
        }
        else {
            return new Message(kualiConfigurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_ORIGIN_CODE_NOT_FOUND) + " (" + originEntry.getFinancialSystemOriginationCode() + ")", Message.TYPE_FATAL);
        }
        return null;
    }
    ...
    protected Message validateReferenceOrigination(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (StringUtils.hasText(referenceFinancialSystemOriginationCode)) {
            ...
            if (originEntryOrigination == null) {
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_REFERENCE_ORIGIN_CODE_NOT_FOUND, " (" + referenceFinancialSystemOriginationCode + ")", Message.TYPE_FATAL);
            }
            else {
    ...
    protected Message validateDocumentNumber(OriginEntryInformation originEntry, OriginEntryInformation workingEntry) {
        ...
        if (!StringUtils.hasText(originEntry.getDocumentNumber())) {
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_DOCUMENT_NUMBER_REQUIRED, Message.TYPE_FATAL);
        }
        else {
    ...
    protected Message validateChart(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (originEntryChart == null) {
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_CHART_NOT_FOUND, originEntry.getChartOfAccountsCode(), Message.TYPE_FATAL);
        }

        if (!originEntryChart.isActive()) {
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_CHART_NOT_ACTIVE, originEntry.getChartOfAccountsCode(), Message.TYPE_FATAL);
        }
    ...
    protected Message validateObjectCode(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (!StringUtils.hasText(originEntry.getFinancialObjectCode())) {
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_OBJECT_CODE_EMPTY, Message.TYPE_FATAL);
        }
        ...
        if (workingEntryFinancialObject == null) {
            ...
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_OBJECT_CODE_NOT_FOUND, objectCodeString, Message.TYPE_FATAL);
        }

        if (!workingEntryFinancialObject.isActive()) {
            String objectCodeString = workingEntry.getUniversityFiscalYear() + "-" + workingEntry.getChartOfAccountsCode() + "-" + workingEntry.getFinancialObjectCode();
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_OBJECT_CODE_NOT_ACTIVE, objectCodeString, Message.TYPE_FATAL);
        }
    ...
    protected Message validateObjectType(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (workingEntryObjectType == null) {
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_OBJECT_TYPE_NOT_FOUND, originEntry.getFinancialObjectTypeCode(), Message.TYPE_FATAL);
        }

        if (!workingEntryObjectType.isActive()) {
            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_OBJECT_TYPE_NOT_ACTIVE, originEntry.getFinancialObjectTypeCode(), Message.TYPE_FATAL);
        }
        return null;
    }
    ...
    protected Message validateSubObjectCode(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (!StringUtils.hasText(originEntry.getFinancialSubObjectCode())) {
            workingEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
            return null;
        }

        if (!OLEConstants.getDashFinancialSubObjectCode().equals(originEntry.getFinancialSubObjectCode())) {
    ...
    protected Message validateBalanceType(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (StringUtils.hasText(balanceTypeCode)) {
            ...
            if (originEntryBalanceType == null) {
                ...
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_BALANCE_TYPE_NOT_FOUND, " (" + balanceTypeCode + ")", Message.TYPE_FATAL);

            } else if (!originEntryBalanceType.isActive()) {
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_BALANCE_TYPE_NOT_ACTIVE, balanceTypeCode, Message.TYPE_FATAL);
            } else {
                ...
                if (originEntryBalanceType.isFinancialOffsetGenerationIndicator()) {
                    ...
                    if (originEntry.getTransactionLedgerEntryAmount().isNegative()) {
                        ...
                        return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_TRANS_CANNOT_BE_NEGATIVE_IF_OFFSET, Message.TYPE_FATAL);
                    }
                    else {
                        ...
                        if (!originEntry.isCredit() && !originEntry.isDebit()) { // entries requiring an offset must be either a
                            // debit or a credit
                            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_DC_INDICATOR_MUST_BE_D_OR_C, originEntry.getTransactionDebitCreditCode(), Message.TYPE_FATAL);
                        }
                        else {
                    ...
                    if (StringUtils.hasText(originEntry.getTransactionDebitCreditCode())) {
                        return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_DC_INDICATOR_MUST_BE_EMPTY, originEntry.getTransactionDebitCreditCode(), Message.TYPE_FATAL);
                    }
                    else {
                        if (originEntry.isCredit() || originEntry.isDebit()) {
                            // budget transactions must be neither debit nor credit
                            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_DC_INDICATOR_MUST_BE_NEITHER_D_NOR_C, originEntry.getTransactionDebitCreditCode(), Message.TYPE_FATAL);
                        }
                        else {
    ...
    protected Message validateUniversityFiscalPeriodCode(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, UniversityDate universityRunDate, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (!StringUtils.hasText(periodCode)) {
            if (universityRunDate.getAccountingPeriod().isOpen()) {
                ...
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_ACCOUNTING_PERIOD_CLOSED, " (year " + universityRunDate.getUniversityFiscalYear() + ", period " + universityRunDate.getUniversityFiscalAccountingPeriod(), Message.TYPE_FATAL);
            }
        }
        else {
            ...
            if (originEntryAccountingPeriod == null) {
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_ACCOUNTING_PERIOD_NOT_FOUND, periodCode, Message.TYPE_FATAL);
            }
            else if (!originEntryAccountingPeriod.isActive()) {
                return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_ACCOUNTING_PERIOD_NOT_ACTIVE, periodCode, Message.TYPE_FATAL);
            }
    ...
    protected List<Message> validateReferenceDocumentFields(OriginEntryInformation originEntry, OriginEntryInformation workingEntry, AccountingCycleCachingService accountingCycleCachingService) {
        ...
        if (numberNullIndicator) {
            ...
            if (OLEConstants.ENCUMB_UPDT_REFERENCE_DOCUMENT_CD.equals(originEntry.getTransactionEncumbranceUpdateCode())) {
                errors.add(MessageBuilder.buildMessage(OLEKeyConstants.ERROR_REF_DOC_NOT_BE_SPACE, Message.TYPE_FATAL));
            }
        }
         else {
            ...
            if (!typeCodeNullIndicator){
                if (accountingCycleCachingService.isCurrentActiveAccountingDocumentType(originEntry.getReferenceFinancialDocumentTypeCode())) {
                    ...
                    errors.add(MessageBuilder.buildMessage(OLEKeyConstants.ERROR_REFERENCE_DOCUMENT_TYPE_NOT_FOUND, originEntry.getReferenceFinancialDocumentTypeCode(), Message.TYPE_FATAL));
                }
            } else {
                errors.add(MessageBuilder.buildMessage(OLEKeyConstants.ERROR_REFERENCE_FIELDS, " " + OLEPropertyConstants.REFERENCE_FIN_DOCUMENT_TYPE_CODE  + " is missing.", Message.TYPE_FATAL));
            }
            ...
            if (!originCodeNullIndicator){
                ...
                if (oc != null) {
                    ...
                    errors.add(MessageBuilder.buildMessage(OLEKeyConstants.ERROR_REFERENCE_ORIGINATION_CODE_NOT_FOUND, " (" + originEntry.getReferenceFinancialSystemOriginationCode() + ")", Message.TYPE_FATAL));
                }
            } else {
                errors.add(MessageBuilder.buildMessage(OLEKeyConstants.ERROR_REFERENCE_FIELDS, " " + OLEPropertyConstants.REFERENCE_FINANCIAL_SYSTEM_ORIGINATION_CODE + " is missing.", Message.TYPE_FATAL));
            }
        }
        ...
        if (workingEntryBalanceType.isFinBalanceTypeEncumIndicator() && !workingEntryObjectType.isFundBalanceIndicator()) {
            if ( // OLEMI-5565 : Allow blank/null for encumbrance update code, since it is the same as "N" during processing and
                 // should not be an error
            org.apache.commons.lang.StringUtils.isBlank(originEntry.getTransactionEncumbranceUpdateCode())
                    || OLEConstants.ENCUMB_UPDT_DOCUMENT_CD.equals(originEntry.getTransactionEncumbranceUpdateCode())
                    || OLEConstants.ENCUMB_UPDT_NO_ENCUMBRANCE_CD.equals(originEntry
                            .getTransactionEncumbranceUpdateCode())
                    || OLEConstants.ENCUMB_UPDT_REFERENCE_DOCUMENT_CD.equals(originEntry
                            .getTransactionEncumbranceUpdateCode())) {
                workingEntry.setTransactionEncumbranceUpdateCode(originEntry.getTransactionEncumbranceUpdateCode());
            }
            else {
                errors.add(MessageBuilder.buildMessage(OLEKeyConstants.ERROR_ENC_UPDATE_CODE_NOT_DRN, " (" + originEntry.getTransactionEncumbranceUpdateCode() + ")", Message.TYPE_FATAL));
            }
        }
        else {
    ...
    protected Message validateDescription(OriginEntryInformation originEntry){
        ...
        if (originEntry.getTransactionLedgerEntryDescription().trim().equals(OLEConstants.EMPTY_STRING)){

            return MessageBuilder.buildMessage(OLEKeyConstants.ERROR_DESCRIPTION_CANNOT_BE_BLANK, Message.TYPE_FATAL);
        }
    ...
    /**
     * @see org.kuali.ole.gl.service.ScrubberValidator#isAccountExpired(org.kuali.ole.coa.businessobject.Account, org.kuali.ole.sys.businessobject.UniversityDate)
     */
    @Override
