...
public class BatchJobStatusLookupableHelperServiceImpl extends KualiLookupableHelperServiceImpl {
    ...
    public List<? extends BusinessObject> getSearchResults(Map<String, String> fieldValues) {
        super.setBackLocation(fieldValues.get(OLEConstants.BACK_LOCATION));
        super.setDocFormKey(fieldValues.get(OLEConstants.DOC_FORM_KEY));
    ...
    public boolean doesModuleServiceHaveJobStatus(BatchJobStatus job){
        if(job!=null) {
            OleModuleServiceImpl moduleService = (OleModuleServiceImpl)getKualiModuleService().getResponsibleModuleServiceForJob(job.getName());
    ...
    public List<HtmlData> getCustomActionUrls(BusinessObject businessObject, List pkNames) {
        if (businessObject instanceof BatchJobStatus) {
            ...
            if ( !SpringContext.getBean(IdentityManagementService.class).hasPermissionByTemplateName(
                    ...
                    OLEConstants.PermissionTemplate.MODIFY_BATCH_JOB.name,
                    permissionDetails ) ) {
                linkText = "View";
            }
            String href = configurationService.getPropertyValueAsString(OLEConstants.APPLICATION_URL_KEY) + "/batchModify.do?methodToCall=start&name="+(UrlFactory.encode(job.getName()))+("&group=")+(UrlFactory.encode(job.getGroup()));
            List<HtmlData> anchorHtmlDataList = new ArrayList<HtmlData>();
            AnchorHtmlData anchorHtmlData = new AnchorHtmlData(href, OLEConstants.START_METHOD, linkText);
