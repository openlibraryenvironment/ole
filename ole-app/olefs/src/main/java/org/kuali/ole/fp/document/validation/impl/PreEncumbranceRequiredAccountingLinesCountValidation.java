...
public class PreEncumbranceRequiredAccountingLinesCountValidation extends GenericValidation {
    ...
    public boolean validate(AttributedDocumentEvent event) {
        ...
        if (sourceAccountingLineList.size() == 0 && targetAccountingLineList.size() == 0 ) {
            GlobalVariables.getMessageMap().putError("sourceAccountingLines", OLEKeyConstants.ERROR_DOCUMENT_PRE_ENCUMBRANCE_SINGLE_SECTION_NO_ACCOUNTING_LINES, new String[] { null });
            return false;
        }
