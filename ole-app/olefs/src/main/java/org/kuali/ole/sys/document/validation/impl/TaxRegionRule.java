...
import org.kuali.ole.sys.businessobject.TaxRegionCounty;
import org.kuali.ole.sys.businessobject.TaxRegionPostalCode;
import org.kuali.ole.sys.businessobject.TaxRegionRate;
import org.kuali.ole.sys.businessobject.TaxRegionState;
...
public class TaxRegionRule extends KfsMaintenanceDocumentRuleBase {
    ...
    public boolean processCustomAddCollectionLineBusinessRules(MaintenanceDocument document, String collectionName, PersistableBusinessObject bo) {
        ...
        if (OLEConstants.TaxRegionConstants.TAX_REGION_RATES.equals(collectionName)) {
            success &= isValidTaxRegionRate((TaxRegionRate) bo, null);
        }
        else if (OLEConstants.TaxRegionConstants.TAX_REGION_STATES.equals(collectionName)) {
            success &= isValidTaxRegionState((TaxRegionState) bo);
        }
        else if (OLEConstants.TaxRegionConstants.TAX_REGION_COUNTIES.equals(collectionName)) {
            success &= isValidTaxRegionCounty((TaxRegionCounty) bo);
        }
        else if (OLEConstants.TaxRegionConstants.TAX_REGION_POSTAL_CODES.equals(collectionName)) {
    ...
    protected boolean isValidEffectiveDate(TaxRegionRate taxRegionRate) {
        ...
        if (taxRegionRate.getEffectiveDate() != null) {
            ...
            if (comparison == 0 || comparison < 0) {
                GlobalVariables.getMessageMap().putError(OLEConstants.TaxRegionConstants.TAX_REGION_EFFECTIVE_DATE, OLEKeyConstants.ERROR_DOCUMENT_TAX_REGION_CANT_ADD_PAST_OR_CURRENT_DATE_FOR_TAX_DISTRICT);
                success = false;
            }
        }
        return success;
    ...
    protected boolean isValidTaxRate(TaxRegionRate taxRegionRate) {
        ...
        if (taxRegionRate.getTaxRate() != null) {
            if (taxRegionRate.getTaxRate().intValue() > 1 || taxRegionRate.getTaxRate().intValue() < 0) {
                GlobalVariables.getMessageMap().putError(OLEConstants.TaxRegionConstants.TAX_REGION_TAX_RATE, OLEKeyConstants.ERROR_DOCUMENT_TAX_REGION_TAX_RATE_BETWEEN0AND1);
                success = false;
            }
        }
    ...
    protected boolean isValidTaxRegionCounty(TaxRegionCounty taxRegionCounty) {
        ...
        if (ObjectUtils.isNull(county) || !county.isActive()) {
            GlobalVariables.getMessageMap().putError(OLEConstants.TaxRegionConstants.TAX_REGION_COUNTY_CODE, OLEKeyConstants.ERROR_DOCUMENT_TAX_REGION_INVALID_COUNTY, new String[] { taxRegionCounty.getCountyCode(), taxRegionCounty.getStateCode() });
            success = false;
        }
        }
    ...
    protected boolean isValidTaxRegionPostalCode(TaxRegionPostalCode taxRegionPostalCode) {
        ...
        if (ObjectUtils.isNull(postalZipCode) || !postalZipCode.isActive()) {
            GlobalVariables.getMessageMap().putError(OLEConstants.TaxRegionConstants.TAX_REGION_POSTAL_CODE, OLEKeyConstants.ERROR_DOCUMENT_TAX_REGION_INVALID_POSTAL_CODE, taxRegionPostalCode.getPostalCode());
            success = false;
        }
        }
