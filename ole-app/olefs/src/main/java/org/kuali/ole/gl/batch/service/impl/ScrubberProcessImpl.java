...
import org.kuali.ole.gl.batch.BatchSortUtil;
import org.kuali.ole.gl.batch.CollectorBatch;
import org.kuali.ole.gl.batch.DemergerSortComparator;
import org.kuali.ole.gl.batch.ScrubberSortComparator;
...
import org.kuali.ole.gl.batch.service.ScrubberProcess;
import org.kuali.ole.gl.batch.service.impl.FilteringOriginEntryFileIterator.OriginEntryFilter;
import org.kuali.ole.gl.businessobject.DemergerReportData;
import org.kuali.ole.gl.businessobject.OriginEntryFieldUtil;
...
import org.kuali.ole.gl.report.PreScrubberReport;
import org.kuali.ole.gl.report.PreScrubberReportData;
import org.kuali.ole.gl.report.TransactionListingReport;
import org.kuali.ole.gl.service.PreScrubberService;
import org.kuali.ole.gl.service.ScrubberReportData;
import org.kuali.ole.gl.service.ScrubberValidator;
import org.kuali.ole.gl.service.impl.ScrubberStatus;
import org.kuali.ole.gl.service.impl.StringHelper;
import org.kuali.ole.sys.OLEConstants;
import org.kuali.ole.sys.OLEKeyConstants;
import org.kuali.ole.sys.OLEParameterKeyConstants.GlParameterConstants;
...
import org.kuali.ole.sys.exception.InvalidFlexibleOffsetException;
import org.kuali.ole.sys.service.DocumentNumberAwareReportWriterService;
...
public class ScrubberProcessImpl implements ScrubberProcess {
    ...
    public void scrubEntries(boolean reportOnlyMode, String documentNumber) {
        ...
        if (universityRunDate == null) {
            throw new IllegalStateException(configurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_UNIV_DATE_NOT_FOUND));
        }
            ...
            while (currentValidLine != null || currentErrorLine != null) {
                ...
                try{
                   ...
                   if (org.apache.commons.lang.StringUtils.isEmpty(currentValidLine)) {
                       String errorDesc = currentErrorLine.substring(pMap.get(OLEPropertyConstants.TRANSACTION_LEDGER_ENTRY_DESC), pMap.get(OLEPropertyConstants.TRANSACTION_LEDGER_ENTRY_AMOUNT));
                       String errorFinancialBalanceTypeCode = currentErrorLine.substring(pMap.get(OLEPropertyConstants.FINANCIAL_BALANCE_TYPE_CODE), pMap.get(OLEPropertyConstants.FINANCIAL_OBJECT_TYPE_CODE));
                   ...
                   String financialBalanceTypeCode = currentValidLine.substring(pMap.get(OLEPropertyConstants.FINANCIAL_BALANCE_TYPE_CODE), pMap.get(OLEPropertyConstants.FINANCIAL_OBJECT_TYPE_CODE));
                   String desc = currentValidLine.substring(pMap.get(OLEPropertyConstants.TRANSACTION_LEDGER_ENTRY_DESC), pMap.get(OLEPropertyConstants.TRANSACTION_LEDGER_ENTRY_AMOUNT));
                   ...
                   String compareStringFromValidEntry = currentValidLine.substring(pMap.get(OLEPropertyConstants.FINANCIAL_DOCUMENT_TYPE_CODE), pMap.get(OLEPropertyConstants.TRANSACTION_ENTRY_SEQUENCE_NUMBER)); 
                   String compareStringFromErrorEntry = currentErrorLine.substring(pMap.get(OLEPropertyConstants.FINANCIAL_DOCUMENT_TYPE_CODE), pMap.get(OLEPropertyConstants.TRANSACTION_ENTRY_SEQUENCE_NUMBER));
                ...
                if (!org.apache.commons.lang.StringUtils.isEmpty(GLEN_RECORD) && !org.apache.commons.lang.StringUtils.isBlank(GLEN_RECORD.trim())) {
                    ...
                    if (!isFatal(transactionErrors)) {
                        ...
                        if (!collectorMode) {
                            ...
                            // subacct
                            // number is dashes
                            if (!OLEConstants.getDashSubAccountNumber().equals(scrubbedEntry.getSubAccountNumber())) {
                            ...
                            if (costShareObjectTypeCodes.evaluationSucceeds() && costShareEncBalanceTypeCodes.evaluationSucceeds() && scrubbedEntryAccount.isForContractsAndGrants() && OLEConstants.SubAccountType.COST_SHARE.equals(subAccountTypeCode) && costShareEncFiscalPeriodCodes.evaluationSucceeds() && costShareEncDocTypeCodes.evaluationSucceeds()) {
    ...
    protected TransactionError generateCostShareEntries(OriginEntryInformation scrubbedEntry, ScrubberReportData scrubberReport) {
        ...
        try {
            ...
            costShareEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
            ...
            if (scrubCostShareAmount.isPositive()) {
                costShareEntry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
            }
            else {
                costShareEntry.setTransactionDebitCreditCode(OLEConstants.GL_CREDIT_CODE);
            ...
            costShareEntry.setProjectCode(OLEConstants.getDashProjectCode());
            ...
            OffsetDefinition offsetDefinition = accountingCycleCachingService.getOffsetDefinition(scrubbedEntry.getUniversityFiscalYear(), scrubbedEntry.getChartOfAccountsCode(), OLEConstants.TRANSFER_FUNDS, scrubbedEntry.getFinancialBalanceTypeCode());
            if (offsetDefinition != null) {
                if (offsetDefinition.getFinancialObject() == null) {
                    ...
                    Message m = new Message(configurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_OFFSET_DEFINITION_OBJECT_CODE_NOT_FOUND) + " (" + objectCodeKey.toString() + ")", Message.TYPE_FATAL);
                ...
                costShareOffsetEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
            }
            else {
                ...
                Message m = new Message(configurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_OFFSET_DEFINITION_NOT_FOUND) + " (" + offsetKey.toString() + ")", Message.TYPE_FATAL);
            ...
            if (costShareEntry.isCredit()) {
                costShareOffsetEntry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
            }
            else {
                costShareOffsetEntry.setTransactionDebitCreditCode(OLEConstants.GL_CREDIT_CODE);
            }
            ...
            if (StringHelper.isNullOrEmpty(costShareSourceAccountEntry.getSubAccountNumber())) {
                costShareSourceAccountEntry.setSubAccountNumber(OLEConstants.getDashSubAccountNumber());
            }

            costShareSourceAccountEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
            ...
            if (scrubCostShareAmount.isPositive()) {
                costShareSourceAccountEntry.setTransactionDebitCreditCode(OLEConstants.GL_CREDIT_CODE);
            }
            else {
                costShareSourceAccountEntry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
            ...
            costShareSourceAccountEntry.setProjectCode(OLEConstants.getDashProjectCode());
            ...
            offsetDefinition = accountingCycleCachingService.getOffsetDefinition(scrubbedEntry.getUniversityFiscalYear(), scrubbedEntry.getChartOfAccountsCode(), OLEConstants.TRANSFER_FUNDS, scrubbedEntry.getFinancialBalanceTypeCode());
            if (offsetDefinition != null) {
                if (offsetDefinition.getFinancialObject() == null) {
                    Map<Transaction, List<Message>> errors = new HashMap<Transaction, List<Message>>();
                ...
                costShareSourceAccountOffsetEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
            }
            else {
                Map<Transaction, List<Message>> errors = new HashMap<Transaction, List<Message>>();
            ...
            if (scrubbedEntry.isCredit()) {
                costShareSourceAccountOffsetEntry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
            }
            else {
                costShareSourceAccountOffsetEntry.setTransactionDebitCreditCode(OLEConstants.GL_CREDIT_CODE);
            }
        ...
        try {
         // Lines 4694 - 4798 of the Pro Cobol listing on Confluence
            ...
            if (scrubbedEntry.getFinancialBalanceTypeCode().equals(scrubbedEntryOption.getActualFinancialBalanceTypeCd()) && scrubbedEntry.getUniversityFiscalYear().intValue() > 1995 && (documentTypeCodes != null && documentTypeCodes.evaluationSucceeds()) && (fiscalPeriodCodes != null && fiscalPeriodCodes.evaluationSucceeds()) && (objectSubTypeCodes != null && objectSubTypeCodes.evaluationSucceeds()) && (subFundGroupCodes != null && subFundGroupCodes.evaluationSucceeds()) && (chartCodes != null && chartCodes.evaluationSucceeds())) {
                ...
                capitalizationEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
    ...
    protected String processPlantIndebtedness(OriginEntryInformation scrubbedEntry, ScrubberReportData scrubberReport) {
        try{
            ...
            if (scrubbedEntry.getFinancialBalanceTypeCode().equals(scrubbedEntryOption.getActualFinancialBalanceTypeCd()) && (subFundGroupCodes != null && subFundGroupCodes.evaluationSucceeds()) && (objectSubTypeCodes != null && objectSubTypeCodes.evaluationSucceeds())) {
                ...
                plantIndebtednessEntry.setTransactionLedgerEntryDescription(OLEConstants.PLANT_INDEBTEDNESS_ENTRY_DESCRIPTION);
                ...
                plantIndebtednessEntry.setTransactionScrubberOffsetGenerationIndicator(true);
                plantIndebtednessEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
                ...
                plantIndebtednessEntry.setSubAccountNumber(OLEConstants.getDashSubAccountNumber());
                plantIndebtednessEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
        ...
        try{
            ...
            if (scrubbedEntry.getFinancialBalanceTypeCode().equals(scrubbedEntryOption.getActualFinancialBalanceTypeCd()) && scrubbedEntry.getUniversityFiscalYear().intValue() > 1995 && (docTypeCodes != null && docTypeCodes.evaluationSucceeds()) && (fiscalPeriods != null && fiscalPeriods.evaluationSucceeds()) && (objSubTypeCodes != null && objSubTypeCodes.evaluationSucceeds()) && (subFundGroupCodes != null && subFundGroupCodes.evaluationSucceeds()) && (chartCodes != null && chartCodes.evaluationSucceeds())) {
                ...
                if (liabilityEntry.isDebit()) {
                    liabilityEntry.setTransactionDebitCreditCode(OLEConstants.GL_CREDIT_CODE);
                }
                else {
                    liabilityEntry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
                }
    ...
    {
        if (scrubbedEntry.isDebit()) {
            fullEntry.setTransactionDebitCreditCode(OLEConstants.GL_CREDIT_CODE);
        }
        else {
            fullEntry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
        }
    }
    ...
    protected void plantFundAccountLookup(OriginEntryInformation scrubbedEntry, OriginEntryFull liabilityEntry) {
        ...
        liabilityEntry.setSubAccountNumber(OLEConstants.getDashSubAccountNumber());
        ObjectCode scrubbedEntryObjectCode = accountingCycleCachingService.getObjectCode(scrubbedEntry.getUniversityFiscalYear(), scrubbedEntry.getChartOfAccountsCode(), scrubbedEntry.getFinancialObjectCode());
        Account scrubbedEntryAccount = accountingCycleCachingService.getAccount(scrubbedEntry.getChartOfAccountsCode(), scrubbedEntry.getAccountNumber());
        scrubbedEntryAccount.setOrganization(accountingCycleCachingService.getOrganization(scrubbedEntryAccount.getChartOfAccountsCode(), scrubbedEntryAccount.getOrganizationCode()));
    ...
    protected TransactionError generateCostShareEncumbranceEntries(OriginEntryInformation scrubbedEntry, ScrubberReportData scrubberReport) {
        try{
            ...
            if (!StringUtils.hasText(costShareEncumbranceEntry.getSubAccountNumber())) {
                costShareEncumbranceEntry.setSubAccountNumber(OLEConstants.getDashSubAccountNumber());
            }
            ...
            costShareEncumbranceEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
            ...
            if (!StringUtils.hasText(scrubbedEntry.getTransactionDebitCreditCode())) {
                if (scrubbedEntry.getTransactionLedgerEntryAmount().isPositive()) {
                    costShareEncumbranceEntry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
                }
                else {
                    costShareEncumbranceEntry.setTransactionDebitCreditCode(OLEConstants.GL_CREDIT_CODE);
            ...
            if (offset != null) {
                if (offset.getFinancialObject() == null) {
                    ...
                    return new TransactionError(costShareEncumbranceEntry, new Message(configurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_NO_OBJECT_FOR_OBJECT_ON_OFSD) + "(" + offsetKey.toString() + ")", Message.TYPE_FATAL));
                }
                ...
                costShareEncumbranceOffsetEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
            }
            else {
                ...
                return new TransactionError(costShareEncumbranceEntry, new Message(configurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_OFFSET_DEFINITION_NOT_FOUND) + "(" + offsetKey.toString() + ")", Message.TYPE_FATAL));
            }
            ...
            if (costShareEncumbranceEntry.isCredit()) {
                costShareEncumbranceOffsetEntry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
            }
            else {
                costShareEncumbranceOffsetEntry.setTransactionDebitCreditCode(OLEConstants.GL_CREDIT_CODE);
            }
            ...
            costShareEncumbranceOffsetEntry.setProjectCode(OLEConstants.getDashProjectCode());
    ...
    public void setCostShareObjectCode(OriginEntryFull costShareEntry, OriginEntryInformation originEntry) {
        ...
        if (originEntryFinancialObject == null) {
            addTransactionError(configurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_OBJECT_CODE_NOT_FOUND), originEntry.getFinancialObjectCode(), Message.TYPE_FATAL);
        }
        ...
        if (objectCode != null) {
            ...
            addTransactionError(configurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_COST_SHARE_OBJECT_NOT_FOUND), costShareEntry.getFinancialObjectCode(), Message.TYPE_FATAL);
        }
    }
         ...
         // This code is 3000-OFFSET to SET-OBJECT-2004 in the Cobol
            ...
            if (offsetDefinition != null) {
                if (offsetDefinition.getFinancialObject() == null) {
                    ...
                    putTransactionError(offsetEntry, configurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_OFFSET_DEFINITION_OBJECT_CODE_NOT_FOUND), offsetKey.toString(), Message.TYPE_FATAL);
                ...
                offsetEntry.setFinancialSubObjectCode(OLEConstants.getDashFinancialSubObjectCode());
            }
            else {
                ...
                putTransactionError(offsetEntry, configurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_OFFSET_DEFINITION_NOT_FOUND), sb.toString(), Message.TYPE_FATAL);
            ...
            if (unitOfWork.offsetAmount.isPositive()) {
                ...
                offsetEntry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
            ...
            offsetEntry.setProjectCode(OLEConstants.getDashProjectCode());
    ...
    protected String checkAndSetTransactionTypeCostShare (String financialBalanceTypeCode, String desc, String currentValidLine){
        ...
        if (TRANSACTION_TYPE_COST_SHARE.equals(transactionType)) {
            ...
            transaction.setFinancialDocumentTypeCode(OLEConstants.TRANSFER_FUNDS);
            transaction.setFinancialSystemOriginationCode(OLEConstants.SubAccountType.COST_SHARE);
    ...
    protected void generateScrubberBlankBalanceTypeCodeReport(String inputFileName) {
        OriginEntryFilter blankBalanceTypeFilter = new OriginEntryFilter() {
            /**
             * @see org.kuali.ole.gl.batch.service.impl.FilteringOriginEntryFileIterator.OriginEntryFilter#accept(org.kuali.ole.gl.businessobject.OriginEntryFull)
             */
            @Override
