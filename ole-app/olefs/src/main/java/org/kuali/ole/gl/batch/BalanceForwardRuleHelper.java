...
import org.kuali.ole.gl.batch.service.impl.exception.NonFatalErrorException;
...
public class BalanceForwardRuleHelper {
    ...
    public BalanceForwardRuleHelper() {
        ...
        annualClosingDocType = parameterService.getParameterValueAsString(OleParameterConstants.GENERAL_LEDGER_BATCH.class, OLEConstants.SystemGroupParameterNames.GL_ANNUAL_CLOSING_DOC_TYPE);
        glOriginationCode = parameterService.getParameterValueAsString(OleParameterConstants.GENERAL_LEDGER_BATCH.class, OLEConstants.SystemGroupParameterNames.GL_ORIGINATION_CODE);
    ...
    public OriginEntryFull generateCumulativeForwardOriginEntry(Balance balance) {
        ...
        activeEntry.setUniversityFiscalPeriodCode(OLEConstants.PERIOD_CODE_CG_BEGINNING_BALANCE);
        ...
        activeEntry.setDocumentNumber(new StringBuffer(OLEConstants.BALANCE_TYPE_ACTUAL).append(balance.getAccountNumber()).toString());
        ...
        if (OLEConstants.BALANCE_TYPE_CURRENT_BUDGET.equals(balance.getBalanceTypeCode()) 
                || OLEConstants.BALANCE_TYPE_BASE_BUDGET.equals(balance.getBalanceTypeCode())  ) {
            ...
            try {
                ...
                wsFinancialObjectTypeDebitCreditCode = OLEConstants.GL_CREDIT_CODE;
                LOG.info(nfee.getMessage());
            }
            if (activeEntry.getTransactionLedgerEntryAmount().isNegative()) {
                if (OLEConstants.GL_CREDIT_CODE.equals(wsFinancialObjectTypeDebitCreditCode)) {
                    activeEntry.setTransactionDebitCreditCode(OLEConstants.GL_DEBIT_CODE);
                }
                else {
                    activeEntry.setTransactionDebitCreditCode(OLEConstants.GL_CREDIT_CODE);
                }
            }
            else {
        ...
        activeEntry.setProjectCode(OLEConstants.getDashProjectCode());
        ...
        if (OLEConstants.BALANCE_TYPE_AUDIT_TRAIL.equals(balance.getBalanceTypeCode())) {
            activeEntry.setFinancialBalanceTypeCode(OLEConstants.BALANCE_TYPE_ACTUAL);
        }
        if (activeEntry.getTransactionLedgerEntryAmount().isNegative()) {
            if (OLEConstants.BALANCE_TYPE_ACTUAL.equals(activeEntry.getFinancialBalanceTypeCode())) {
    ...
    public OriginEntryFull generateGeneralForwardOriginEntry(Balance balance) {
        ...
        entry.setDocumentNumber(new StringBuffer(OLEConstants.BALANCE_TYPE_ACTUAL).append(balance.getAccountNumber())/* .append(balance.getChartOfAccountsCode()) */.toString());
        ...
        if (transactionLedgerEntryAmount.isNegative()) {
            if (OLEConstants.GL_DEBIT_CODE.equals(wsFinancialObjectTypeDebitCreditCode)) {
        ...
        if (OLEConstants.BALANCE_TYPE_AUDIT_TRAIL.equals(entry.getFinancialBalanceTypeCode())) {
            entry.setFinancialBalanceTypeCode(OLEConstants.BALANCE_TYPE_ACTUAL);
        }
        if (transactionLedgerEntryAmount.isNegative()) {
            if (OLEConstants.BALANCE_TYPE_ACTUAL.equals(entry.getFinancialBalanceTypeCode())) {
    ...
    private String getTransactionEncumbranceUpdateCode(Balance balance) throws NonFatalErrorException {
        ...
        if (encumIndicator == null) {
            ...
            updateCode = OLEConstants.ENCUMB_UPDT_NO_ENCUMBRANCE_CD;
        }
    ...
    private String getFinancialObjectTypeDebitCreditCode(Balance balance) throws NonFatalErrorException {
        ...
        if (null != balanceObjectTypeDebitCreditCode) {
            if (ObjectHelper.isOneOf(balanceObjectTypeDebitCreditCode, new String[] { OLEConstants.GL_CREDIT_CODE, OLEConstants.GL_DEBIT_CODE })) {
