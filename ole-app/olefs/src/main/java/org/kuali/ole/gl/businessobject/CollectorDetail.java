...
public class CollectorDetail extends PersistableBusinessObjectBase {
    ...
    protected LinkedHashMap toStringMapper_RICE20_REFACTORME() {
        ...
        m.put(OLEPropertyConstants.DOCUMENT_NUMBER, getDocumentNumber());
        return m;
    }
    ...
    public void setFromFileForCollectorDetail(String detailLine, Map<String, String>accountRecordBalanceTypeMap, Date curDate, UniversityDate universityDate, int lineNumber, MessageMap messageMap) {
        ...
        try{
            ...
            if (!GeneralLedgerConstants.getSpaceUniversityFiscalYear().equals(detailLine.substring(pMap.get(OLEPropertyConstants.UNIVERSITY_FISCAL_YEAR), pMap.get(OLEPropertyConstants.CHART_OF_ACCOUNTS_CODE)))) {
                try {
                    setUniversityFiscalYear(new Integer(getValue(detailLine, pMap.get(OLEPropertyConstants.UNIVERSITY_FISCAL_YEAR), pMap.get(OLEPropertyConstants.CHART_OF_ACCOUNTS_CODE))));
                }
                catch (NumberFormatException e) {
                    messageMap.putError(OLEConstants.GLOBAL_ERRORS, OLEKeyConstants.ERROR_CUSTOM, "Collector detail university fiscal year " + lineNumber + " string " + detailLine.substring(pMap.get(OLEPropertyConstants.UNIVERSITY_FISCAL_YEAR), pMap.get(OLEPropertyConstants.CHART_OF_ACCOUNTS_CODE)));
            ...
            if (!GeneralLedgerConstants.getSpaceChartOfAccountsCode().equals(detailLine.substring(pMap.get(OLEPropertyConstants.UNIVERSITY_FISCAL_YEAR), pMap.get(OLEPropertyConstants.CHART_OF_ACCOUNTS_CODE))))
                setChartOfAccountsCode(getValue(detailLine, pMap.get(OLEPropertyConstants.CHART_OF_ACCOUNTS_CODE), pMap.get(OLEPropertyConstants.ACCOUNT_NUMBER)));
            else
                setChartOfAccountsCode(GeneralLedgerConstants.getSpaceChartOfAccountsCode());            
            setAccountNumber(getValue(detailLine, pMap.get(OLEPropertyConstants.ACCOUNT_NUMBER), pMap.get(OLEPropertyConstants.SUB_ACCOUNT_NUMBER)));
            ...
            setSubAccountNumber(getValue(detailLine, pMap.get(OLEPropertyConstants.SUB_ACCOUNT_NUMBER), pMap.get(OLEPropertyConstants.FINANCIAL_OBJECT_CODE)));
            setFinancialObjectCode(getValue(detailLine, pMap.get(OLEPropertyConstants.FINANCIAL_OBJECT_CODE), pMap.get(OLEPropertyConstants.FINANCIAL_SUB_OBJECT_CODE)));
            setFinancialSubObjectCode(getValue(detailLine, pMap.get(OLEPropertyConstants.FINANCIAL_SUB_OBJECT_CODE), pMap.get(OLEPropertyConstants.FINANCIAL_BALANCE_TYPE_CODE)));
            ...
            setFinancialObjectTypeCode(getValue(detailLine, pMap.get(OLEPropertyConstants.FINANCIAL_OBJECT_TYPE_CODE), pMap.get(OLEPropertyConstants.COLLECTOR_DETAIL_SEQUENCE_NUMBER)));
            setUniversityFiscalPeriodCode(universityDate.getUniversityFiscalAccountingPeriod());
            setCollectorDetailSequenceNumber(getValue(detailLine, pMap.get(OLEPropertyConstants.COLLECTOR_DETAIL_SEQUENCE_NUMBER), pMap.get(OLEPropertyConstants.FINANCIAL_DOCUMENT_TYPE_CODE)));
            setFinancialDocumentTypeCode(getValue(detailLine, pMap.get(OLEPropertyConstants.FINANCIAL_DOCUMENT_TYPE_CODE), pMap.get(OLEPropertyConstants.FINANCIAL_SYSTEM_ORIGINATION_CODE)));
            setFinancialSystemOriginationCode(getValue(detailLine, pMap.get(OLEPropertyConstants.FINANCIAL_SYSTEM_ORIGINATION_CODE), pMap.get(OLEPropertyConstants.DOCUMENT_NUMBER)));
            setDocumentNumber(getValue(detailLine, pMap.get(OLEPropertyConstants.DOCUMENT_NUMBER), pMap.get(OLEPropertyConstants.COLLECTOR_DETAIL_AMOUNT)));
            try {
                setCollectorDetailItemAmount(getValue(detailLine, pMap.get(OLEPropertyConstants.COLLECTOR_DETAIL_AMOUNT), pMap.get(OLEPropertyConstants.COLLECTOR_DETAIL_GL_CREDIT_CODE)));
            }
            catch (NumberFormatException e) {
                setCollectorDetailItemAmount(KualiDecimal.ZERO);
                messageMap.putError(OLEConstants.GLOBAL_ERRORS, OLEKeyConstants.ERROR_CUSTOM, "Collector detail amount cannot be parsed on line " + lineNumber + " amount string " + detailLine.substring(pMap.get(OLEPropertyConstants.COLLECTOR_DETAIL_AMOUNT), pMap.get(OLEPropertyConstants.GL_CREDIT_CODE)));
            }
            if (OLEConstants.GL_CREDIT_CODE.equalsIgnoreCase(getValue(detailLine, pMap.get(OLEPropertyConstants.COLLECTOR_DETAIL_GL_CREDIT_CODE), pMap.get(OLEPropertyConstants.COLLECTOR_DETAIL_NOTE_TEXT)))) {
                setCollectorDetailItemAmount(getCollectorDetailItemAmount().negated());
            }
            setCollectorDetailNoteText(getValue(detailLine, pMap.get(OLEPropertyConstants.COLLECTOR_DETAIL_NOTE_TEXT), GeneralLedgerConstants.getSpaceAllCollectorDetailFields().length()));
