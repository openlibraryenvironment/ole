 ...
 */
package org.kuali.ole.fp.businessobject.format;
...
public class CashDrawerStatusCodeFormatter extends Formatter {
    ...
    public CashDrawerStatusCodeFormatter() {
        CLOSED_CD = OLEConstants.CashDrawerConstants.STATUS_CLOSED;
        CLOSED_MSG = SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(OLEKeyConstants.CashDrawer.CASH_DRAWER_STATUS_CLOSED);

        OPEN_CD = OLEConstants.CashDrawerConstants.STATUS_OPEN;
        OPEN_MSG = SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(OLEKeyConstants.CashDrawer.CASH_DRAWER_STATUS_OPEN);

        LOCKED_CD = OLEConstants.CashDrawerConstants.STATUS_LOCKED;
        LOCKED_MSG = SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(OLEKeyConstants.CashDrawer.CASH_DRAWER_STATUS_LOCKED);
    }
