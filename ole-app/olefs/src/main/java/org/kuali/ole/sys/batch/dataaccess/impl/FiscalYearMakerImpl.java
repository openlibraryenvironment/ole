...
public class FiscalYearMakerImpl extends PlatformAwareDaoBaseOjb implements FiscalYearMaker {
    ...
    protected boolean hasExtension() {
        if ( hasExtension == null ) {
            hasExtension = persistenceStructureService.hasReference(businessObjectClass, OLEPropertyConstants.EXTENSION);
        }
    ...
    public Criteria createSelectionCriteria(Integer baseFiscalYear) {
        ...
        if (MutableInactivatable.class.isAssignableFrom(businessObjectClass) && fields.contains(OLEPropertyConstants.ACTIVE) && !carryForwardInactive) {
