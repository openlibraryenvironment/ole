...
public class ObjectCodeDescriptionFormatter extends CodeDescriptionFormatterBase {
    ...
    protected Map<String, PersistableBusinessObject> getValuesToBusinessObjectsMap(Set values) {
        ...
        criteria.put(OLEConstants.UNIVERSITY_FISCAL_YEAR_PROPERTY_NAME, universityFiscalYear);
        criteria.put(OLEConstants.CHART_OF_ACCOUNTS_CODE_PROPERTY_NAME, chartOfAccountsCode);
        criteria.put(OLEConstants.FINANCIAL_OBJECT_CODE_PROPERTY_NAME, values);
        Collection<ObjectCode> coll = SpringContext.getBean(BusinessObjectService.class).findMatchingOrderBy(ObjectCode.class, criteria, OLEConstants.VERSION_NUMBER, true);
