...
public class PurchaseOrderRetransmitDocumentPresentationController extends PurchaseOrderDocumentPresentationController {
    ...
    // MSU Contribution OLEMI-8595 DTT-2396 OLECNTRB-946
    @Override
    protected boolean canPreviewPrintPo(PurchaseOrderDocument poDocument) {
        ...
        if (canPreviewPrintPo) {
            return !(poDocument.getDocumentHeader().getWorkflowDocument().isEnroute() && PurchaseOrderStatuses.APPDOC_CHANGE_IN_PROCESS
                    .equals(poDocument.getApplicationDocumentStatus()));
        }
