...
public class CashReceiptCheckAmountPositiveValidation extends GenericValidation {
    ...
    public boolean validate(AttributedDocumentEvent event) {
        if (getCheckForValidation().getAmount().isNegative()) {
            GlobalVariables.getMessageMap().putError(OLEPropertyConstants.CHECK_AMOUNT, OLEKeyConstants.CashReceipt.ERROR_NEGATIVE_CHECK_AMOUNT, OLEPropertyConstants.CHECKS);
            return false;
        }
        return true;
    }
