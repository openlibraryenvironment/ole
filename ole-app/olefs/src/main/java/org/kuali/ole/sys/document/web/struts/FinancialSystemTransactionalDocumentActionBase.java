 ...
 */
package org.kuali.ole.sys.document.web.struts;
...
public class FinancialSystemTransactionalDocumentActionBase extends KualiTransactionalDocumentActionBase {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if( isDocumentLocked(mapping, form, request)) {
            String message  = SpringContext.getBean(ParameterService.class).getParameterValueAsString(OLEConstants.CoreModuleNamespaces.OLE, OleParameterConstants.PARAMETER_ALL_DETAIL_TYPE, OLEConstants.DOCUMENT_LOCKOUT_DEFAULT_MESSAGE);
    ...
    protected boolean isDocumentLocked(ActionMapping mapping , ActionForm form, HttpServletRequest request) {
        ...
        boolean exists = SpringContext.getBean(ParameterService.class).parameterExists(document.getClass(),OLEConstants.DOCUMENT_LOCKOUT_PARM_NM );
        if(exists) {
            boolean documentLockedOut = SpringContext.getBean(ParameterService.class).getParameterValueAsBoolean(document.getClass(),OLEConstants.DOCUMENT_LOCKOUT_PARM_NM );
