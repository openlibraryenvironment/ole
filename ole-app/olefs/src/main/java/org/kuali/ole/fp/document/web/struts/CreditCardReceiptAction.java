...
import org.kuali.ole.fp.document.validation.impl.CreditCardReceiptDocumentRuleUtil;
...
public class CreditCardReceiptAction extends CapitalAccountingLinesActionBase {
    ...
    protected boolean validateNewCreditCardReceipt(CreditCardDetail creditCardReceipt) {
        GlobalVariables.getMessageMap().addToErrorPath(OLEPropertyConstants.NEW_CREDIT_CARD_RECEIPT);
        boolean isValid = CreditCardReceiptDocumentRuleUtil.validateCreditCardReceipt(creditCardReceipt);
        GlobalVariables.getMessageMap().removeFromErrorPath(OLEPropertyConstants.NEW_CREDIT_CARD_RECEIPT);
        return isValid;
    }
