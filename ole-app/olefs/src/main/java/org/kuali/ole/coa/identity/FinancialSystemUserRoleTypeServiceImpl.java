...
/**
 * This role type service was designed to be attached to the OLE-SYS User role
 */
public class FinancialSystemUserRoleTypeServiceImpl extends RoleTypeServiceBase {
    ...
    protected boolean performMatch(Map<String,String> qualification, Map<String,String> roleQualifier) {
        ...
        if (StringUtils.isBlank(roleQualifier.get(OleKimAttributes.CHART_OF_ACCOUNTS_CODE)) || StringUtils.isBlank(roleQualifier.get(OleKimAttributes.ORGANIZATION_CODE))) {
            return false;
        }
        // exact match on namespace or qualifier namespace of ole-sys works
        if (StringUtils.equals(roleQualifier.get(KimConstants.AttributeConstants.NAMESPACE_CODE), qualification.get(KimConstants.AttributeConstants.NAMESPACE_CODE))
                || StringUtils.equals(OLEConstants.CoreModuleNamespaces.OLE, roleQualifier.get(KimConstants.AttributeConstants.NAMESPACE_CODE))) {
            return true;
        }
        return false;
    }
    ...
    public List<RoleMembership> getMatchingRoleMemberships(Map<String,String> qualification, List<RoleMembership> roleMemberList) {
        ...
        for (RoleMembership rmi : roleMemberList) {
            ...
            if (!(StringUtils.isBlank(rmi.getQualifier().get(OleKimAttributes.CHART_OF_ACCOUNTS_CODE)) || StringUtils.isBlank(rmi.getQualifier().get(OleKimAttributes.ORGANIZATION_CODE)))) {
                ...
                else if (StringUtils.equals(OLEConstants.CoreModuleNamespaces.OLE, rmi.getQualifier().get(KimConstants.AttributeConstants.NAMESPACE_CODE))) {
    ...
    /**
     * note: for validating OLE-SYS User membership
    ...
    public List<RemotableAttributeError> validateAttributes(String kimTypeId, Map<String,String> attributes) {
        ...
        if(StringUtils.isBlank(chartCode) && StringUtils.isBlank(organizationCode)){
            ...
            removeError(OleKimAttributes.CHART_OF_ACCOUNTS_CODE, errorList );
            removeError(OleKimAttributes.ORGANIZATION_CODE, errorList );
          ...
          //- none are required if not
            ...
            if(StringUtils.isBlank(chartCode) || StringUtils.isBlank(organizationCode) || StringUtils.isBlank(namespaceCode)) {
                errorList.add(RemotableAttributeError.Builder.create(KimConstants.AttributeConstants.NAMESPACE_CODE, OLEKeyConstants.ERROR_CHART_OR_ORG_NOTEMPTY_ALL_REQUIRED).build());
            }
        }
