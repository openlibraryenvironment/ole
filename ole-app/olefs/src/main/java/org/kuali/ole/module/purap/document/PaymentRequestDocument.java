...
public class PaymentRequestDocument extends AccountsPayableDocumentBase {
    ...
    protected Integer invoiceIdentifier;
     ...
     *
     * @param paymentRequestPositiveApprovalIndicator
     *         The paymentRequestPositiveApprovalIndicator to set.
     */
            ...
            if (this.getFinancialSystemDocumentHeader().getWorkflowDocument().isProcessed()) {
                if (!PaymentRequestStatuses.APPDOC_AUTO_APPROVED.equals(getApplicationDocumentStatus())) {
                  ...
                  //  updateAndSaveAppDocStatus(PurapConstants.PaymentRequestStatuses.APPDOC_DEPARTMENT_APPROVED);
                    ...
                    payMap.put("purchaseOrderIdentifier",this.getPurchaseOrderIdentifier());
                    List<OlePaymentRequestDocument> olePaymentRequestDocument = (List<OlePaymentRequestDocument>) KRADServiceLocator.getBusinessObjectService().findMatching(OlePaymentRequestDocument.class,payMap);

                    if(!(SpringContext.getBean(OlePaymentRequestService.class).getPaymentMethod(olePaymentRequestDocument.get(0).getPaymentMethodId()).equals(OLEConstants.DEPOSIT)))   {
                        updateAndSaveAppDocStatus(PurapConstants.PaymentRequestStatuses.APPDOC_DEPARTMENT_APPROVED);
                    }
                    else {
    ...
    /**
     * @see org.kuali.ole.module.purap.document.AccountsPayableDocumentBase#processNodeChange(java.lang.String, java.lang.String)
     */
    @Override
    ...
    /**
     * @see org.kuali.ole.module.purap.document.PurchasingAccountsPayableDocumentBase#getItem(int)
     */
    @Override
