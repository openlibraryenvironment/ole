...
import static org.kuali.ole.sys.OLEPropertyConstants.REVERSAL_DATE;
...
public class JournalVoucherReversalDateValidation extends GenericValidation {
    ...
    public boolean validate(AttributedDocumentEvent event) {
        ...
        if (null != getJournalVoucherForValidation().getReversalDate() && getJournalVoucherForValidation().getReversalDate().before(today)) {
            GlobalVariables.getMessageMap().putError(DOCUMENT_ERROR_PREFIX + REVERSAL_DATE, OLEKeyConstants.ERROR_DOCUMENT_INCORRECT_REVERSAL_DATE);
            return false;
        }
        return true;
    }
