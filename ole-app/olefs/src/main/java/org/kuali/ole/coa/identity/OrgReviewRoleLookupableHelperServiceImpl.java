...
public class OrgReviewRoleLookupableHelperServiceImpl extends KualiLookupableHelperServiceImpl {
    ...
    public List<HtmlData> getCustomActionUrls(BusinessObject businessObject, List pkNames){
        ...
        if(StringUtils.isNotBlank(getMaintenanceDocumentTypeName()) && allowsMaintenanceEditAction(businessObject) && !orr.isDelegate()) {
            ...
            //createDelegationUrl.setDisplayText(OLEConstants.COAConstants.ORG_REVIEW_ROLE_CREATE_DELEGATION_DISPLAY_TEXT);
    ...
    protected AnchorHtmlData getCreateDelegationUrl(BusinessObject businessObject, List pkNames){
        ...
        parameters.put(OLEConstants.DISPATCH_REQUEST_PARAMETER, KRADConstants.MAINTENANCE_EDIT_METHOD_TO_CALL);
        ...
        return new AnchorHtmlData(href, KRADConstants.DOC_HANDLER_METHOD, OLEConstants.COAConstants.ORG_REVIEW_ROLE_CREATE_DELEGATION_DISPLAY_TEXT);
    }
    ...
    protected List<? extends BusinessObject> getMemberSearchResults(Map<String, String> fieldValues){
        ...
        String documentTypeName = fieldValues.get(OleKimAttributes.FINANCIAL_SYSTEM_DOCUMENT_TYPE_CODE);
        List<OleKimDocDelegateMember> searchResultsDelegationMembers = new ArrayList<OleKimDocDelegateMember>();
        ...
        flattenedSearchResults.addAll(flattenToOrgReviewMembers(fieldValues.get(OLEPropertyConstants.ACTIVE), documentTypeName, searchResultsRoleMembers));
        flattenedSearchResults.addAll(flattenToOrgReviewDelegationMembers(fieldValues.get(OLEPropertyConstants.ACTIVE), documentTypeName, searchResultsDelegationMembers));
    ...
    protected void filterOrgReview(Map<String, String> fieldValues, List<OrgReviewRole> searchResults){
        ...
        String financialSystemDocumentTypeCode = fieldValues.get(OleKimAttributes.FINANCIAL_SYSTEM_DOCUMENT_TYPE_CODE);
        String chartOfAccountsCode = fieldValues.get(OLEPropertyConstants.CHART_OF_ACCOUNTS_CODE);
        String organizationCode = fieldValues.get(OLEPropertyConstants.ORGANIZATION_CODE);
    ...
    protected List<OleKimDocDelegateMember> searchDelegations(Map<String, String> searchCriteriaDelegateMembers){
        ...
        ArrayList<OleKimDocDelegateMember> results = new ArrayList<OleKimDocDelegateMember>(totalRowCount);
        if ( totalRowCount > 0 ) {
            for ( DelegateMember dm : tmpResults ) {
                results.add( new OleKimDocDelegateMember( dm ) );
            }
        }
        return results;
    }
    ...
    protected List<OrgReviewRole> flattenToOrgReviewDelegationMembers(String active, String documentTypeName, List<OleKimDocDelegateMember> delegationMembers){
        ...
        List<OrgReviewRole> orgReviewRoles = new ArrayList<OrgReviewRole>();
        boolean activeInd = getBooleanValueForString(active, true);
        for(OleKimDocDelegateMember member: delegationMembers){
            if ( LOG.isDebugEnabled() ) {
                LOG.debug( "Converting Delegation Member: " + member );
            }
            if( StringUtils.isBlank(active) || activeInd == member.isActive() ) {
                OrgReviewRole orr = new OrgReviewRole();
                orgReviewRoleService.populateOrgReviewRoleFromDelegationMember(orr, member.getRoleMemberId(),
                        member.getDelegationMemberId());
                if ( LOG.isDebugEnabled() ) {
    ...
    protected String getQueryString(String parameter){
        if(StringUtils.isBlank(parameter)) {
            return OLEConstants.WILDCARD_CHARACTER;
        } else {
            return OLEConstants.WILDCARD_CHARACTER+parameter+OLEConstants.WILDCARD_CHARACTER;
        }
    }
    ...
    protected Map<String, String> buildOrgReviewRoleSearchCriteria(String documentTypeName, Map<String, String> fieldValues){
        ...
        if(StringUtils.isNotBlank(chartOfAccountsCode)){
            searchCriteriaMain.put(MEMBER_ATTRIBUTE_NAME_KEY, OleKimAttributes.CHART_OF_ACCOUNTS_CODE);
        ...
        if(StringUtils.isNotBlank(organizationCode)){
            searchCriteriaMain.put(MEMBER_ATTRIBUTE_NAME_KEY, OleKimAttributes.ORGANIZATION_CODE);
    ...
    public void validateSearchParameters(Map fieldValues) {
        if ( fieldValues != null ) {
            orgReviewRoleService.validateDocumentType((String)fieldValues.get(OleKimAttributes.FINANCIAL_SYSTEM_DOCUMENT_TYPE_CODE));
        }
