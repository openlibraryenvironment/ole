...
public class SubAccountMaintainableImpl extends FinancialSystemMaintainable {
    ...
    public static final String[] COA_CODE_NAMES = {        
        OLEPropertyConstants.A21_SUB_ACCOUNT + "." + OLEPropertyConstants.COST_SHARE_SOURCE_CHART_OF_ACCOUNTS_CODE, 
    };
    public static final String[] ACCOUNT_NUMBER_NAMES = {        
        OLEPropertyConstants.A21_SUB_ACCOUNT + "." + OLEPropertyConstants.COST_SHARE_SOURCE_ACCOUNT_NUMBER, 
    };
    ...
    public void refresh(String refreshCaller, Map fieldValues, MaintenanceDocument document) {
        ...
        boolean canEdit = !restrictions.isHiddenSectionId(OLEConstants.SUB_ACCOUNT_EDIT_CG_ICR_SECTION_ID) && !restrictions.isReadOnlySectionId(OLEConstants.SUB_ACCOUNT_EDIT_CG_ICR_SECTION_ID);
        
        // after account lookup, refresh the CG ICR account fields
        if (refreshCaller != null && (refreshCaller.toUpperCase().endsWith(OLEConstants.LOOKUPABLE_SUFFIX.toUpperCase())) && fieldValues.containsKey("document.newMaintainableObject.accountNumber") && canEdit) {
