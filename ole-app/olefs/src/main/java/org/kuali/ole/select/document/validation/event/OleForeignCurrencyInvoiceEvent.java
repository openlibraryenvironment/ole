...
public class OleForeignCurrencyInvoiceEvent extends KualiDocumentEventBase {

    private OleInvoiceItem payItem;

    protected OleForeignCurrencyInvoiceEvent(String errorPathPrefix, Document document) {
        super("Creating Note Tag" + getDocumentId(document), errorPathPrefix, document);
    }

    public OleForeignCurrencyInvoiceEvent(Document document, OleInvoiceItem payItem) {
        this("", document);
    ...
    public boolean invokeRuleMethod(BusinessRule rule) {
        return ((OleValidationRule) rule).processCustomForeignCurrencyInvoiceBusinessRules(document, payItem);
    }
