 ...
 */
public class UnlistedElectronicMonographLocalDataValueResolver extends LocalDataMappingValueResolver {
    @Override
    ...
    private String getAttributeValue() {
        ...
        PurchaseOrder.Order.UnlistedElectronicMonograph unlistedElectronicMonograph = order.getUnlistedElectronicMonograph();
        if (null != unlistedElectronicMonograph) {
            PurchaseOrder.Order.UnlistedElectronicMonograph.OrderDetail orderDetail = unlistedElectronicMonograph.getOrderDetail();
            ...
            if (getSourceFieldName().equalsIgnoreCase(GobiConstants.LOCAL_DATA1) ||
                ...
                orderDetail = unlistedElectronicMonograph.getOrderDetail();
                List<PurchaseOrder.Order.UnlistedElectronicMonograph.OrderDetail.LocalData> localData =
                        orderDetail.getLocalData();

                for (Iterator<PurchaseOrder.Order.UnlistedElectronicMonograph.OrderDetail.LocalData> iterator = localData.iterator(); iterator.hasNext(); ) {
                    PurchaseOrder.Order.UnlistedElectronicMonograph.OrderDetail.LocalData local = iterator.next();
