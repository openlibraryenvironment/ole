...
import org.kuali.ole.gl.batch.SufficientFundsAccountUpdateStep;
import org.kuali.ole.gl.batch.service.SufficientFundsAccountUpdateService;
...
public class SufficientFundsAccountUpdateServiceImpl implements SufficientFundsAccountUpdateService {
     ...
     * @see org.kuali.ole.gl.batch.service.SufficientFundsAccountUpdateService#rebuildSufficientFunds()
     */
    @Override
    public void rebuildSufficientFunds() { // driver
        ...
        if ( LOG.isInfoEnabled() ) {
            LOG.info( "Rebuilding sufficient funds for fiscal year: " + universityFiscalYear );
        }
        initService();
        ...
        LOG.info("rebuildSufficientFunds() Converting O types to A types");
        Map criteria = new HashMap();
        criteria.put(OLEPropertyConstants.ACCOUNT_FINANCIAL_OBJECT_TYPE_CODE, OLEConstants.SF_TYPE_OBJECT);
        ...
        LOG.info("rebuildSufficientFunds() Calculating SF balances for all A types");

        criteria.put(OLEPropertyConstants.ACCOUNT_FINANCIAL_OBJECT_TYPE_CODE, OLEConstants.SF_TYPE_ACCOUNT);
        ...
        // JHK: commented this out - the above purge deletes everything from the table and the rebuildSfrbList will only have O and A records which the code below ignored
//        LOG.info("rebuildSufficientFunds() Handle any non-A and non-O types");
//        for (Iterator iter = boService.findAll(SufficientFundRebuild.class).iterator(); iter.hasNext();) {
//            SufficientFundRebuild sfrb = (SufficientFundRebuild) iter.next();
//
//            if ((!OLEConstants.SF_TYPE_ACCOUNT.equals(sfrb.getAccountFinancialObjectTypeCode())) && (!OLEConstants.SF_TYPE_OBJECT.equals(sfrb.getAccountFinancialObjectTypeCode()))) {
//                ++sfrbRecordsReadCount;
//                transactionErrors = new ArrayList<Message>();
//                addTransactionError(kualiConfigurationService.getPropertyString(OLEKeyConstants.ERROR_INVALID_SF_OBJECT_TYPE_CODE));
//                ++warningCount;
//                ++sfrbNotDeletedCount;
//                reportWriterService.writeError(sfrb, transactionErrors);
//            }
//        }
        ...
        LOG.info("rebuildSufficientFunds() Create report");
    ...
    protected void initService() {
        ...
        if (options == null) {
            throw new IllegalStateException(kualiConfigurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_UNIV_DATE_NOT_FOUND));
        }
    }
    ...
    public void convertOtypeToAtypes(SufficientFundRebuild sfrb) {
        ...
        for (Iterator fundBalancesIter = fundBalances.iterator(); fundBalancesIter.hasNext();) {
            ...
            criteria.put(OLEPropertyConstants.CHART_OF_ACCOUNTS_CODE, sfbl.getChartOfAccountsCode());
            criteria.put(OLEPropertyConstants.ACCOUNT_FINANCIAL_OBJECT_TYPE_CODE, OLEConstants.SF_TYPE_ACCOUNT);
            criteria.put(OLEPropertyConstants.ACCOUNT_NUMBER_FINANCIAL_OBJECT_CODE, sfbl.getAccountNumber());
            ...
            if (altSfrb == null) {
                ...
                altSfrb.setAccountFinancialObjectTypeCode(OLEConstants.SF_TYPE_ACCOUNT);
    ...
    public void calculateSufficientFundsByAccount(SufficientFundRebuild sfrb) {
        ...
        if ((sfrbAccount.getAccountSufficientFundsCode() != null)
                && (OLEConstants.SF_TYPE_ACCOUNT.equals(sfrbAccount.getAccountSufficientFundsCode())
                        || OLEConstants.SF_TYPE_CASH_AT_ACCOUNT.equals(sfrbAccount.getAccountSufficientFundsCode())
                        || OLEConstants.SF_TYPE_CONSOLIDATION.equals(sfrbAccount.getAccountSufficientFundsCode())
                        || OLEConstants.SF_TYPE_LEVEL.equals(sfrbAccount.getAccountSufficientFundsCode())
                        || OLEConstants.SF_TYPE_OBJECT.equals(sfrbAccount.getAccountSufficientFundsCode())
                        || OLEConstants.SF_TYPE_NO_CHECKING.equals(sfrbAccount.getAccountSufficientFundsCode()))) {
            ++sfrbRecordsDeletedCount;
            ...
            if (OLEConstants.SF_TYPE_NO_CHECKING.equalsIgnoreCase(sfrbAccount.getAccountSufficientFundsCode())) {
            ...
            if (balancesIterator == null) {
                addTransactionError(kualiConfigurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_BALANCE_NOT_FOUND_FOR) + universityFiscalYear + ")");
                ++warningCount;
                ++sfrbNotDeletedCount;
                return;
            }
            ...
            while (balancesIterator.hasNext()) {
                ...
                if ( LOG.isDebugEnabled() ) {
                    LOG.debug("Processing Balance for SF: " + balance.getChartOfAccountsCode() + " / " + balance.getAccountNumber() + " / " + balance.getObjectCode() );
                }
                ...
                if ( LOG.isDebugEnabled() ) {
                    LOG.debug("SF Code: " + sfrbAccount.getAccountSufficientFundsCode() + " / Using SF Object Code: '" + tempFinObjectCd + "'" );
                }
                ...
                if (sfrbAccount.isForContractsAndGrants()) {
                    if ( LOG.isDebugEnabled() ) {
                        LOG.debug("Account is C&G.  Adding C&G beginning balance amount (" + balance.getContractsGrantsBeginningBalanceAmount() + ") to the annual balance amount.");
                    }
                ...
                if (OLEConstants.SF_TYPE_CASH_AT_ACCOUNT.equals(sfrbAccount.getAccountSufficientFundsCode())) {
                    if ( LOG.isDebugEnabled() ) {
                        LOG.debug("SF type is Cash at account");
                    }
            ...
            addTransactionError(kualiConfigurationService.getPropertyValueAsString(OLEKeyConstants.ERROR_INVALID_ACCOUNT_SF_CODE_FOR));
            ++warningCount;
            ++sfrbNotDeletedCount;
            return;
        }
    ...
    protected void processObjectOrAccount(Account sfrbAccount, Balance balance) {
        if (options.getFinObjTypeExpenditureexpCd().equals(balance.getObjectTypeCode()) || options.getFinObjTypeExpendNotExpCode().equals(balance.getObjectTypeCode()) || options.getFinObjTypeExpNotExpendCode().equals(balance.getObjectTypeCode()) || options.getFinancialObjectTypeTransferExpenseCd().equals(balance.getObjectTypeCode())) {
            ...
            if ( LOG.isDebugEnabled() ) {
                LOG.debug("Skipped balance record.  Object Type did not match.");
                LOG.debug( balance.getObjectTypeCode() + " NOT IN ( " + options.getFinObjTypeExpenditureexpCd() + "," + options.getFinObjTypeExpendNotExpCode() + "," + options.getFinObjTypeExpNotExpendCode() + "," + options.getFinancialObjectTypeTransferExpenseCd() );
            }
        }
    }
