...
public class KualiBatchFileAdminAction extends KualiAction {
    ...
    public ActionForward delete(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (question == null) {
            String questionText = kualiConfigurationService.getPropertyValueAsString(OLEKeyConstants.QUESTION_BATCH_FILE_ADMIN_DELETE_CONFIRM);
            ...
            if ("confirmDelete".equals(question)) {
                ...
                if (ConfirmationQuestion.YES.equals(buttonClicked)) {
                    try {
                        file.delete();
                        status = kualiConfigurationService.getPropertyValueAsString(OLEKeyConstants.MESSAGE_BATCH_FILE_ADMIN_DELETE_SUCCESSFUL);
                        status = MessageFormat.format(status, displayFileName);
                    }
                    catch (SecurityException e) {
                        status = kualiConfigurationService.getPropertyValueAsString(OLEKeyConstants.MESSAGE_BATCH_FILE_ADMIN_DELETE_ERROR);
                        status = MessageFormat.format(status, displayFileName);
                    }
                }
                else if (ConfirmationQuestion.NO.equals(buttonClicked)) {
                    status = kualiConfigurationService.getPropertyValueAsString(OLEKeyConstants.MESSAGE_BATCH_FILE_ADMIN_DELETE_CANCELLED);
                    status = MessageFormat.format(status, displayFileName);
                }
                if (status != null) {
