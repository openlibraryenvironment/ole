...
public class CollectorHeader extends PersistableBusinessObjectBase {
    ...
    protected LinkedHashMap toStringMapper_RICE20_REFACTORME() {
        ...
        m.put(OLEPropertyConstants.ORGANIZATION_CODE, this.organizationCode);
        if (this.processTransmissionDate != null) {
            m.put(OLEPropertyConstants.PROCESS_TRANSMISSION_DATE, this.processTransmissionDate.toString());
        }
        if (this.processBatchSequenceNumber != null) {
            m.put(OLEPropertyConstants.PROCESS_BATCH_SEQUENCE_NUMBER, this.processBatchSequenceNumber.toString());
        }
        if (this.processTotalRecordCount != null) {
            m.put(OLEPropertyConstants.PROCESS_TOTAL_RECORD_COUNT, this.processTotalRecordCount.toString());
        }
        if (this.processTotalAmount != null) {
            m.put(OLEPropertyConstants.PROCESS_TOTAL_AMOUNT, this.processTotalAmount.toString());
        }
        return m;
    }
