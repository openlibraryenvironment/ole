...
public class OlePaymentRequestDescEvent extends KualiDocumentEventBase {

    private OlePaymentRequestItem payItem;


    protected OlePaymentRequestDescEvent(String errorPathPrefix, Document document) {
        super("Creating Note Tag" + getDocumentId(document), errorPathPrefix, document);
    }

    public OlePaymentRequestDescEvent(Document document, OlePaymentRequestItem payItem) {
        this("", document);
    ...
    public boolean invokeRuleMethod(BusinessRule rule) {
        return ((OleValidationRule) rule).processCustomPaymentRequestDescriptionBusinessRules(document, payItem);
    }
