...
public class CreditCardReceiptMinimumCreditCardReceiptValidation extends GenericValidation {
    ...
    public boolean validate(AttributedDocumentEvent event) {
        ...
        if (ccr.getCreditCardReceipts().size() < requiredMinimumCount) {
            GlobalVariables.getMessageMap().putError(OLEPropertyConstants.NEW_CREDIT_CARD_RECEIPT, OLEKeyConstants.CreditCardReceipt.ERROR_DOCUMENT_CREDIT_CARD_RECEIPT_REQ_NUMBER_RECEIPTS_NOT_MET);
            return false;
        }
        return true;
    }
