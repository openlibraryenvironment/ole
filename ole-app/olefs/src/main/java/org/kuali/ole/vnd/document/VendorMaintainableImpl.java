...
public class VendorMaintainableImpl extends FinancialSystemMaintainable {
    ...
    public void saveBusinessObject() {
        ...
        VendorEventLog eventLog = new VendorEventLog();
        eventLog.setDate(new Timestamp(System.currentTimeMillis()));
        eventLog.setLogTypeId("3");
        eventLog.setUserId(GlobalVariables.getUserSession().getPrincipalId());
        if (vendorDetail.getVendorHeaderGeneratedIdentifier() != null && vendorDetail.getVendorDetailAssignedIdentifier() != null) {
            ...
            if (!vendorDetails.get(0).isActiveIndicator() && vendorDetail.isActiveIndicator()) {
                ...
                vendorMap.put(OLEConstants.GOKB_ID, vendorDetail.getGokbId());
                vendorMap.put(OLEConstants.OlePatron.PATRON_ACTIVE_IND, vendorDetail.isActiveIndicator());
                List<VendorDetail> vendorDetailList = (List<VendorDetail>) KRADServiceLocator.getBusinessObjectService().findMatching(VendorDetail.class, vendorMap);
                if (vendorDetailList.size() < 0) {
                    eventLog.setNote("Vendor is active :" + vendorDetail.getVendorInactiveReason().getVendorInactiveReasonDescription());
    ...
    public void processAfterEdit( MaintenanceDocument document, Map<String,String[]> parameters ) {
        ...
        Map hash = new HashMap();
            ...
            if(holdingsRecords.size()>0){
                vendorDetail.setLinkedToEHoldings(true);
            }else{
                vendorDetail.setLinkedToEHoldings(false);
            }
        }
