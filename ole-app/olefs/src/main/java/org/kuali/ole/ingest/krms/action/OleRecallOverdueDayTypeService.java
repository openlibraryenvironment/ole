 ...
 */
public class OleRecallOverdueDayTypeService extends ActionTypeServiceBase {
    ...
    public Action loadAction(ActionDefinition actionDefinition) {
       String parameter= actionDefinition.getAttributes().get("recallOverdueDay");
       return new OleRecallOverdueDayAction(parameter);
    }

    public class OleRecallOverdueDayAction implements Action{
        private int numberOfDays;

        public OleRecallOverdueDayAction(String numberOfDays) {
        ...
        public void execute(ExecutionEnvironment environment) {
            ...
            if(dataCarrierService.getData(OLEConstants.ERROR_ACTION)!=null){
                errors.addAll((List<String>)dataCarrierService.getData(OLEConstants.ERROR_ACTION));
            }
            List<Integer> listOfOverDueDays = (List<Integer>) dataCarrierService.getData(OLEConstants.LIST_RECALLED_OVERDUE_DAYS);
            if(listOfOverDueDays!=null){
                for(Integer overDueDays : listOfOverDueDays){
                    if(overDueDays>=numberOfDays){
                        errors.add(OLEConstants.RECALL_OVERDUE_DAY_LIMIT_ERROR);
