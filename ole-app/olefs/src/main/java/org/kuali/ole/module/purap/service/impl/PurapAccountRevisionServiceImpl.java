...
public class PurapAccountRevisionServiceImpl implements PurapAccountRevisionService {
    ...
    /**
     * @see org.kuali.ole.module.purap.service.PurapAccountHistoryService#savePaymentRequestAccountHistories(java.util.List,
     ...
     *      java.lang.Integer, java.lang.String)
     */
    public void saveInvoiceAccountRevisions(List<InvoiceItem> invoiceItems, Integer postingYear, String postingPeriodCode) {
        List<InvoiceAccountRevision> accountHistories = new ArrayList<InvoiceAccountRevision>();
        for (InvoiceItem item : invoiceItems) {
            ...
            for (PurapAccountRevisionGroup histGroup : historyAcctLineGroups.keySet()) {
                ...
                // build history record and save
                InvoiceAccountRevision history = (InvoiceAccountRevision) histGroup.buildRevisionRecord(InvoiceAccountRevision.class);
                history.setAccountRevisionTimestamp(dateTimeService.getCurrentTimestamp());
                accountHistories.add(history);
                existList.add(histGroup);
            }
            ...
            for (PurapAccountRevisionGroup group : currentAcctLineGroups.keySet()) {
                if (!existList.contains(group)) {
                    // set change amount same as new amount
                    group.setChangeAmount(group.getAmount());
                    InvoiceAccountRevision history = (InvoiceAccountRevision) group.buildRevisionRecord(InvoiceAccountRevision.class);
                    history.setAccountRevisionTimestamp(dateTimeService.getCurrentTimestamp());
                    accountHistories.add(history);
                }
            }
    ...
    /**
     * @see org.kuali.ole.module.purap.service.PurapAccountHistoryService#saveCreditMemoAccountHistories(java.util.List,
     *      java.lang.Integer, java.lang.String)
     */
