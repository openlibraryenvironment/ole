...
import org.kuali.ole.fp.businessobject.VoucherSourceAccountingLine;
...
public class JournalVoucherAction extends VoucherAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.isNotBlank(journalVoucherForm.getOriginalBalanceType()) && !journalVoucherForm.getSelectedBalanceType().getCode().equals(journalVoucherForm.getOriginalBalanceType())) {
            returnForward = super.dispatchMethod(mapping, form, request, response, OLEConstants.CHANGE_JOURNAL_VOUCHER_BALANCE_TYPE_METHOD);
     ...
     * error correction.
     * 
     * @see org.kuali.ole.fp.document.web.struts.VoucherAction#correct(org.apache.struts.action.ActionMapping,
     *      org.apache.struts.action.ActionForm, javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)
     */
    @Override
    public ActionForward correct(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        jvDoc.refreshReferenceObject(OLEPropertyConstants.BALANCE_TYPE);
    ...
    protected int determineBalanceTypeEncumbranceChangeMode(JournalVoucherForm journalVoucherForm) throws Exception {
        ...
        if (origBalType.getCode().equals(OLEConstants.BALANCE_TYPE_EXTERNAL_ENCUMBRANCE) && !newBalType.getCode().equals(OLEConstants.BALANCE_TYPE_EXTERNAL_ENCUMBRANCE)) {
            balanceTypeExternalEncumbranceChangeMode = EXT_ENCUMB_TO_NON_EXT_ENCUMB;
        }
        else if (!origBalType.getCode().equals(OLEConstants.BALANCE_TYPE_EXTERNAL_ENCUMBRANCE) && newBalType.getCode().equals(OLEConstants.BALANCE_TYPE_EXTERNAL_ENCUMBRANCE)) {
    ...
    protected ActionForward processChangeBalanceTypeConfirmationQuestion(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (jvDoc.getSourceAccountingLines().size() != 0) {
            ...
            if (question == null) { // question hasn't been asked
                ...
                return this.performQuestionWithoutInput(mapping, form, request, response, OLEConstants.JOURNAL_VOUCHER_CHANGE_BALANCE_TYPE_QUESTION, message, OLEConstants.CONFIRMATION_QUESTION, OLEConstants.CHANGE_JOURNAL_VOUCHER_BALANCE_TYPE_METHOD, "");
            }
            else {
                String buttonClicked = request.getParameter(OLEConstants.QUESTION_CLICKED_BUTTON);
                if ((OLEConstants.JOURNAL_VOUCHER_CHANGE_BALANCE_TYPE_QUESTION.equals(question)) && ConfirmationQuestion.NO.equals(buttonClicked)) {
    ...
    protected String buildBalanceTypeChangeConfirmationMessage(JournalVoucherForm jvForm, ConfigurationService kualiConfiguration) throws Exception {
        ...
        if (balanceTypeAmountChangeMode == SINGLE_AMT_TO_CREDIT_DEBIT_MODE) {
            message = kualiConfiguration.getPropertyValueAsString(OLEKeyConstants.QUESTION_CHANGE_JV_BAL_TYPE_FROM_SINGLE_AMT_TO_CREDIT_DEBIT_MODE);
            ...
            if (balanceTypeExternalEncumbranceChangeMode == NON_EXT_ENCUMB_TO_EXT_ENCUMB) {
                newMessage = StringUtils.replace(message, "{3}", kualiConfiguration.getPropertyValueAsString(OLEKeyConstants.QUESTION_CHANGE_JV_BAL_TYPE_FROM_SINGLE_AMT_TO_EXT_ENCUMB_CREDIT_DEBIT_MODE));
            }
            else {
            ...
            message = kualiConfiguration.getPropertyValueAsString(OLEKeyConstants.QUESTION_CHANGE_JV_BAL_TYPE_FROM_CREDIT_DEBIT_TO_SINGLE_AMT_MODE);
            // see if we need the extra bit about the external encumbrance
            String newMessage = new String("");
            if (balanceTypeExternalEncumbranceChangeMode == EXT_ENCUMB_TO_NON_EXT_ENCUMB) {
                newMessage = StringUtils.replace(message, "{3}", kualiConfiguration.getPropertyValueAsString(OLEKeyConstants.QUESTION_CHANGE_JV_BAL_TYPE_FROM_EXT_ENCUMB_CREDIT_DEBIT_TO_SINGLE_AMT_MODE));
            }
            else {
                newMessage = StringUtils.replace(message, "{3}", "");
            }
            ...
            message = kualiConfiguration.getPropertyValueAsString(OLEKeyConstants.QUESTION_CHANGE_JV_BAL_TYPE_FROM_EXT_ENCUMB_TO_NON_EXT_ENCUMB);
        }
        else if (balanceTypeExternalEncumbranceChangeMode == NON_EXT_ENCUMB_TO_EXT_ENCUMB) {
            message = kualiConfiguration.getPropertyValueAsString(OLEKeyConstants.QUESTION_CHANGE_JV_BAL_TYPE_FROM_NON_EXT_ENCUMB_TO_EXT_ENCUMB);
        }
    ...
    protected void switchFromSingleAmountModeToCreditDebitMode(JournalVoucherForm journalVoucherForm) {
        ...
        for (int i = 0; i < sourceLines.size(); i++) {
            ...
            sourceLine.setDebitCreditCode(OLEConstants.GL_DEBIT_CODE); // default to debit
    ...
    protected void switchFromCreditDebitModeToSingleAmountMode(JournalVoucherForm journalVoucherForm) {
        ...
        for (int i = 0; i < sourceLines.size(); i++) {
            ...
            sourceLine.setDebitCreditCode(OLEConstants.GL_DEBIT_CODE); // single sided is always debit
    ...
    protected ActionForward processRouteOutOfBalanceDocumentConfirmationQuestion(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (question == null) { // question hasn't been asked
            ...
            if (jvDoc.getBalanceType().isFinancialOffsetGenerationIndicator()) {
                ...
                message = StringUtils.replace(kualiConfiguration.getPropertyValueAsString(OLEKeyConstants.QUESTION_ROUTE_OUT_OF_BALANCE_JV_DOC_SINGLE_AMT_MODE), "{0}", currencyFormattedTotal);
            }
    ...
    protected void uploadAccountingLines(boolean isSource, ActionForm form) throws FileNotFoundException, IOException {
        ...
        jvForm.getJournalVoucherDocument().refreshReferenceObject(OLEPropertyConstants.BALANCE_TYPE);
