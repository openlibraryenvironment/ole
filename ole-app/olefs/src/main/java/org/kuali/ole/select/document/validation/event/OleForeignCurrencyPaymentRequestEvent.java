...
public class OleForeignCurrencyPaymentRequestEvent extends KualiDocumentEventBase {
    ...
    protected OleForeignCurrencyPaymentRequestEvent(String errorPathPrefix, Document document) {
        super("Creating Note Tag" + getDocumentId(document), errorPathPrefix, document);
    }

    public OleForeignCurrencyPaymentRequestEvent(Document document, OlePaymentRequestItem payItem) {
        this("", document);
    ...
    public boolean invokeRuleMethod(BusinessRule rule) {
        return ((OleValidationRule) rule).processCustomForeignCurrencyPaymentRequestBusinessRules(document, payItem);
    }
