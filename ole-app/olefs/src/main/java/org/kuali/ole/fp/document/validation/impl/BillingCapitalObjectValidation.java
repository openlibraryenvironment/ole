...
import org.kuali.ole.fp.document.InternalBillingDocument;
    ...
    public boolean validate(AttributedDocumentEvent event) {
        ...
        if(GlobalVariables.getMessageMap().getNumberOfPropertiesWithErrors() < 1) {
            if (accountingLineForValidation.isSourceAccountingLine() && isCapitalObject(accountingLineForValidation)) {
                GlobalVariables.getMessageMap().putError(OLEPropertyConstants.FINANCIAL_OBJECT_CODE, OLEKeyConstants.ERROR_DOCUMENT_IB_CAPITAL_OBJECT_IN_INCOME_SECTION);
                result = false;
            }
            // TODO phase II
