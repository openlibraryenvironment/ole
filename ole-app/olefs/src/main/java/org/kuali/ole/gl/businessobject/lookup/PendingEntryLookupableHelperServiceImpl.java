...
import org.kuali.ole.gl.businessobject.inquiry.EntryInquirableImpl;
import org.kuali.ole.gl.businessobject.inquiry.InquirableFinancialDocument;
...
public class PendingEntryLookupableHelperServiceImpl extends AbstractGeneralLedgerLookupableHelperServiceImpl {
    ...
    public HtmlData getInquiryUrl(BusinessObject businessObject, String propertyName) {
        if (OLEPropertyConstants.DOCUMENT_NUMBER.equals(propertyName) && businessObject instanceof GeneralLedgerPendingEntry) {
    ...
    public void validateSearchParameters(Map fieldValues) {
        ...
        if (!StringUtils.isEmpty(valueFiscalYear)) {
            try {
                ...
                GlobalVariables.getMessageMap().putError(UNIVERSITY_FISCAL_YEAR, OLEKeyConstants.ERROR_CUSTOM, new String[] { OLEKeyConstants.PendingEntryLookupableImpl.FISCAL_YEAR_FOUR_DIGIT });
                throw new ValidationException("errors in search criteria");
            }
        }

        if (!allRequiredsForAccountSearch(fieldValues) && !allRequiredsForDocumentSearch(fieldValues)) {
            GlobalVariables.getMessageMap().putError("universityFiscalYear", OLEKeyConstants.ERROR_GL_LOOKUP_PENDING_ENTRY_NON_MATCHING_REQUIRED_FIELDS, new String[] {});
            throw new ValidationException("errors in search criteria");
        }
    }
    ...
    public List getSearchResults(Map fieldValues) {
        ...
        String limitConfig = parameterService.getParameterValueAsString(OleParameterConstants.NERVOUS_SYSTEM_LOOKUP.class, OLEConstants.LOOKUP_RESULTS_LIMIT_URL_KEY);
        Integer limit = null;
        if (limitConfig != null) {
