...
public class DisbursementVoucherNonEmployeeTravelValidation extends GenericValidation {
    ...
    public boolean validate(AttributedDocumentEvent event) {  
        ...
        errors.addToErrorPath(OLEPropertyConstants.DV_NON_EMPLOYEE_TRAVEL);
        ...
        if (OLEConstants.COUNTRY_CODE_UNITED_STATES.equals(nonEmployeeTravel.getDvTravelFromCountryCode()) && StringUtils.isBlank(nonEmployeeTravel.getDisbVchrTravelFromStateCode())) {
            errors.putError(OLEPropertyConstants.DISB_VCHR_TRAVEL_FROM_STATE_CODE, OLEKeyConstants.ERROR_DV_TRAVEL_FROM_STATE);
            isValid = false;
        }
        
        if (OLEConstants.COUNTRY_CODE_UNITED_STATES.equals(nonEmployeeTravel.getDisbVchrTravelToCountryCode()) && StringUtils.isBlank(nonEmployeeTravel.getDisbVchrTravelToStateCode())) {
            errors.putError(OLEPropertyConstants.DISB_VCHR_TRAVEL_TO_STATE_CODE, OLEKeyConstants.ERROR_DV_TRAVEL_TO_STATE);
            isValid = false;
        }

        if (!isValid) {
            errors.removeFromErrorPath(OLEPropertyConstants.DV_NON_EMPLOYEE_TRAVEL);
        ...
        if (perDiemSectionComplete) { // Only validate if per diem section is filled in
            if (nonEmployeeTravel.getDisbVchrPerdiemCalculatedAmt().compareTo(nonEmployeeTravel.getDisbVchrPerdiemActualAmount()) != 0 && StringUtils.isBlank(nonEmployeeTravel.getDvPerdiemChangeReasonText())) {
                errors.putError(OLEPropertyConstants.DV_PERDIEM_CHANGE_REASON_TEXT, OLEKeyConstants.ERROR_DV_PERDIEM_CHANGE_REQUIRED);
                isValid = false;
            }
        }
        ...
        if (perDiemSectionComplete) { // Only validate if per diem section is filled in
            ...
            if (calculatedPerDiem.compareTo(nonEmployeeTravel.getDisbVchrPerdiemCalculatedAmt()) != 0) {
                errors.putErrorWithoutFullErrorPath(OLEConstants.GENERAL_NONEMPLOYEE_TAB_ERRORS, OLEKeyConstants.ERROR_DV_PER_DIEM_CALC_CHANGE);
                isValid = false;
            }
        }
        ...
        if (personalVehicleSectionComplete) {
            ...
            if (ObjectUtils.isNotNull(currentCalcAmt) && ObjectUtils.isNotNull(currentActualAmt)) {
                ...
                if (calculatedMileageAmount.compareTo(document.getDvNonEmployeeTravel().getDisbVchrMileageCalculatedAmt()) != 0) {
                    errors.putErrorWithoutFullErrorPath(OLEConstants.GENERAL_NONEMPLOYEE_TAB_ERRORS, OLEKeyConstants.ERROR_DV_MILEAGE_CALC_CHANGE);
                    isValid = false;
                }
                ...
                if (performTravelMileageLimitInd) {
                    ...
                    if (currentCalcAmt.subtract(currentActualAmt).isNegative()) {
                        errors.putError(OLEPropertyConstants.DV_PERSONAL_CAR_AMOUNT, OLEKeyConstants.ERROR_DV_ACTUAL_MILEAGE_TOO_HIGH);
                        isValid = false;
                    }
                }
            }
    ...
    protected boolean validateTravelAmount(DisbursementVoucherDocument document) {
        ...
        if (paidAmount.compareTo(travelAmount) != 0) {
            GlobalVariables.getMessageMap().putErrorWithoutFullErrorPath(OLEConstants.DV_CHECK_TRAVEL_TOTAL_ERROR, OLEKeyConstants.ERROR_DV_TRAVEL_CHECK_TOTAL);
            return false;
        }
        return true;
    }
    ...
    private boolean validatePerDiemSection(DisbursementVoucherDocument document, MessageMap errors) {
        ...
        if (perDiemUsed) {
            if (StringUtils.isBlank(document.getDvNonEmployeeTravel().getDisbVchrPerdiemCategoryName())) {
                errors.putError(OLEPropertyConstants.DISB_VCHR_PERDIEM_CATEGORY_NAME, OLEKeyConstants.ERROR_DV_PER_DIEM_CATEGORY);
                perDiemSectionComplete = false;
            }
            if (ObjectUtils.isNull(document.getDvNonEmployeeTravel().getDisbVchrPerdiemRate())) {
                errors.putError(OLEPropertyConstants.DISB_VCHR_PERDIEM_RATE, OLEKeyConstants.ERROR_DV_PER_DIEM_RATE);
                perDiemSectionComplete = false;
            }
            if (ObjectUtils.isNull(document.getDvNonEmployeeTravel().getDisbVchrPerdiemCalculatedAmt())) {
                errors.putError(OLEPropertyConstants.DISB_VCHR_PERDIEM_CALCULATED_AMT, OLEKeyConstants.ERROR_DV_PER_DIEM_CALC_AMT);
                perDiemSectionComplete = false;
            }
            if (ObjectUtils.isNull(document.getDvNonEmployeeTravel().getDisbVchrPerdiemActualAmount())) {
                errors.putError(OLEPropertyConstants.DISB_VCHR_PERDIEM_ACTUAL_AMOUNT, OLEKeyConstants.ERROR_DV_PER_DIEM_ACTUAL_AMT);
                perDiemSectionComplete = false;
            }
        }
    ...
    private boolean validatePersonalVehicleSection(DisbursementVoucherDocument document, MessageMap errors) {
        ...
        if (personalVehilcleUsed) {
            if (ObjectUtils.isNull(document.getDvNonEmployeeTravel().getDisbVchrAutoFromCityName())) {
                errors.putError(OLEPropertyConstants.DISB_VCHR_AUTO_FROM_CITY_NAME, OLEKeyConstants.ERROR_DV_AUTO_FROM_CITY);
                personalVehicleSectionComplete = false;
            }
            if (ObjectUtils.isNull(document.getDvNonEmployeeTravel().getDisbVchrAutoToCityName())) {
                errors.putError(OLEPropertyConstants.DISB_VCHR_AUTO_TO_CITY_NAME, OLEKeyConstants.ERROR_DV_AUTO_TO_CITY);
                personalVehicleSectionComplete = false;
            }
            ...
            if (ObjectUtils.isNull(document.getDvNonEmployeeTravel().getDisbVchrAutoFromStateCode())) {
                errors.putError(OLEPropertyConstants.DISB_VCHR_AUTO_FROM_STATE_CODE, OLEKeyConstants.ERROR_DV_AUTO_FROM_STATE);
                personalVehicleSectionComplete = false;
            }
            if (ObjectUtils.isNull(document.getDvNonEmployeeTravel().getDisbVchrAutoToStateCode())) {
                errors.putError(OLEPropertyConstants.DISB_VCHR_AUTO_TO_STATE_CODE, OLEKeyConstants.ERROR_DV_AUTO_TO_STATE);
                personalVehicleSectionComplete = false;
            }
            ...
            if (ObjectUtils.isNull(document.getDvNonEmployeeTravel().getDvPersonalCarMileageAmount())) {
                errors.putError(OLEPropertyConstants.DV_PERSONAL_CAR_MILEAGE_AMOUNT, OLEKeyConstants.ERROR_DV_MILEAGE_AMT);
                personalVehicleSectionComplete = false;
            }
            if (ObjectUtils.isNull(document.getDvNonEmployeeTravel().getDisbVchrMileageCalculatedAmt())) {
                errors.putError(OLEPropertyConstants.DISB_VCHR_MILEAGE_CALCULATED_AMT, OLEKeyConstants.ERROR_DV_MILEAGE_CALC_AMT);
                personalVehicleSectionComplete = false;
            }
            if (ObjectUtils.isNull(document.getDvNonEmployeeTravel().getDisbVchrPersonalCarAmount())) {
                errors.putError(OLEPropertyConstants.DISB_VCHR_PERSONAL_CAR_AMOUNT, OLEKeyConstants.ERROR_DV_MILEAGE_ACTUAL_AMT);
                personalVehicleSectionComplete = false;
            }
        }
