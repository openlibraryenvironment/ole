...
public class PurchasingServiceImpl extends PersistenceServiceStructureImplBase implements PurchasingService {
    ...
    public String getDefaultAssetTypeCodeNotThisFiscalYear() {
        ...
        return parameterService.getParameterValueAsString(OleParameterConstants.CAPITAL_ASSET_BUILDER_DOCUMENT.class, PurapParameterConstants.CapitalAsset.PURCHASING_DEFAULT_ASSET_TYPE_WHEN_NOT_THIS_FISCAL_YEAR);
    }
    ...
    public boolean checkValidRoomNumber(CapitalAssetLocation location) {
        ...
        if (StringUtils.isNotBlank(location.getBuildingCode()) && StringUtils.isNotBlank(location.getBuildingRoomNumber())) {
            ...
            objectKeys.put(OLEPropertyConstants.CAMPUS_CODE, location.getCampusCode());
            objectKeys.put(OLEPropertyConstants.BUILDING_CODE, location.getBuildingCode());
            objectKeys.put(OLEPropertyConstants.BUILDING_ROOM_NUMBER, location.getBuildingRoomNumber());
            ...
            if (ObjectUtils.isNull(room)) {
                ...
                String label = getDataDictionaryService().getDataDictionary().getBusinessObjectEntry(Room.class.getName()).getAttributeDefinition(OLEPropertyConstants.BUILDING_ROOM_NUMBER).getLabel();
