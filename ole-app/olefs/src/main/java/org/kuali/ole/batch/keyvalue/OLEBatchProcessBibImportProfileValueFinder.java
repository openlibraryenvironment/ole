 ...
 */
public class OLEBatchProcessBibImportProfileValueFinder extends KeyValuesBase  {

    private static final Logger LOG = Logger.getLogger(OLEBatchProcessBibImportProfileValueFinder.class);
    ...
    public List<KeyValue> getKeyValues() {
        ...
        List<KeyValue> options = initProfileDetailsForBibImport();
        return options;
    }

    public static List<KeyValue> initProfileDetailsForBibImport() {

        List<KeyValue> keyValues = new ArrayList<KeyValue>();
        BusinessObjectService businessObjectService = KRADServiceLocator.getBusinessObjectService();
        Map parentCriteria1 = new HashMap();
        parentCriteria1.put("batchProcessProfileType", "Bib Import");
    ...
    public static List<String> retrieveBatchProfileNamesForBibImport(String locationVal) {
        List<KeyValue> locationKeyValues = retrieveProfileDetailsForBibImport();
    ...
    private static List<KeyValue> retrieveProfileDetailsForBibImport() {
        long currentTime = System.currentTimeMillis() / 1000;
        if (locationKeyValues == null) {
            locationKeyValues = initProfileDetailsForBibImport();
    ...
    public  static String getValue(String batchVal){
        String value="";
        locationKeyValues=retrieveProfileDetailsForBibImport();
        for (KeyValue keyValue : locationKeyValues) {
