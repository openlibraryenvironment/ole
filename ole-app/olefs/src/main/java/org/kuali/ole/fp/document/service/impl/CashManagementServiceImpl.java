 ...
 */
package org.kuali.ole.fp.document.service.impl;
...
import org.kuali.ole.fp.businessobject.format.CashDrawerStatusCodeFormatter;
import org.kuali.ole.fp.document.CashManagementDocument;
import org.kuali.ole.fp.document.CashReceiptDocument;
import org.kuali.ole.fp.document.dataaccess.CashManagementDao;
...
import org.kuali.ole.fp.exception.CashDrawerStateException;
import org.kuali.ole.fp.exception.InvalidCashReceiptState;
import org.kuali.ole.fp.service.CashDrawerService;
import org.kuali.ole.sys.OLEConstants;
import org.kuali.ole.sys.OLEConstants.CashDrawerConstants;
import org.kuali.ole.sys.OLEConstants.CurrencyCoinSources;
import org.kuali.ole.sys.OLEConstants.DepositConstants;
import org.kuali.ole.sys.OLEConstants.DocumentStatusCodes;
...
public class CashManagementServiceImpl implements CashManagementService {
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#getCashManagementDocumentForCashReceiptId(java.lang.String)
     */
    @Override
    public CashManagementDocument getCashManagementDocumentForCashReceiptId(String documentId) {
        ...
        primaryKeys.put(OLEPropertyConstants.DOCUMENT_NUMBER, documentId);
        CashReceiptDocument crDoc = (CashReceiptDocument) businessObjectService.findByPrimaryKey(getDataDictionaryService().getDocumentClassByTypeName(OLEConstants.FinancialDocumentTypeCodes.CASH_RECEIPT), primaryKeys);
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#createCashManagementDocument(java.lang.String,
    ...
    public CashManagementDocument createCashManagementDocument(String campusCode, String docDescription, String annotation) {
        ...
        DocumentAuthorizer documentAuthorizer = SpringContext.getBean(DocumentHelperService.class).getDocumentAuthorizer(OLEConstants.FinancialDocumentTypeCodes.CASH_MANAGEMENT);
        documentAuthorizer.canInitiate(OLEConstants.FinancialDocumentTypeCodes.CASH_MANAGEMENT, user);
        ...
        if (cd.isClosed()) {
            ...
            try {
                ...
                throw new IllegalArgumentException("unable to create CashManagementDocument", e);
            }
        }
        else {
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#addInterimDeposit(org.kuali.ole.fp.document.CashManagementDocument,
     *      java.lang.String, org.kuali.ole.fp.businessobject.BankAccount, java.util.List)
     */
    @Override
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#cancelCashManagementDocument(org.kuali.ole.fp.document.CashManagementDocument)
     */
    @Override
    public void cancelCashManagementDocument(CashManagementDocument cmDoc) {
        ...
        String[] cashieringSourcesToDelete = { OLEConstants.CurrencyCoinSources.CASH_RECEIPTS, CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_IN, OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_OUT };
     ...
     * </ul>
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#cancelDeposit(org.kuali.ole.fp.businessobject.Deposit)
     */
    @Override
    public void cancelDeposit(Deposit deposit) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("deposit deposit type = "+deposit.getDepositTypeCode()+"; constant = "+OLEConstants.DepositConstants.DEPOSIT_TYPE_FINAL+"; are they equal? "+deposit.getDepositTypeCode().equals(OLEConstants.DepositConstants.DEPOSIT_TYPE_FINAL));
        }
        if (deposit.getDepositTypeCode().equals(OLEConstants.DepositConstants.DEPOSIT_TYPE_FINAL)) {
            CashDrawer drawer = cashDrawerService.getByCampusCode(deposit.getCashManagementDocument().getCampusCode());
            CurrencyDetail currencyDetail = cashManagementDao.findCurrencyDetailByCashieringStatus(deposit.getCashManagementDocument().getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.DEPOSITS);
            if (currencyDetail != null) {
                drawer.addCurrency(currencyDetail);
                businessObjectService.delete(currencyDetail);
            }
            CoinDetail coinDetail = cashManagementDao.findCoinDetailByCashieringStatus(deposit.getCashManagementDocument().getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.DEPOSITS);
            if (coinDetail != null) {
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#finalizeCashManagementDocument(org.kuali.ole.fp.document.CashManagementDocument)
     */
    @Override
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#retrieveCashReceipts(org.kuali.ole.fp.businessobject.Deposit)
     */
    @Override
    ...
    protected void finalizeCashReceiptsForDeposit(Deposit deposit) {
        ...
        for (Object o: cashReceipts) {
            CashReceiptDocument crDoc = (CashReceiptDocument)o;
            crDoc.getFinancialSystemDocumentHeader().setFinancialDocumentStatusCode(OLEConstants.DocumentStatusCodes.CashReceipt.FINAL);
            documentService.updateDocument(crDoc);
        }
    }
     ...
     * application rules.
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#applyCashieringTransaction(org.kuali.ole.fp.document.CashManagementDocument, org.kuali.ole.fp.businessobject.CashieringTransaction)
     * @see org.kuali.ole.fp.document.validation.impl.CashieringTransactionRule#processCashieringTransactionApplicationRules(CashManagementDocument)
     */
    @Override
    ...
    protected void saveChecks(CashManagementDocument cmDoc) {
        if (cmDoc.getChecks() != null) {
            for (Check check: cmDoc.getChecks()) {
                ...
                check.setCashieringStatus(OLEConstants.CheckSources.CASH_MANAGEMENT);
    ...
    protected void saveMoneyInCash(CashManagementDocument cmDoc, CashieringTransaction trans) {
        ...
        CoinDetail cumulativeMoneyInCoin = cashManagementDao.findCoinDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_IN);
        // add the new money in coin
        cumulativeMoneyInCoin.add(trans.getMoneyInCoin());
        // save the cumulative
        businessObjectService.save(cumulativeMoneyInCoin);

        CurrencyDetail cumulativeMoneyInCurrency = cashManagementDao.findCurrencyDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_IN);
    ...
    protected void saveMoneyOutCash(CashManagementDocument cmDoc, CashieringTransaction trans) {
        CoinDetail cumulativeMoneyOutCoin = cashManagementDao.findCoinDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_OUT);
        cumulativeMoneyOutCoin.add(trans.getMoneyOutCoin());
        businessObjectService.save(cumulativeMoneyOutCoin);

        CurrencyDetail cumulativeMoneyOutCurrency = cashManagementDao.findCurrencyDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_OUT);
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#getOpenItemsInProcess(org.kuali.ole.fp.document.CashManagementDocument)
     */
    @Override
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#getRecentlyClosedItemsInProcess(org.kuali.ole.fp.document.CashManagementDocument)
     */
    @Override
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#generateMasterCoinDetail(org.kuali.ole.fp.document.CashManagementDocument)
     */
    @Override
    public CoinDetail generateMasterCoinDetail(CashManagementDocument cmDoc) {
        ...
        masterDetail.setCashieringStatus(OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_MASTER);

        masterDetail.zeroOutAmounts();

        CoinDetail cashReceiptDetail = cashManagementDao.findCoinDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_RECEIPTS);
        if (cashReceiptDetail != null) {
            masterDetail.add(cashReceiptDetail);
        }

        CoinDetail depositDetail = cashManagementDao.findCoinDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.DEPOSITS);
        if (depositDetail != null) {
            masterDetail.subtract(depositDetail);
        }

        CoinDetail moneyInDetail = cashManagementDao.findCoinDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_IN);
        if (moneyInDetail != null) {
            masterDetail.add(moneyInDetail);
        }

        CoinDetail moneyOutDetail = cashManagementDao.findCoinDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_OUT);
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#generateMasterCurrencyDetail(org.kuali.ole.fp.document.CashManagementDocument)
     */
    @Override
    public CurrencyDetail generateMasterCurrencyDetail(CashManagementDocument cmDoc) {
        ...
        CurrencyDetail cashReceiptDetail = cashManagementDao.findCurrencyDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_RECEIPTS);
        if (cashReceiptDetail != null) {
            masterDetail.add(cashReceiptDetail);
        }

        CurrencyDetail depositDetail = cashManagementDao.findCurrencyDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.DEPOSITS);
        if (depositDetail != null) {
            masterDetail.subtract(depositDetail);
        }

        CurrencyDetail moneyInDetail = cashManagementDao.findCurrencyDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_IN);
        if (moneyInDetail != null) {
            masterDetail.add(moneyInDetail);
        }

        CurrencyDetail moneyOutDetail = cashManagementDao.findCurrencyDetailByCashieringStatus(cmDoc.getDocumentNumber(), CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.CASH_MANAGEMENT_OUT);
        if (moneyOutDetail != null) {
            masterDetail.subtract(moneyOutDetail);
        }
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#selectCashieringChecksForDeposit(java.lang.String, java.lang.Integer)
     */
    @Override
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#selectUndepositedCashieringChecks(java.lang.String)
     */
    @Override
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#selectDepositedCashieringChecks(java.lang.String)
     */
    @Override
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#calculateUndepositedCheckTotal(java.lang.String)
     */
    @Override
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#allowDocumentCancellation(org.kuali.ole.fp.document.CashManagementDocument)
     */
    @Override
    ...
    protected boolean existCashReceipts(CashManagementDocument cmDoc) {
        List<CashReceiptDocument> cashReceipts = SpringContext.getBean(CashReceiptService.class).getCashReceipts(cmDoc.getCampusCode(), new String[] {OLEConstants.DocumentStatusCodes.CashReceipt.VERIFIED, OLEConstants.DocumentStatusCodes.CashReceipt.INTERIM, OLEConstants.DocumentStatusCodes.CashReceipt.FINAL} );
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#selectNextAvailableCheckLineNumber(java.lang.String)
     */
    @Override
     ...
     *
     * @see org.kuali.ole.fp.document.service.CashManagementService#getCashDetailsForFinalDeposit(java.lang.String)
     */
    @Override
    public Map<Class, Object> getCashDetailsForFinalDeposit(String documentNumber) {
        CurrencyDetail finalDepositCurrencyDetail = cashManagementDao.findCurrencyDetailByCashieringStatus(documentNumber, CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.DEPOSITS);
        CoinDetail finalDepositCoinDetail = cashManagementDao.findCoinDetailByCashieringStatus(documentNumber, CashieringTransaction.DETAIL_DOCUMENT_TYPE, OLEConstants.CurrencyCoinSources.DEPOSITS);
