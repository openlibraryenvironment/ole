...
public enum RequisitionItemFixture {
    ...
    REQ_QTY_UNRESTRICTED_ITEM_3(false, // itemRestrictedIndicator
            PurApItemFixture.BASIC_QTY_ITEM_3, // purApItemFixture
    ...
    REQ_ITEM_NO_APO(false, // itemRestrictedIndicator
            ...
            new RequisitionAccountingLineFixture[] { RequisitionAccountingLineFixture.BASIC_REQ_ACCOUNT_3 } // requisitionAccountMultiFixtures
    ),
    ...
    ITEM_WITH_MULTI_COPIES_MULTI_PARTS(false, // itemRestrictedIndicator
            PurApItemFixture.BASIC_QTY_ITEM_FOR_COPIES, // purApItemFixture
            new RequisitionAccountingLineFixture[] { RequisitionAccountingLineFixture.BASIC_REQ_ACCOUNT_1 }, // requisitionAccountMultiFixtures
            OLERequisitionCopiesFixture.MULTI_COPIES_MULTI_PARTS //oleCopyFixture
    ),
    ...
    private OLERequisitionCopiesFixture oleRequisitionCopiesFixture;
    ...
    private RequisitionItemFixture(boolean itemRestrictedIndicator, PurApItemFixture purApItemFixture, RequisitionAccountingLineFixture[] requisitionAccountingLineFixtures, OLERequisitionCopiesFixture oleRequisitionCopiesFixture) {
        this.itemRestrictedIndicator = itemRestrictedIndicator;
        this.purApItemFixture = purApItemFixture;
        this.requisitionAccountingLineFixtures = requisitionAccountingLineFixtures;
        this.oleRequisitionCopiesFixture = oleRequisitionCopiesFixture;
    }
    ...
    public OleRequisitionItem createRequisitionItem() {
        OleRequisitionItem item = (OleRequisitionItem) purApItemFixture.createPurApItem(OleRequisitionItem.class);
        ...
        if (oleRequisitionCopiesFixture != null) {
            OleCopyHelperServiceImpl oleCopyHelperService = (OleCopyHelperServiceImpl) SpringContext.getBean(OleCopyHelperService.class);
            OleRequisitionCopies oleRequisitionCopies = oleRequisitionCopiesFixture.createOleRequisitionCopies();
            ...
            item.setOleRequisitionCopy(oleRequisitionCopies);
            List<OleCopy> copyList = oleCopyHelperService.setCopyValues(oleRequisitionCopies, item.getItemTitleId(), volChar);
            item.getCopyList().addAll(copyList);
            item.setItemNoOfParts(oleRequisitionCopies.getParts());
            item.getCopies().add(oleRequisitionCopies);
        }
        return item;
    }
