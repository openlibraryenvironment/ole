...
public final class AccountingDocumentTestUtils {
    ...
    public static void testRouteDocument(FinancialSystemTransactionalDocument document, DocumentService documentService) throws Exception {
        ...
        if (!document.getDocumentHeader().getWorkflowDocument().isApproved()) {
            WorkflowTestUtils.waitForStatusChange(document.getDocumentNumber(), DocumentStatus.ENROUTE,DocumentStatus.PROCESSED,DocumentStatus.FINAL);
        }
    }
    ...
    public static void testAddItemWithCopy(OleRequisitionDocument document, List<OleRequisitionItem> items, int expectedCopyTotal) throws Exception {
        ...
        OleRequisitionItem oleRequisitionItem = (OleRequisitionItem)document.getItems().get(0);
        assertEquals("copy count mismatch", expectedCopyTotal, oleRequisitionItem.getCopyList().size() );
    }
}
